 ...
 *
 */
public class MsgContainer2{
	private static Log log=LogFactory.getLog(MsgContainer2.class);
	...
	public static  DataInputStream getSocketDIS(){
		if(in==null||null==msgSocket||msgSocket.isClosed()||!msgSocket.isConnected()){
			try {
				in=new DataInputStream(MsgContainer2.getSocketInstance().getInputStream());
			} catch (IOException e) {
				in=null;
			}
		}
	...
	public static   DataOutputStream getSocketDOS(){
		if(out==null||null==msgSocket||msgSocket.isClosed()||!msgSocket.isConnected()){
			try {
				out=new DataOutputStream(MsgContainer2.getSocketInstance().getOutputStream());
			} catch (IOException e) {
				out=null;
			}
		}
	...
	private static  boolean connectISMG(){
		...
		connect.setAuthenticatorSource(MsgUtils.getAuthenticatorSource(MsgConfig.getSpId(), MsgConfig.getSpSharedSecret(), timestamp));//md5(企业代码+密匙+时间戳)
...
//			}
	...
	private static boolean sendShortMsg(String msg,String cusMsisdn){
		try {			
			...
			if (succ) {
				log.info("向手机号码："+cusMsisdn+"发送成功");
			} else {
				log.info("向手机号码："+cusMsisdn+"发送失败");
			}
			return succ;
		} catch (Exception e) {
			try {
