...
public class SysoutSyncThread extends Thread{
	private LazyDynaBean bean;
	private String hr_only_field;//人员的唯一指标
	private String outinfo="";
	boolean isSend = false;
	private String sendValue = "0";
	public SysoutSyncThread(LazyDynaBean bean,String hr_only_field)
	{
		this.bean=bean;
			...
			if(ishr||isOrg||isPost)
			{
				if ("1".equals(sendValue)) {// 发送变动通知
					...
					if(mess!=null&&mess.length()>0)
					{
						...
						updateSyncSysFlag(conn,mess);//根据返回消息处理同步信息
					}else
					{
						failinfo(conn,"接口返回信息内容为空！");
					}
...
//				mess=stub.sendSyncMsg(buf.toString());
	...
	private SysoutSyncInterf getSysoutSyncInterf(String url) {
		SysoutSyncInterf stub = null;
		try {
	...
	private String getMessage(String paramValue) {
		...
		try {
			...
			mess = SyncDataService.sendSyncMsg(paramValue);
