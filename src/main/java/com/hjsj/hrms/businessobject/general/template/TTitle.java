...
//		case 5://页码
		...
		case 7: //照片
             ...
             } else
			 {
				...
				strc.append("tp<img src='/servlet/DisplayOleContent?filename="+fileName+"' height='"+this.rheight+"' width='"+this.rwidth+"' />tp"); 
			 }
			break;
		default:
			strc.append(this.hz);
	...
	private String getFontStyle()
	{
		...
		double fontsize = this.getRealFontSize(this.fontsize);
		...
		if(this.hz.indexOf("\r")!=-1||this.hz.indexOf("\n")!=-1) {
			style.append(";white-space:pre-line");//在默认情况下空白会被忽略，所以无论是文本里的空白、换行都会被当做一个空格处理，若想显示空白，需添加属性white-space:  
			                                      //pre保留换行和空白 或者pre-line 保留换行，忽略空白  解决标题中带有换行得情况
		}
