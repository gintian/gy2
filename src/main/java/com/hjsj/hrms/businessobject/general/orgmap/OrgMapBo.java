...
public class OrgMapBo {
	...
	public String createOrgMap(List rs,List rootnode,String rootdesc,ParameterBo parameter,Connection conn,String code,boolean ishistory)
	{
		...
		else
		{
			...
			try{			
				_out = new FileOutputStream(System.getProperty("java.io.tmpdir")+System.getProperty("file.separator")
						+  url);
				...
				if(!rs.isEmpty())
				{
					if("true".equalsIgnoreCase(parameter.getGraphaspect()))
					{
						for(int i=0;i<rs.size();i++)
						{
							...
							//y=0;
							/*if(orgmapbean.get("text").toString().equals("普工处"))
                            ...
                            //if(orgmapbean.get("text")!=null&&orgmapbean.get("text").toString().equalsIgnoreCase("办公厅"))                            	
							if(!ishistory && "true".equalsIgnoreCase(parameter.getIsshowpersonconut())&&"true".equalsIgnoreCase(parameter.getIsshowpersonname())&&!"zz".equalsIgnoreCase(orgmapbean.get("codesetid").toString())&&!"xn".equalsIgnoreCase(orgmapbean.get("codesetid").toString()))
							{
							...
							{
								//System.out.println(orgmapbean.get("text").toString()+"--"+x+"--"+y+"--"+x1+"--"+x2+"--"+y1+"--"+y2);
	...
	private void createRectCell(Document document,PdfWriter writer,String text,String subhead,ParameterBo parameter,float x,float y,String codesetid) throws Exception
	{
		 ...
		 if("true".equalsIgnoreCase(parameter.getGraph3d()))
		 {
		     ...
		     //String aTextarr[]=compTextOnecell.oneLineWordCountPdf(parameter.getCellwidth(),parameter.getFontsize(),parameter.getCellheight(),text.length());
		     /*String aTextarr[]=compTextOnecell.oneTableWordCountPDF(parameter.getCellwidth(), parameter.getCellheight(), text, parameter.getFontsize());
		     String fontsize=aTextarr[0];
		     ...
		     if(_colscount>1)
		     {
		     	...
		     	/*for(int i=0;i<_colscount+1;i++)
		     	{
		     		...
		     		if(i==_colscount)
			      	{
		     			cb.showTextAligned(PdfContentByte.ALIGN_CENTER, text.substring(i*_wordcount), x+28+(Integer.parseInt(parameter.getCellwidth())/10)+alignx, y-Integer.parseInt(parameter.getCellheight())/4-i*20+valigny,0);
			      	}else if(i==0)
			      	{
			      		cb.showTextAligned(PdfContentByte.ALIGN_CENTER, text.substring(i*_wordcount,(i+1)*_wordcount), x+28+(Integer.parseInt(parameter.getCellwidth())/10), y-Integer.parseInt(parameter.getCellheight())/4-5,0);
			      	}else
			      	{
			      		cb.showTextAligned(PdfContentByte.ALIGN_CENTER, text.substring(i*_wordcount,(i+1)*_wordcount), x+28+(Integer.parseInt(parameter.getCellwidth())/10), y-Integer.parseInt(parameter.getCellheight())/4-i*20,0);
			      	}
		     	}*/
		    	for(int i=0;i<wordList.size();i++)
		    	{
		    		if(i==wordList.size()-1)
			      	{
		    			cb.showTextAligned(PdfContentByte.ALIGN_CENTER, (String)wordList.get(i), x+13+(Integer.parseInt(parameter.getCellwidth())/10), y-Integer.parseInt(parameter.getCellheight())/4-i*iFize-5+valigny,0);
			      	}else if(i==0)
			      	{
			      		cb.showTextAligned(PdfContentByte.ALIGN_CENTER, (String)wordList.get(i), x+13+(Integer.parseInt(parameter.getCellwidth())/10), y-Integer.parseInt(parameter.getCellheight())/4-5,0);
			      	}else
			      	{
			      		cb.showTextAligned(PdfContentByte.ALIGN_CENTER, (String)wordList.get(i), x+13+(Integer.parseInt(parameter.getCellwidth())/10), y-Integer.parseInt(parameter.getCellheight())/4-i*iFize-5,0);
			      	}
		    	}
		     }else
		     {
		     	 cb.showTextAligned(PdfContentByte.ALIGN_CENTER, text, x+13+(Integer.parseInt(parameter.getCellwidth())/10), y-Integer.parseInt(parameter.getCellheight())/4-10,0);
		     }
		     cb.endText();  
		 }
		 else
	...
	public int[] getOrgMapPageSize(ParameterBo parameter,String code,Connection conn,String username) throws GeneralException
	{
		...
		if(this.orgtype!=null&& "vorg".equalsIgnoreCase(this.orgtype))
		{
			...
			else
			{
				...
				}else
				{
					...
					sqlstr.append("(SELECT MAX(grade) +1 AS grade from vorganization where codeitemid like '");
					sqlstr.append(code);
					sqlstr.append("%') aa,(SELECT COUNT(*) as counts FROM vorganization b WHERE b.parentid LIKE '");
					sqlstr.append(code);
					sqlstr.append("%' AND b.codeitemid = b.childid and b.codeitemid like '");
					sqlstr.append(code);
					sqlstr.append("%') bb");
		...
		}else
		{
			...
			else
			{
				...
				{
					sqlstr.append("select aa.grade -bb.grade  as grade,cc.counts + dd.counts as leafagechildscount from ");
					sqlstr.append("(SELECT MAX(grade) + 1 as grade from t#"+username+"organization where codeitemid like '");
					sqlstr.append(code+"%'");
					if(!bShowPos) {
					...
					sqlstr.append(") aa,"); 
					sqlstr.append("(SELECT grade FROM t#"+username+"organization WHERE codeitemid = '");
					sqlstr.append(code+"'");
					if(!bShowPos) {
                        sqlstr.append(" and codesetid<>'@K'");
                    }
					...
					sqlstr.append(") bb,");
					sqlstr.append("(SELECT COUNT(*) as counts FROM "+"t#"+username+"organization b WHERE b.parentid LIKE '");
					sqlstr.append(code+"%'");
					if(!bShowPos) {
                        sqlstr.append(" and codesetid<>'@K'");
                    }
					...
					sqlstr.append(code);
					sqlstr.append("%') cc,");
					sqlstr.append("(SELECT COUNT(*) as counts FROM "+"t#"+username+"organization org WHERE (codeitemid <> childid) AND (org.codeitemid LIKE '");
					sqlstr.append(code+"%')");
					if(!bShowPos) {
                        sqlstr.append(" and codesetid<>'@K'");
                    }
					...
					sqlstr.append(" AND (NOT EXISTS (SELECT * FROM "+"t#"+username+"organization orge WHERE orge.codeItemId = org.childId)) and org.codeitemid like '");
					sqlstr.append(code);
					sqlstr.append("%') dd");
				}else
				{
					...
					sqlstr.append(") aa,(SELECT COUNT(*) as counts FROM "+"t#"+username+"organization b WHERE b.parentid LIKE '");
					sqlstr.append(code+"%'");
					if(!bShowPos) {
                        sqlstr.append(" and codesetid<>'@K'");
                    }
					...
					sqlstr.append(" AND (NOT EXISTS (SELECT * FROM "+"t#"+username+"organization orge WHERE orge.codeItemId = org.childId))) cc ");
				}
			}
		}
		...
		if(!rs.isEmpty()&&rs.size()>0)
		{
			if("true".equalsIgnoreCase(parameter.getGraphaspect()))
			{
				...
				System.out.println(Integer.parseInt(parameter.getCellwidth()));
				System.out.println(Integer.parseInt(parameter.getCellheight()));
				System.out.println(Integer.parseInt(parameter.getCellhspacewidth()));
				System.out.println(parameter.getPagespacewidth());
				System.out.println("++++++++++++");
				System.out.println(orgbean.get("leafagechildscount").toString());
				System.out.println(Integer.parseInt(parameter.getCellheight()));
...
//				pagesize[1]=(1+Integer.parseInt(orgbean.get("grade").toString()))*(Integer.parseInt(parameter.getCellheight())*7/6 + Integer.parseInt(parameter.getCellvspacewidth())*2) + parameter.getPagespaceheight()*2;
			...
			//System.out.println(pagesize[0]+"===="+pagesize[1]);
		    if(pagesize[0]>14400)
		    {
		    	//pagesize[0]=14400;
	...
	public int[] getHistoryOrgMapPageSize(ParameterBo parameter,String code,Connection conn,String username,String catalog_id) throws GeneralException
	{
		...
		if(!rs.isEmpty()&&rs.size()>0)
		{
			...
			{
				LazyDynaBean orgbean=(LazyDynaBean)rs.get(0);	
				String grade=orgbean.get("grade")!=null && orgbean.get("grade").toString().length()>0?orgbean.get("grade").toString():"0";
				if(grade.indexOf(".")!=-1)
				{
					grade=grade.substring(0,grade.indexOf("."));
				}
				pagesize[0]=(1+Integer.parseInt(grade))*(Integer.parseInt(parameter.getCellwidth()) +Integer.parseInt(parameter.getCellheight())/6 + Integer.parseInt(parameter.getCellhspacewidth())*2) + parameter.getPagespacewidth()*2;
				pagesize[1]=(1+Integer.parseInt(orgbean.get("leafagechildscount").toString())) * (Integer.parseInt(parameter.getCellheight()) + Integer.parseInt(parameter.getCellvspacewidth())) + parameter.getPagespaceheight() *2;
			}else
			{
				LazyDynaBean orgbean=(LazyDynaBean)rs.get(0);
				...
				pagesize[0]=(1+Integer.parseInt(orgbean.get("leafagechildscount").toString())) * (Integer.parseInt(parameter.getCellwidth()) + Integer.parseInt(parameter.getCellhspacewidth())) + parameter.getPagespacewidth() *2;
				pagesize[1]=(1+Integer.parseInt(grade))*(Integer.parseInt(parameter.getCellheight())*7/6 + Integer.parseInt(parameter.getCellvspacewidth())*2) + parameter.getPagespaceheight()*2;
			}
			 if(pagesize[0]<180)
			 {
				 pagesize[0]=180;
