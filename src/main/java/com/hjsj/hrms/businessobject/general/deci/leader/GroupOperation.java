...
public class GroupOperation {
	...
	public GroupOperation(Connection conn,UserView userView)
	{
		this.conn=conn;
	 ...
	 * @return
	 */
	public HashMap getLeadberMap(String bz_field,String groupid_setid,String groupid_codesetid,String groupid_value,String display_messn,ArrayList display_list,String code,String kind){
		...
		String[] fieldstr = display_messn.split(",");
		ArrayList fieldlist =new ArrayList();
		ArrayList fieldIdlist =new ArrayList();
		if(display_messn!=null&&display_messn.length()>0){
			for(int i=0;i<fieldstr.length;i++)
			{
				String itemid=fieldstr[i];
				FieldItem fielditem=DataDictionary.getFieldItem(itemid);			
				fielditem.setVisible(true);
				fieldlist.add(fielditem);
				
				fieldIdlist.add(fielditem.getFieldsetid());
			}
	
			for(int r=0;r<nbaselist.size();r++)
			{
				userbase = nbaselist.get(r).toString();
				//加按标记搜索后的结果
				StringBuffer a0100sql = new StringBuffer();
				a0100sql.append("select DISTINCT A.A0100 from ");
				if(!"A01".equalsIgnoreCase(groupid_setid)){
					a0100sql.append(userbase+groupid_setid+" A,");
					a0100sql.append("(select a0100,max(i9999) I9999 from "+userbase+groupid_setid+" group by a0100 ) B");
					if(!groupid_codesetid.equals("")&&!groupid_value.equals("")){
						a0100sql.append(" where "+groupid_codesetid+" = "+groupid_value);
					}
					a0100sql.append(" and A.A0100 = B.A0100 and A.I9999 = B.I9999 ");
				}else{
					a0100sql.append(userbase+groupid_setid+" A");
					if(!groupid_codesetid.equals("")&&!groupid_value.equals("")){
						a0100sql.append(" where "+groupid_codesetid+" = "+groupid_value);
				...
				StringBuffer innerStrsql = new StringBuffer();
				
				strsql.append("select '"+userbase+"' as nbase ,"+userbase+"A01.A0100,"+userbase+"A01.B0110,"+userbase+"A01.E0122,"+userbase+"A01.E01A1,"+userbase+"A01.A0000,");
				
				leftsql.append("");
				int index = 0;
				for(int i =0;i<fieldstr.length;i++){
					String fieldId = (String)fieldIdlist.get(i);
					if(fieldId.equals("A01"))
						strsql.append(userbase+fieldId+"."+fieldstr[i].toUpperCase()+",");
					else{
						strsql.append(userbase+fieldId+index+"."+fieldstr[i].toUpperCase()+",");
						leftsql.append("left join (select A.A0100,"+fieldstr[i].toUpperCase()+" from "+userbase+fieldId+" A,");
						leftsql.append("(select a0100,max(i9999) I9999 from "+userbase+fieldId+" group by a0100 ) B ");
						leftsql.append("where A.a0100 = B.a0100 and A.i9999 = B.i9999 ) "+userbase+fieldId+index);
						leftsql.append(" on "+userbase+"A01.a0100 = "+userbase+fieldId+index+".a0100 ");
						
					}
					index++;
				}
				strsql.setLength(strsql.length()-1);
				fromsql.append(" from "+userbase+"A01 ");
				
				innerStrsql.append("where "+userbase+"A01.a0100 in (select "+userbase+"A01.a0100 from "+userbase+"A01");
				if("2".equals(kind) && code!=null && code.length()>0)
				{
					innerStrsql.append(" where b0110 like '");
					innerStrsql.append(code);
					innerStrsql.append("%'");
				}			   
				else if("1".equals(kind)  && code!=null && code.length()>0)
				{
					innerStrsql.append(" where e0122 like '"); 
					innerStrsql.append(code);
					innerStrsql.append("%'");
				}				
				else if("0".equals(kind) && code!=null && code.length()>0)
				{
					innerStrsql.append(" where e01a1 like '");
					innerStrsql.append(code);
					innerStrsql.append("%'");
				}else if("2".equals(kind))
				{
					innerStrsql.append(" where 1=1");
				}
				else
				{
					innerStrsql.append(" where 1=2");
				}
				
				innerStrsql.append(" and A0100 in (");
				innerStrsql.append(a0100sql.toString());
				innerStrsql.append("))");
				
				union_Sql.append(strsql.toString());
				union_Sql.append(fromsql.toString());
				union_Sql.append(leftsql.toString());
				union_Sql.append(innerStrsql.toString());
				union_Sql.append(" union ");
			}
			union_Sql.setLength(union_Sql.length()-7);
		}
		//System.out.println(union_Sql.toString());
		map.put("select_str",union_Sql.toString());
		...
		if(display_list!=null&&display_list.size()>0)
		{
			...
			for(int r=0;r<nbaselist.size();r++)
			{
				...
				select_str.append(" from "+nbase+groupid_setid);
				...
				select_str.append(nbase+groupid_setid+".a0100=AAA.a0100 ");
				String itemid="";
				String oldItemid="";
				for(int i=0;i<fieldlist.size();i++)
				{
					...
					if(i==0)
					{
						select_str.append(nbase+groupid_setid+".A0100="+itemid+i+".A0100 ");
					}else
					{
				...
				if(!"A01".equalsIgnoreCase(groupid_setid))
				{
					select_str.append(",(select a0100,Max(i9999) as i9999 from "+nbase+groupid_setid+"  Group by a0100) D");
				}
				select_str.append(" where "+nbase+groupid_setid+"."+groupid_codesetid.toUpperCase()+"='"+groupid_value.toUpperCase()+"'");
				String whereCode="";
			    if("2".equals(kind)) {
				...
				if(!"A01".equalsIgnoreCase(groupid_setid))
				{
					select_str.append(" and "+nbase+groupid_setid+".A0100=D.A0100 and "+nbase+groupid_setid+".I9999=D.I9999");
				}			
	...
	public String beanlistHtml(ArrayList display_list,ArrayList fieldlist,ArrayList beanlist,int curpage,int pagesize,int sumsize,String code,String kind)
	{
		...
		for(int i=0;i<beanlist.size();i++)
		{
			if(i>=(curpage-1)*pagesize&&(i<(curpage)*pagesize))
			{
				...
				for(int r=0;r<fieldlist.size();r++)
				{
						...
						//html.append(" <a href='/workbench/browse/showselfinfo.do?b_search=link&userbase="+bean.get("nbase")+"&a0100="+bean.get("a0100")+"&flag=notself&returnvalue=1' target='_blank'>&nbsp;"+value+"</a>");
                    {
						...
						if(viewvalue.length()==0) {
							...
							html.append(" <a href='javascript:void(0);' onclick=openwin('/workbench/browse/showposinfo.do?b_browse=link&npage=1&tabid="+card_tabid+"&inforkind=4&a0100="+value+"');>&nbsp;"+viewvalue+"&nbsp;</a>");
						}else {
							html.append("&nbsp;"+viewvalue+"&nbsp;");
						}
				...
				html.append("<td align='center' class='RecordRow' nowrap><a href=\"###\" onclick='openwin1(\"/general/inform/synthesisbrowse/mycard.do?b_mysearch=link`userbase="+bean.get("nbase")+"`a0100="+PubFunc.encrypt(bean.get("a0100").toString())+"`multi_cards=-1`inforkind=1`npage=1`userpriv=noinfo`flick=1\");'>");
	...
	public int getLeadberConnt(String bz_field,String candid_setid,String candid_codesetid,String candid_value,String code,String kind)
	{
		int count=0;
		StringBuffer sql=new StringBuffer();
		ArrayList nbaselist=new ArrayList();
	 ...
	 * @return
	 */
	public ArrayList getLeadberEmpMess(String bz_field,String candid_setid,String candid_codesetid,String candid_value,String code,String kind)
	{
		StringBuffer sql=new StringBuffer();
		ArrayList nbaselist=new ArrayList();
		if(bz_field.length()<=0) {
		...
		try
		{
			...
			if(com.hrms.hjsj.utils.Sql_switcher.searchDbServer()==1) {
                rs=dao.search(sql.toString()+" order by AAA.B0110,AAA.E0122,AAA.A0000,a0101");
            } else {
                rs=dao.search(sql.toString()+" order by B0110,E0122,A0000,a0101");
            }
			while(rs.next())
			{
				DynaBean bean = new LazyDynaBean();	
			...
			for(int i=0;i<beanlist.size();i++)
			{
				if(i>=(curpage-1)*pagesize&&(i<(curpage)*pagesize))
				{
				    ...
				    if(!"".equals(filename)){
				        	...
				        	photourl.append(PubFunc.encrypt(filename));//liuy 领导桌面安全改造
				    }else{
					...
					//html.append("<a href='/workbench/browse/showselfinfo.do?b_search=link&userbase="+bean.get("nbase")+"&a0100="+a0100+"&flag=notself&returnvalue='1' target='_blank'>");
	...
	/** 
	 * 过滤得到要查询的所有usr后缀
     * @param ArrayList 从itemid得到的所有usr后缀（有重复的）
     * @return ArrayList
     */
    private ArrayList getUsrSuffix(ArrayList fieldlist){
    	ArrayList list = new ArrayList();
    	Iterator it = fieldlist.iterator();
    	String fieldSetId1="",fieldSetId2 = "";
    	while(it.hasNext()){
    		fieldSetId1 =it.next().toString();
    		if(list.size()==0){
    			list.add(fieldSetId1);
    			fieldSetId2 =fieldSetId1;
    		}
    		if(!fieldSetId1.equals(fieldSetId2)){
    			list.add(fieldSetId1);
    			fieldSetId2 =fieldSetId1;
    		}
    	}
