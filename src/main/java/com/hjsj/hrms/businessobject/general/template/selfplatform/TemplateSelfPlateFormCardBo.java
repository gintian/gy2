 ...
 * <p>@version: 7.0</p>
 */
public class TemplateSelfPlateFormCardBo {
	...
	private String curPageId;//当前pageId
	private HashMap nodePrivMap;//存放nodeprivate 节点。
	/**
	 * @param conn
	 * @param tabid
	 * @param pageid
	 * @return
	 */
	public TemplateSelfPlateFormCardBo(Connection conn,UserView userview,int tabid ) {
		this.conn = conn;
	...
	public TemplateSelfPlateFormCardBo(Connection conn,UserView userview,TemplateParam paramBo) {
		this.conn = conn;
		this.userView = userview;
	...
	public ArrayList getAllCell() {
		if (cardCellList==null) {
            cardCellList=  utilBo.getPageCell(this.tabId,StringUtils.isEmpty(this.curPageId)?-1:Integer.parseInt(this.curPageId));
        }
		return cardCellList;
	}
	 ...
	 * @return ArrayList
	 */
	public ArrayList getFieldValueList(String ins_id,String moduleId) throws Exception {
		...
		try {
			...
			if(Sql_switcher.dbflag==Constant.DAMENG) {
				staitic_="static_o";
			}
			try {
				ArrayList cellList = getAllCell();
				for (int i = 0; i < cellList.size(); i++) {
					...
					{
						AttachmentBo attachmentBo = new AttachmentBo(userView, this.conn, this.tabId+"");//获取附件设置的大小，传给前台控制。
			    		attachmentBo.initParam(false);
			    		String attachmenttype=setBo.getAttachmentType(); //公共附件
			    		String rwPriv="2";//附件默认可编辑  用不到。。。
			    		String gridno=setBo.getGridno()+"";
			    		String pageid=setBo.getPageId()+"";
			    		...
			    		ArrayList list=attachmentBo.getAttachmentList(ins_id,this.a0100,this.basePre,create_user,attachmenttype,rwPriv,pageid_gridno);
			    		...
			    		for(Iterator tt=list.iterator();tt.hasNext();)
			    		{
			    			...
			    			valueMap2.put("fileId",filepath); //主键ID
			    			valueMap2.put("filepath", filepath);
			    			valueMap2.put("name",enty!=null?enty.getName():""); //上传的原文件名
			    			subData.add(valueMap2);
			    		}
			    		Gson gson = new Gson();
	                    fldValue = gson.toJson(subData);
	                    disValue = fldValue;
	                    bean = getFieldValueBean(setBo.getPageId()+"",setBo.getUniqueId() + "",
	                            setBo.getTableFieldName(), fldValue, disValue);
						...
						if(StringUtils.isNotBlank(sub_domain)){
							...
							if(childlist!=null&&childlist.size()>0)
							{
								...
								if(StringUtils.isNotEmpty((String)element.getAttributeValue("file_type"))){
									file_type=(String)element.getAttributeValue("file_type");
								}
							}
						}
						bean.set("filetype", file_type);
	                    fieldValueList.add(bean);
					}
					else {
	...
	private HashMap getFieldPrivMap(ArrayList allCellList) {
		HashMap filedPrivMap=getFieldPrivMap(allCellList, this.task_id);
		return filedPrivMap;
	}
    ...
    */
    public HashMap getFieldPrivMap(ArrayList allCellList, String taskId) {
            ...
            if (this.paramBo.getSp_mode()==0&&!"3".equals(this.paramBo.getReturnFlag())&&this.paramBo.isBsp_flag()) {// 获取节点的读写权限 我的申请浏览也不需要判断节点权限
                nodePrivMap = utilBo.getFieldPrivFillable(taskId.split(",")[0],tabId);
            }else {
            	nodePrivMap = new HashMap();
			}
            for (int i = 0; i < allCellList.size(); i++) {
                ...
                if (setBo.isABKItem()) {
                    ...
                    if (nodePrivMap!=null&&nodePrivMap.get(tableFieldName_lin) != null) {// 如果是无权限,跳出
	...
	private LazyDynaBean getFieldBean(TemplateSet setBo, HashMap privMap) {
		...
		if (setBo.isSubflag()) {// 添加子集指标属性
			...
			SubSetDomain subSetDomain = subsetBo.getSubDomain();
			bean.set("subSetDomain", subSetDomain);
			bean.set("subXml", subsetBo.getSubFieldsPropertys("0", "1"));
		}
        ...
        if (nodePrivMap!=null&&!nodePrivMap.isEmpty()&&StringUtils.isNotBlank(fieldName)&&nodePrivMap.get(fieldName.toLowerCase()) != null) {
            String rwPriv = (String)nodePrivMap.get(fieldName.toLowerCase());
            if("3".equalsIgnoreCase(rwPriv)) {
                isYneed=true;
            }
        }
        bean.set("yneed",isYneed);
