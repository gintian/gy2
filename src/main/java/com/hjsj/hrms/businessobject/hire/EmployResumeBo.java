				...
				if("A".equals(itemtype))
				{
					if("0".equals(codesetid))
					{
						if(!"Othaz7".equals(newTable)){
							//if(!resumeState.equals("-1")&&!resumeState.equals("-3")||!newTable.equalsIgnoreCase("z03")){
	...
	public String getSql_from(String dbname,ArrayList fielditemList,HashMap resumeFieldMap,String resumeState,ArrayList posConditionList,String personType,String z0301,String codeid,String upValue,String vague,UserView userView,String queryType,String hireObjectId,String applyStartDate,String applyEndDate)
	{
		...
		try
		{
			...
			if("0".equals(personType))//来自应聘库的查询
			{
					...
					if("-3".equals(resumeState))//-3代表着全部
					{
						...
						if(!("".equals(applyStartDate.trim())&&"".equals(applyEndDate.trim()))){
								if(Sql_switcher.searchDbServer() == Constant.MSSQL){	
									sql_str = "(select * from "+dbname+"a01 where A0100 in (select  A0100 from zp_pos_tache where  apply_date between '"+applyStartDate+" 00:00:00' and '"+applyEndDate+" 23:59:59'  ) )   ";	
							}else{
								sql_str = "(select * from "+dbname+"a01 where A0100 in (select  A0100 from zp_pos_tache where  apply_date between to_date('"+applyStartDate+" 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date('"+applyEndDate+" 23:59:59','yyyy-mm-dd hh24:mi:ss') ) )   ";
							}
							from.append(sql_str+" ");
						}
				...
				//from.append(" from zp_pos_tache zpt1 left join "+dbname+"a01 on zpt1.a0100="+dbname+"a01.a0100 left join z03 on zpt1.zp_pos_id=z03.z0301");
			}
			else if("4".equals(personType))//1代表着来自人才库  4：我的收藏夹
			{		
				  from.append(" from "+dbname+"a01");	
				//from.append(" from zp_pos_tache zpt  left join  (select * from codeitem where codesetid='36') code0 on zpt.resume_flag=code0.codeitemid left join "+dbname+"a01 on zpt.a0100="+dbname+"a01.a0100  left join z03 on zpt.zp_pos_id=z03.z0301  ");
				//from.append("from "+dbname+"a01 left join (select z03.*,zpt.* from zp_pos_tache zpt left join z03 on z03.z0301 = zpt.zp_pos_id) zpt on zpt.a0100 = "+dbname+"a01.a0100");
				...
				if(!"-1".equals(resumeState)&&!"-2".equals(resumeState)&&!"-3".equals(resumeState))
				{
		    		if(userView.isSuper_admin()|| "1".equals(userView.getGroupId()))
		    		{
	    	    		...
	    	    		if(codeid.trim().length()>=0)
	    	    		{
	    		    		...
	    		    		if(codeid.indexOf("`")==-1)
	    		    		{
	    		    		//	from.append(" and z03.z0311 like '"+codeid.substring(2)+"%'");
	    		    			 //String _str=Sql_switcher.isnull("z03.z0336","''");
	    		    			 //from.append(" and ( ( z03.z0311 like '"+codeid.substring(2)+"%' and  "+_str+"<>'01' ) or ( z03.z0321 like '"+codeid.substring(2)+"%' and  "+_str+"='01' ) or ( z03.z0325 like '"+codeid.substring(2)+"%' and  "+_str+"='01' ) ) ");
	    		    		}
	    		    		else
	    		    		{
	    		    			String _str=Sql_switcher.isnull("z03.z0336","''");
		    		    ...
		    		    //		from.append(" and ( "+tempSql.substring(3)+" ) ");
	    				...
	    				//from.append(" and z03.z0319='04' ");
					
					
					
					//from.append(" and ( Z0311 like '"+codeid+"%'  or ( ( z0303 is not null and z0303 like '"+codeid+"%') or ( z0305 is not null and z0305 like '"+codeid+"%')    ) )  ");
					//from.append(" and z03.z0311 like '"+codeid+"%'");
	    		    	}
			    	}
					...
					if("-2".equals(resumeState))//已选职位，（没选我管理的职位的简历）
					{
						if(userView.isSuper_admin()|| "1".equals(userView.getGroupId()))
			    		{
		    	    		...
		    	    		if(codeid.trim().length()>=0)
		    	    		{
		    		    		...
		    		    		if(codeid.indexOf("`")==-1)
		    		    		{
		    		    		//	from.append(" and z03.z0311 not like '"+codeid.substring(2)+"%'");
		    		    			String _str=Sql_switcher.isnull("z03.z0336","''");
			    		    	...
			    		    	//	from.append(" and ( "+tempSql.substring(4)+" ) ");
					...
					else
					{
						if("".equals(active_field)&& "-3".equals(resumeState))//这个应该是全部(raido那个按钮个人管理范围之内的)
						{
							...
							if(userView.isSuper_admin()|| "1".equals(userView.getGroupId())|| "".equals(codeid))
				    		{
			    	    		...
			    	    		if(codeid.trim().length()>=0)
			    	    		{
			    	    		    /**当查询全部的时候应该根据当前用的管理范围进行控制,无论codeid中是否包含`都可以使用else里面的方法**/
//			    		    		if(codeid.indexOf("`")==-1)
//			    		    		{
//			    		    		//	from.append(" and z03.z0311 like '"+codeid.substring(2)+"%'");
//			    		    			 //String _str=Sql_switcher.isnull("z03.z0336","''");
//			    		    			 //from.append("  ( ( z03.z0311 like '"+codeid.substring(2)+"%' and  "+_str+"<>'01' ) or ( z03.z0321 like '"+codeid.substring(2)+"%' and  "+_str+"='01' ) or ( z03.z0325 like '"+codeid.substring(2)+"%' and  "+_str+"='01' ) ) ");
//			    		    		}
//			    		    		else
//			    		    		{
			    		    			String _str=Sql_switcher.isnull("z03.z0336","''");
...
//				    	    		}
	...
	public ArrayList getZbList(String setid)
	{
		...
		try
		{
			/*RowSet frowset=null;
			ContentDAO dao = new ContentDAO(this.conn);
			frowset=dao.search("select itemid,itemdesc  from t_hr_busiField  where useflag <> 0 and fieldsetid ='"+setid+"'");
			CommonData top = new CommonData();
			top = new CommonData("","");
			zb_list.add(top);
			zb_list.add(new CommonData("(~系统时间~)","(~系统时间~)"));
			while(frowset.next()){
				String itemid = frowset.getString("itemid");
				String itemdesc = frowset.getString("itemdesc");
                if(!itemid.equalsIgnoreCase("A0100") && !itemid.equalsIgnoreCase("Z0503") && !itemid.equalsIgnoreCase("Z0505") && !itemid.equalsIgnoreCase("Z0507") && !itemid.equalsIgnoreCase("Z0509"))
                             continue;	
				if(itemid.equalsIgnoreCase("a0100"))
						itemdesc="人员姓名";
	...
	public ArrayList getZbjList()
	{
		...
		try
		{
			...
			RowSet frowset=null;
			ContentDAO dao = new ContentDAO(this.conn);
			frowset=dao.search("select fieldsetid , fieldsetdesc from t_hr_busitable where useflag <> 0 and fieldsetid = 'Z05'");
			while(frowset.next())
			{
				CommonData data1=new CommonData(frowset.getString("fieldsetid"),frowset.getString("fieldsetdesc"));
				list.add(data1);
			}*/
	...
	public ArrayList getMailTempList(UserView userView) 
	{
		...
		try
		{
			...
			{
				if(userView.getUnit_id().length()>2)
				{
					
					if(userView.getUnit_id().indexOf("`")==-1)
						sql.append(" and b0110 like '"+userView.getUnit_id().substring(2)+"%'");
					else
					{
						StringBuffer tempSql=new StringBuffer("");
...
//					if(mymap.get(a0100+"$")==null){
//						
//						mymap.put(a0100+"$",k);
//						k++;
//					}
//					
//			    	if(zjmap.get(a0100+"#")!=null)
//					{
//						String x = (String)zjmap.get(a0100+"#");
//						int xx=Integer.parseInt(x);
//						xx++;
//						zjmap.put(a0100+"#", String.valueOf(xx));
//						numberMap.put(mymap.get(a0100+"$"), String.valueOf(xx));
//					}
//					else		
//					{   
//						int xx=0;
...
//	}
	...
	public ArrayList getitemIDList(){
		...
		try{
			...
			data=new CommonData("","请选择...");
			list.add(data);
			while(frowset.next()){
				data=new CommonData(frowset.getString("itemid"),frowset.getString("itemdesc"));
				list.add(data);
			}
		}catch(Exception e)
		{
	...
	public ArrayList getModeList(){
		...
		list.add(new CommonData("1","不导入"));
		list.add(new CommonData("2","替换"));
		list.add(new CommonData("3","追加"));
		return list;
...
//		<input type="checkbox" name="itemlistform.select[14]" value="true">
