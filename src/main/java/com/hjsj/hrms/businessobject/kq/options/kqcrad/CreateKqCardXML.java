...
public class CreateKqCardXML {
     ...
     * @param flag
     */
    public CreateKqCardXML(String params,String action,String target,String flag,String dbtype) {
     ...
     * @param flag
     */
    public CreateKqCardXML(String params,String action,String target,String flag,String dbtype,boolean isPost,String viewPost) {
        this.params=PubFunc.keyWord_reback(params);
        this.target=target;
        this.action=action;
        this.flag=flag;
    ...
    public CreateKqCardXML(String params,String action,String target,String flag) {
        this.params=PubFunc.keyWord_reback(params);
        this.target=target;
        this.action=action;
        this.flag=flag;
     ...
     * @param conn
     */
    private void getEmploys_Href(UserView userview,String parentid,Element root,Connection conn,String s_where,String kq_card)
    {
      String strsql=getPrivSql(userview, parentid,conn,s_where,kq_card);      
      if("".equals(strsql)) {
          return;
      }
    ...
    public String outOrgEmpTree(UserView userview,String parentid,String kq_type,String kq_card)throws GeneralException
    {
        ...
        try
        {
        	...
        	if(parentid.toUpperCase().indexOf("GP")==-1)
        	{
                ...
                while (rset.next())
                {
                   ...
                   if(!(this.action==null|| "".equals(this.action)))
                   {
      		          //theaction=this.action+"?b_search=link&a_code="+rset.getString("codesetid")+rset.getString("codeitemid");
                	   //theaction="javascript:org_item('"+rset.getString("codesetid")+rset.getString("codeitemid")+"')";
                   }
                   if(theaction==null|| "".equals(theaction)) {
                       child.setAttribute("href", "javascript:void(0)");
                   } else {
                   ...
                   child.setAttribute("target", this.target);
                   //"/common/sendcard/loadtree?
          		   String url="target="+this.target+"&flag="+this.flag+"&dbtype="+this.dbtype+"&action="+this.action;
          		   url=url+"&kq_type="+kq_type+"&kq_card="+kq_card+"&params=parentId<>codeitemid and parentid%3D'" + rset.getString("codeitemid")+"'";
          		   url=url+"&id="+rset.getString("codesetid")+rset.getString("codeitemid");
          		   if (!isPost) {
         			   url = url + "&viewPost=" + this.viewPost;
         		   }
          		   
          		   url = "/common/sendcard/loadtree?encryptParam=" + PubFunc.encryption(url);
                ...
                String s_where="";
  		        ...
  		        {
  			       s_where=" and "+kq_type+"='02'";
  		        }  	*/	        
                if("1".equals(flag)) {
                    getEmploys_Href(userview,parentid,root,conn,s_where,kq_card);
                }
            }
    ...
    private String getPrivSql(UserView userview, String parentid,Connection conn,String s_where,String kq_card) {
		StringBuffer strSql=new StringBuffer();	
		...
		try
		  {
			...
			{        	
			    ...
			    if(kq_card!=null&&kq_card.length()>0)
  		        {
			    	strSql.append(" and ("+kq_card+" is null or "+kq_card+"='')");
  		        }
			    strSql.append(" UNION ");         		
			}
