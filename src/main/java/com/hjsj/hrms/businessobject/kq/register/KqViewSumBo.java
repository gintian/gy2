 ...
 * */
	 ...
	 * **/
	 public ArrayList getKqReportHtml(String code,String kind,String coursedate,String curpagestr,ReportParseVo parsevo,UserView userView,HashMap formHM)throws GeneralException
	 {
		    ...
		    allrows =getAllRecordNum(code,kind,coursedate,kq_dbase_list);
		    int curpage = 1;
...
//		    html.append(" style='position:absolute;top: "+parsevo.getTop()+"");		
		    html.append(" style='top: "+parsevo.getTop()+"");		    
...
//			  body高度
	...
	public String getBodyHeadHtml(HashMap hashmap)throws GeneralException
    {
	    ...
	    onefield= executeOneTable(1, 7, parsevo.getHead_fn(), sytle, parsevo.getBody_fz()+ "", "部门", getStyleName("10"),itemleng);
        bodyheadhtml.append(executeTable(1,7,parsevo.getHead_fn(),sytle,body_height+"",onefield,getStyleName("2"))); 
        onefield=executeOneTable(1, 7, parsevo.getHead_fn(), sytle, parsevo.getBody_fz()+ "", "岗位", getStyleName("10"),itemleng);
        bodyheadhtml.append(executeTable(1,7,parsevo.getHead_fn(),sytle,body_height+"",onefield,getStyleName("2"))); 
        onefield= executeOneTable(1, 7, parsevo.getHead_fn(), sytle, parsevo.getBody_fz()+ "", "工号", getStyleName("10"),itemleng);
        bodyheadhtml.append(executeTable(1,7,parsevo.getHead_fn(),sytle,body_height+"",onefield,getStyleName("2"))); 
    ...
    public String getBodyHtml(String code,String kind,String kq_duration,int curpage,int pagesize,ArrayList a0100list,HashMap hashmap)throws GeneralException
    {
    	...
    	for(int i=0;i<a0100list.size();i++)
    	{
            ...
            String one_date = getOneA0100Data(code, kind, a0100, kq_duration, body_hieght); // 头下面每个人的1到30号的内容
...
//     	   String t_str="&nbsp;&nbsp;时间: "+PubFunc.getStringDate("HH:mm:ss");
    ...
    public ArrayList getA0100List(String code,String kind,String kq_duration,int curpage,int pagesize,ArrayList kq_dbase_list,ArrayList keylist)throws GeneralException
    {
        ...
        try{
            ...
            if(curpage == allrows/pagesize +1 && allrows > pagesize) {
                endrows = allrows % pagesize;
            }
        	...
        	if (Sql_switcher.searchDbServer() == 2) {
        		sql.append("select * from (select b.a0100,b.a0101,b.nbase,b.e0122,b.e01a1,rownum m from (");
	        	...
	        	sql.append("select * from (select top "+endrows+" * from (select top ");
	        	sql.append(pagesize * curpage);
	        	sql.append("* from (");
            ...
            while(rowSet.next()){
               ...
               a0100[3] = rowSet.getString("e0122"); // 考勤表加入部门
               a0100[4] = rowSet.getString("e01a1"); // 考勤表加入职位
               a0100[5] = "";
               if (gnoField != null && gnoField.length() > 0){
	               vo = new RecordVo(a0100[2] + "A01");
				   vo.setString("a0100", a0100[0]);
				   
				   if (dao.isExistRecordVo(vo)) {
					   if (vo != null) {
						   String gnValue= vo.getString(gnoField.toLowerCase());
						   a0100[5] = gnValue; 
					   }
					}
               }
    ...
    private String select_Q05_a0100(String code, String kind,
		...
		if (sortItem != null && sortItem.length() > 0
			...
			for (int i = 0; i < strs.length; i++) {
				...
				if (!",a0100,a0101,q03z0,nbase,dbid,a0000,e0122,e01a1,".contains(","
						+ str[0].toLowerCase() + ",")) {
					coloum.append(str[0]);
					coloum.append(",");
				}
		...
		for (int i = 0; i < kq_dbase_list.size(); i++) {
			...
			sqlstr.append("select distinct a0100,a0101,q03z0,nbase,e0122,e01a1,"
	                + coloum.toString() + "dbid,a0000  from ").append(this.getCurTab());
    ...
    public String getOneA0100Data(String code,String kind,String a0100[],String kq_duration,int body_hieght)throws GeneralException
    {
        ...
        try{
              ...
              one_date.append(executeTable(1, 7, parsevo.getHead_fn(), sytle, body_hieght + "", e0122, getStyleName("6")));
              // 职位
              String e01a = AdminCode.getCode("@K", a0100[4]) != null ? AdminCode.getCode("@K", a0100[4]).getCodename() : "";
              one_date.append(executeTable(1, 7, parsevo.getHead_fn(), sytle, body_hieght + "", e01a, getStyleName("6")));
              // 工号
              KqParameter para = new KqParameter(this.userView, "", this.conn);
              HashMap map = para.getKqParamterMap();
              String gnoField = (String) map.get("g_no");//工号指标
              ...
              if (gnoField != null && gnoField.length() > 0) {
                  one_date.append(executeTable(1, 7, parsevo.getHead_fn(), sytle, body_hieght + "", a0100[5], getStyleName("6")));
              } else {
                  one_date.append(executeTable(1, 7, parsevo.getHead_fn(), sytle, body_hieght + "", "", getStyleName("6")));
              }
            
              String sql_one_a0100 = select_Q05_one_emp(a0100[0],a0100[2],kq_duration,code,kind,column.toString());
    ...
    public String select_Q05_one_emp(String a0100,String nbase, String kq_duration,String code,String kind,String column){
   	   StringBuffer sqlstr= new StringBuffer();
...
//  得到在考勤范围内的部门员工编号，并添加到list中
    ...
    { 
    	String whereA0100 = KqReportInit.count_kq_a0100_q05(this.getCurTab(), code,kind,kq_duration,kq_dbase_list,this.userView);
    	ContentDAO dao = new ContentDAO(this.conn);
        String count="";  
        RowSet rowSet=null;
        try{
	...
	private String getTurnPageCode(int curPage,String turnpage_sytle,int sum_page,String coursedate)
	{
		...
		code.append("<select name='curpage' id='curpage' size='1' onchange='javascript:change()'>");
	...
	public ArrayList getDateList(Connection conn,String coursedate)throws GeneralException
	  {
	      ...
	      String kq_dd;
	  	  ...
	  	  {
	  	      //zxj 20151231 需要取当前考勤人员库范围内人数据
	  	      KqUtilsClass kqUtilsClass = new KqUtilsClass(this.conn, this.userView);
              ArrayList kqNbases = kqUtilsClass.getKqPreList();
              String nbaseWhr = "nbase IN(";
              for (int i=0; i<kqNbases.size(); i++) {
                  nbaseWhr = nbaseWhr + "'" + (String)kqNbases.get(i) + "'";
                  if (i<kqNbases.size()-1) {
                      nbaseWhr = nbaseWhr + ",";
                  }
              }
              nbaseWhr = nbaseWhr + ")";
            
	  		  String sql="SELECT scope FROM " + this.getCurTab() + " where q03z0='"+coursedate+"'";
	  		  sql = sql + " and " + nbaseWhr;
		      ContentDAO dao = new ContentDAO(conn);
				try {
					rowSet = dao.search(sql.toString());
					String scope="";
