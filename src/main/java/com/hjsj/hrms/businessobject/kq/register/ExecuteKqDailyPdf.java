...
//    private final int r_add_height=16;  原来
	...
	public String executePdf(String code,String kind,String coursedate, ReportParseVo parsevo,UserView userView,HashMap formHM)throws GeneralException
	{
    	...
    	ArrayList kqq03list=kqView.getKqBookItemList();  //本月出缺勤情况统计小计 头内容
    	// 是否包含身份证，1为不包含，0为包含
    	String pu="1";
	    ...
	    // 每页中记录条数
	    int pagesize=0;
	    if(this.kqtablejudge && ("#tjxm[1]".equalsIgnoreCase(parsevo.getBody_dept()) || this.noSelected))
	    {
	    	if("0".equals(pu))
	    	{
        ...
        //zxj 10150730 从227行调到这里，214、217行getAllRecordNum需要用到这个条件
...
//	    ArrayList a0100list=kqViewDailyBo.getA0100List(code,kind,datelist,1,recordNum,kq_dbase_list,keylist);//所有人员信息
	    ...
	    kqViewDailyBo.setSortItem(sortItem);
	    if(!"all".equals(dbty)&&dbty.length()>0)
		{
		...
		String url = this.userView.getUserName()+ "_" + context.trim() + ".pdf";
		...
		double ave_data_width = 0;
		double ave_sp_width = 0;
		...
		if (this.kqtablejudge && ("#tjxm[1]".equalsIgnoreCase(parsevo.getBody_tjxm()) || this.noSelected)) {
			// 最多输出考勤统计项目14列，宽度限制  --郑文龙 最大 49列
			int kqq03 = kqq03list.size();
			if(kqq03 + m > 18) {
				kqq03 = 18 - m;
			}
	    	maxColumn = getColnum(datelist, kqq03);
	    	if ("#pos[1]".equalsIgnoreCase(parsevo.getBody_pos()) || this.noSelected) {
		    	if("1".equals(para.getKq_orgView_post())){
		    		maxColumn--;
		    	}
			...
			if(maxColumn>0)
			{
				...
				ave_data_width=ave_width-0.005;
				double sp_sum_width=1-ave_data_width*(maxColumn - m);
				
//				double ave_sp_width=sp_sum_width/17;
				
				// 使报表的序号，姓名，工号三列变宽点
				ave_sp_width=sp_sum_width/m;
				
				
			}
		} else {
			maxColumn = getColnum(datelist, 0);
		    if("1".equals(para.getKq_orgView_post())){
		    	maxColumn--;
		    }
			widths = new float[maxColumn];;
		...
		widths[col++]=Float.parseFloat(ave_sp_width+"");
		if ("#dept[1]".equalsIgnoreCase(parsevo.getBody_dept()) || this.noSelected) {
			widths[col++]=Float.parseFloat(ave_sp_width+"");
		}
		if ("#pos[1]".equalsIgnoreCase(parsevo.getBody_pos()) || this.noSelected) {
		...
		{
			widths[col++]=Float.parseFloat(ave_data_width+"");
		}
		...
		try
		{
		  ...
		  for(int curpage = 1;curpage<=sum_page;curpage++)
		  {
			  ...
			  getTableBodyHead(document,writer,table,datelist,kqq03list,maxColumn);  
			  start_record=getTableBody(code,kind,document,writer,table, datelist,a0100list,start_record,end_record,item_list,coursedate,kqq03list,maxColumn);
	...
	/**
	 * 计算总列数
	 * @param dateList ArrayList 时间列
	 * @param kqq03 int 考勤统计项目数
	 * @return
	 */
	private int getColnum(ArrayList dateList, int kqq03) {
		int maxColnum = 2;
		...
		if ("#kqfu[1]".equalsIgnoreCase(parsevo.getBody_kqfu()) || this.noSelected) {
			maxColnum += dateList.size();
		}
		
		// 是否显示统计项目
		if ("#tjxm[1]".equalsIgnoreCase(parsevo.getBody_tjxm()) || this.noSelected) {
			maxColnum += kqq03;
		}
		
		
//		if (maxColnum > dateList.size() + 18) {
//			maxColnum = dateList.size() + 18;
//		}
		
		realClonum = maxColnum;
...
//				paragraph=new Paragraph("      "+kq_year+" 年 第"+kq_duration+" 期间",font); //原来
	 ...
	 * 
	 * */
	public int  getTableBody(String code,String kind,Document document,PdfWriter writer,PdfPTable table,ArrayList datelist,ArrayList a0100list,int start_record,int end_record,ArrayList item_list,String coursedate,ArrayList kqq03list,int maxColnum)throws GeneralException
	{
        ...
        for(int i=start_record;i<=a0100list.size()&&i<=end_record;i++)
    	{
			...
			row_h=getOneA0100Data(document,writer,table,code,kind,a0100,datelist,body_h,item_list,font,i,coursedate,kqq03list,maxColnum);
...
//				end_record=end_record-1;
     ...
     * */
    public int getOneA0100Data(Document document,PdfWriter writer,PdfPTable table,String code,String kind,String a0100[],ArrayList datelist,float body_h,ArrayList item_list,Font font,int num,String coursedate,ArrayList kqq03list,int maxColnum)throws GeneralException
    {
		...
		col ++;
		if (this.realClonum < 7) {
			if (col == realClonum) {
				cell.setColspan(maxColnum - (maxColnum/realClonum) * (col-1));
			} else {
				cell.setColspan(maxColnum/realClonum);
			}
		} else {
			cell.setColspan(1);
		}
        	 ...
        	 for(int i=0;i<=fielditemlist.size();i++)
          	 {
        		if(i<fielditemlist.size())
        		{
        			FieldItem fielditem=(FieldItem)fielditemlist.get(i);
             	    ...
             	    if(!"N".equals(fielditem.getItemtype())&&!"q03z0".equals(fielditem.getItemid())
              	    		...
              	    		&&!"modtime".equalsIgnoreCase(fielditem.getItemid()) && !"modusername".equalsIgnoreCase(fielditem.getItemid()))
            	    {
             	    	...
             	    	if (!"D".equalsIgnoreCase(fielditem.getItemtype())) {
            	    		...
            	    		java.sql.Date dateValue = rowSet.getDate(fielditem.getItemid());
            	    		//由于只用来判断有值无值即可，所以有值则默认为“1”
            	    		if (dateValue != null) {
                                sr = "1";
                            }
            	    	}
...
//       		    }  
           ...
           if(this.kqtablejudge && ("#tjxm[1]".equalsIgnoreCase(parsevo.getBody_tjxm()) || this.noSelected))
           {
        	   getOneA0100Value(code,kind,a0100,coursedate,kqq03list,body_h,table,font,col,maxColnum);
           }
        }catch(Exception e){
    ...
    public void getOneA0100Value(String code,String kind,String a0100[],String coursedate,ArrayList kqq03list,float body_h,PdfPTable table,Font font, int col, int maxColnum)
    {
//    	ArrayList fielditemlist = DataDictionary.getFieldList("q03",
//				Constant.USED_FIELD_SET);
...
//       	    		columnlist.add(fielditem.getItemid());
       	    		lsit.add(String.valueOf(fielditem.getDecimalwidth()));
       	    		columnlist.add(lsit);
       	    	}
    		}
    		else {
...
//         		String itd=(String)columnlist.get(t);
         		...
         		for(int p=0;p<listd.size();p++)
         		{
         				...
         				itemdecimal = (String) listd.get(3);
         				break;
         		}
         		FieldItem item = (FieldItem) DataDictionary.getFieldItem(itd, "Q03");
                if("D".equalsIgnoreCase(item.getItemtype())) {
         		...
         		sql_on_a0100.append(" where Q03Z0='"+coursedate+"' and ");
...
//            				table.addCell(itemid);
        					...
        					{
        						int num = (Integer.parseInt(itemdecimal));
        						if(itemid.indexOf(".")!=-1) {
								for (int k = 0; k < num; k++) {
									itemid += "0";
								}
	...
	public void getTableBodyHead(Document document,PdfWriter writer,PdfPTable table,ArrayList datelist,ArrayList kqq03list,int maxColnum)
	{
		Font font=getFont(parsevo.getBody_fn(),parsevo.getBody_fb(),parsevo.getBody_fi(),parsevo.getBody_fu(),parsevo.getBody_fz(),"");  //生成字体样式
