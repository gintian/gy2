...
public class ExecuteKqSumPdf {
	 private Connection conn;
		...
		public String executePdf(String code,String kind,String coursedate, ReportParseVo parsevo,UserView userView,HashMap formHM)throws GeneralException
		{
			...
			String url = userView.getUserName()+ "_" + context.trim() + ".pdf";
			...
			try
			{
				fos = new FileOutputStream(System.getProperty("java.io.tmpdir")+System.getProperty("file.separator")+url);
			...
			int maxColumn=columnlist.size()+5;
			...
			if(maxColumn>0)
			{
				...
				{
					widths[i+3]=Float.parseFloat(ave_data_width+"");
				    r=i;
 				}
				widths[r+4]=Float.parseFloat(ave_sp_width+"");
			}
	    ...
	    public void getOneA0100Data(Document document,PdfWriter writer,PdfPTable table,String code,String kind,String a0100[],String coursedate,float body_h,HashMap hashmap,Font font,int num)throws GeneralException
	    {
	    	...
	    	String sql_one_a0100=KqReportInit.select_Q05_one_emp(this.curTab, a0100[0],a0100[2],coursedate,code,kind,this.columns.toString());
			...
			String e0122UM = AdminCode.getCode("UM", a0100[3]) != null ? AdminCode.getCode("UM", a0100[3]).getCodename() : "";
			paragraph=new Paragraph(e0122UM,font_1);
            cell = new PdfPCell(paragraph);
            ltrb= getLtrb("7");
            cell=excecute(cell,7,ltrb);
            cell.setFixedHeight(body_h);               //设置列最大高度
            ...
            String e01aK = AdminCode.getCode("@K", a0100[4]) != null ? AdminCode.getCode("@K", a0100[4]).getCodename() : "";
            paragraph=new Paragraph(e01aK,font_1);
            cell = new PdfPCell(paragraph);
            ltrb= getLtrb("7");
            cell=excecute(cell,7,ltrb);
            cell.setFixedHeight(body_h);               //设置列最大高度
            ...
            if (gnoField != null && gnoField.length() > 0) {
                paragraph=new Paragraph(a0100[5],font_1);
            } else {
                paragraph=new Paragraph(a0100[0],font_1);
            }
	        ...
	        try{
	          ...
	          font_1=getFont(parsevo.getBody_fn(),parsevo.getBody_fb(),parsevo.getBody_fi(),parsevo.getBody_fu(),parsevo.getBody_fz(),"");
	           if(rowSet.next())
	           {
	        	   for(int i=0;i< this.columnList.size();i++ )
	               	  {
	        		      String fileds[]=(String[])this.columnList.get(i);
