...
public class SearchStandardDuty_paramFieldTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
		...
		ArrayList rightlist=new ArrayList();
		 try {
			RowSet rs=dao.search("select str_value from Constant where Upper(Constant)='SPOST_MAINSET_FIELD'");
			if(rs.next())
		...
		for(int m=0;m<rightss.length;m++){
			if(rightss[m].length()>0){
				...
				//对未构库的指标不在页面上显示 27009 wangb1 20170503
