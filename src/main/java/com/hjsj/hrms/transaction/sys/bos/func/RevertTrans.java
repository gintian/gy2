 ...
 * 
 */
public class RevertTrans extends IBusiness {
    /**
	 */
	...
	public void execute() throws GeneralException {
		...
		try {
			...
			Document doc1=null;
			if(this.getFormHM().get("function_dom")!=null){//dml 2011-04-19
				 doc1 = (Document)this.getFormHM().get("function_dom");
			}else{
				...
				for(int k=0;k<list.size();k++){
					Element dd=(Element)list.get(k);
					String id=dd.getAttributeValue("id");							//通过备份文件中节点id查找session中xml要更新的节点
					String xpath = "//function[@id=\"" + id.toLowerCase() + "\"]";
		        	...
		        	if(function!=null){												//如果session中xml存在该节点，则找到其父节点删除该节点将备份中的节点加入到该节点下。
		        		...
		        		if(parent!=null&&parent.length()!=0){
		        			String temp="//function[@id=\"" + parent.toLowerCase() + "\"]";
				...
				this.getFormHM().put("function_dom", doc1);
