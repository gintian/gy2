...
public class SaveDuty_paramFieldTrans extends IBusiness {
	public void execute() throws GeneralException {
		// TODO Auto-generated method stub
		String[] right_fields=(String[])this.getFormHM().get("right_fields");
		String fieldstr="";
		ArrayList browseFields=new ArrayList();
		ArrayList listfield=DataDictionary.getFieldList("k01",Constant.USED_FIELD_SET);
		...
		try {
			dao.delete("delete from Constant where Upper(Constant)='POST_MAINSET_FIELD'", new ArrayList());
			String sql="insert into Constant(Constant,str_value)values(?,?)";
			ArrayList list=new ArrayList();
			list.add("POST_MAINSET_FIELD");
			list.add(fieldstr);
			dao.insert(sql, list);
