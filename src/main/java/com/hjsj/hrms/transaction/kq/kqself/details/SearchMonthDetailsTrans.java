...
public class SearchMonthDetailsTrans extends IBusiness {
    private ArrayList operList(String mm) throws GeneralException {
        ...
        try {
            ...
            while (this.frowset.next()) {
                ...
                CommonData durationvo = new CommonData(this.frowset.getString("kq_year") + "-"
                        + this.frowset.getString("kq_duration"), this.frowset.getString("kq_duration") + '('
                        + PubFunc.FormatDate(this.frowset.getDate("kq_start")).replaceAll("-", "\\.") + '-'
                        + PubFunc.FormatDate(this.frowset.getDate("kq_end")).replaceAll("-", "\\.") + ')');
    ...
    private ArrayList getYear() throws GeneralException {
        ...
        //zxj 20160513 年度按倒序取
        ystr.append("select distinct kq_year from kq_duration where finished='0' order by kq_year desc");
        try {
    ...
    private ArrayList getField(String table) {
        ...
        for (int i = 0; i < fieldlist.size(); i++) {
            ...
            field.setValue("");
            field.setViewvalue("");
            if ("b0110".equals(field.getItemid()) || "a0101".equals(field.getItemid()) || "e0122".equals(field.getItemid())
                    || "nbase".equals(field.getItemid()) || "e01a1".equals(field.getItemid())
                    || "a0100".equals(field.getItemid()) || "q03z3".equals(field.getItemid())
                    || "q03z5".equals(field.getItemid()) || "state".equals(field.getItemid()))
