...
//    			continue;   		
	...
	try
	{
        ...
        ArrayList dbpre = kqUtilsClass.getKqPreList();
        ...
        if((dbpre==null||dbpre.size()==0)&& "1".equals(type))
	 ...
	 */
	private String combine_SQL(String type, ArrayList dbpre, StringBuffer sfactor, StringBuffer sexpr, ArrayList list, boolean bhis, boolean bresult,boolean blike,String filter_factor) throws GeneralException {
        ...
        if((!userView.isSuper_admin())&& "1".equals(type))
        {
        	...
        	{   
        		...
        		if ("0".equalsIgnoreCase(personPriv)) {
        			FactorList factorlist = new FactorList(sexpr.toString(), PubFunc.getStr(sfactor.toString()), dbpre.get(i).toString(),false, false, true, 1, userView.getUserName(),"");	
        			strWhere = factorlist.getSqlExpression();
        		} else {
        			strWhere=userView.getPrivSQLExpression(sexpr.toString()+"|"+PubFunc.getStr(sfactor.toString()),dbpre.get(i).toString(),bhis,bresult,fieldlist);
        		}
          	    strSelect=getSelectString(fieldlist,type,dbpre.get(i).toString());
        	    strSql.append(strSelect);
        	    strSql.append(strWhere);  
        	    //zxj 20170606 考勤管理范围权限
        	    strSql.append(" and ").append(dbpre.get(i).toString()).append("A01.a0100 in (select a0100 ").append(RegisterInitInfoData.getWhereINSql(userView, dbpre.get(i).toString())).append(")");
        	    //wlh修改添加检索过滤
			    ...
			    { 
			 	    strSql.append(" and " + getFilterSQL(userView,dbpre.get(i).toString(),alUsedFields,this.getFrameconn(),filter_factor));
			    }
        	    strSql.append(" UNION ");         		
        	}
...
//     	    strSql.append(" order by dbase desc,b0110,e0122,a0000");        	
        ...
        else
        {
            if("1".equals(type))
            {
         	   ...
         	   for(int i=0;i<dbpre.size();i++)
         	   {
             	   FactorList factor_bo=new FactorList(sexpr.toString(),PubFunc.getStr(sfactor.toString()),dbpre.get(i).toString(),bhis,blike,bresult,Integer.parseInt(type),userView.getUserId());
