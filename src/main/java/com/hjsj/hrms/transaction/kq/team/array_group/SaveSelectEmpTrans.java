...
public class SaveSelectEmpTrans extends IBusiness {
    ...
    public void execute() throws GeneralException {
        try {
            ...
            String start_date = (String) hm.get("start_date"); //开始时间
            String end_date = (String) hm.get("end_date"); //结束时间
            String flag = (String) hm.get("flag"); //true:继承班组排班 false：不继承排班
            ...
            sql.append("insert into kq_group_emp (a0100,nbase,group_id,a0101,b0110,e0122,e01a1) ");
            sql.append("values(?,?,?,?,?,?,?)");
            for (int i = 0; i < selectedinfolist.size(); i++) {
            	
                LazyDynaBean selectrec = (LazyDynaBean) selectedinfolist.get(i);
                ...
                //String nbase =kqUtilsClass.getNbaseByA0100(a0100);
                one_list.add(a0100);
                one_list.add(nbase);
            ...
            try {
                ...
                for (int i = 0; i < selectedinfolist.size(); i++) {
                    ...
                    if ("true".equalsIgnoreCase(flag)) {
                        ...
                        deleteSQL(nbase, a0100, start_date, end_date);
                        //第二步:对加入进来的人进行排班
                        saveInheritCalss(nbase, a0100, group_id, date_list);
                    }

                    if ("1".equals(zhji))
                        upclassvalue(a0100, nbase, group_id);//调换班组更改主集
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
     ...
     * 新建临时表的名字
     * **/
    private static String getTmpTableName(String UserId, String PrivCode) {
