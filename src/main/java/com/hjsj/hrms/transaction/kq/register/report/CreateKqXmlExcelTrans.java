...
import com.hjsj.hrms.businessobject.kq.register.ExecuteKqSumExcel;
...
public class CreateKqXmlExcelTrans extends IBusiness{
	...
	public void execute() throws GeneralException {
		...
		try{
			...
			String allpage = (String) hm.get("allpage");
...
//			    	code=this.userView.getUserOrgId();
			...
			String url = this.userView.getUserName()+ "_" + context.trim() + ".xls";
			if("q05".equals(parsevo.getValue().trim())){
				...
				executeKqSumExcel.executeExcel(code,kind,coursedate,parsevo,this.userView,this.getFormHM(),workbook,sheet,row,cell,curpage,allpage);
			}
