...
public class SearchRedeployRestTrans extends IBusiness {
    private String table_set = "q25";

    public void execute() throws GeneralException {
        try {
            this.getFormHM().put("table", this.table_set);
            ...
            selectAllOperate.allOperate("q25");
            ...
            if (code == null || code.length() <= 0) {
                ...
                if (code != null) {
                    if ("UN".equals(RegisterInitInfoData.getKqPrivCode(userView))) {
                        ...
                        kind = "1";
                    } else if ("@K".equalsIgnoreCase(RegisterInitInfoData.getKqPrivCode(userView))) {
                        kind = "0";
                    } else {
                        kind = "2";
                    }
            ...
            if ("3".equals(frist)) {
             ...
             //   kq_end_str = datelist2.get(1).toString();
                //取今年最后一天
            ...
            for (int i = 0; i < dblist.size(); i++) {
                ...
                sql.append(" from " + this.table_set + " q,kq_class kq," + nbase + "A01 A01");
                sql.append(" where 1=1 and q.A0100=A01.A0100");
                sql.append(" and nbase='" + nbase + "'");
                sql.append(" and q.q25z7=kq.class_id");
                /*sql.append(" and q19z1>="+Sql_switcher.dateValue(kq_start+" 00:00:00"));
                sql.append(" and q19z1<="+Sql_switcher.dateValue(kq_end+" 23:59:59"));*/
                sql.append(" and (( " + this.table_set + "z1>='" + kq_start_str + "'");
                sql.append(" and " + this.table_set + "z1<='" + kq_end_str + "' )");
                sql.append(" or(" + this.table_set + "z3>='" + kq_start_str + "'");
                sql.append(" and " + this.table_set + "z3<='" + kq_end_str + "' ))");
                if ("1".equals(kind)) {
                ...
                sql.append(" and q.q25z5 <> '01'");
                sql.append(" UNION ");
            }
            sql.setLength(sql.length() - 7);
            selectAllOperate.allSelectApp("Q25", dblist);
            ...
            String relatTableid = "25";
            this.getFormHM().put("returnURL", "/kq/app_check_in/redeploy_rest/redeploydata.do?b_search=link");
            StringBuffer whereINStr = new StringBuffer();
            whereINStr.append("1=1 and ");
            /*whereINStr.append(" and "+this.table_set+".q25z7=kq_class.class_id");
            sql.append(" and q19z1>="+Sql_switcher.dateValue(kq_start+" 00:00:00"));
            sql.append(" and q19z1<="+Sql_switcher.dateValue(kq_end+" 23:59:59"));*/
            whereINStr.append(" (( " + this.table_set + "z1>='" + kq_start_str + "'");
            whereINStr.append(" and " + this.table_set + "z1<='" + kq_end_str + "' )");
            whereINStr.append(" or(" + this.table_set + "z3>='" + kq_start_str + "'");
            whereINStr.append(" and " + this.table_set + "z3<='" + kq_end_str + "' ))");
            if ("1".equals(kind)) {
