package com.hjsj.hrms.transaction.kq.team.history;
    ...
    public void execute() throws GeneralException {
        try {
            ...
            if (select_pre == null || select_pre.length() <= 0)
                select_pre = "all";
            ArrayList session_y_list = (ArrayList) this.getFormHM().get("session_y_list");
            if (session_y_list == null || session_y_list.size() <= 0) {
                session_y_list = RegisterDate.sessionYaer(this.getFrameconn(), "1");
            }
            
            if (session_y_list == null || session_y_list.size() <= 0)
                throw new GeneralException("没有考勤封存期间");
            
            String session_y = (String) this.getFormHM().get("session_y");
            String session_y_old = (String) this.getFormHM().get("session_y_old");
            if (session_y == null || session_y.length() <= 0) {
...
//            String kqTypeWhr = kqUtilsClass.getKqTypeWhere(KqConstant.KqType.STOP, true);
            ...
            if (kq_dbase_list != null && kq_dbase_list.size() > 0) {
                if (code != null && code.length() > 0) {
                    if ("2".equals(kind)) {
                        ...
                        String code_kind = RegisterInitInfoData.getDbB0100(code, kind, this.getFormHM(), this.userView, this.getFrameconn());
            ...
            if (select_name != null && select_name.length() > 0) {
            	 String select_a0100 = "";
            	 String select_nbase = "";
            	 ArrayList list = kqShiftClass.getUserRecord(select_name, kq_dbase_list, code, kind, "");
                 if (list.size() >= 1) {
