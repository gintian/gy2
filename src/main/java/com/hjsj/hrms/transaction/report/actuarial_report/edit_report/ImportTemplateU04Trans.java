...
public class ImportTemplateU04Trans extends IBusiness {
	...
	public void execute() throws GeneralException {
		...
		try {
			...
			ArrayList fieldlist = reportCycleBo.getU04FieldList(report_id, false);
			String unitcode = "";
			...
			sql.append("update U04 set ");
			StringBuffer insert_sql = new StringBuffer();
			StringBuffer insert_value = new StringBuffer();
			insert_sql.append("insert into U04(");
			int factCols = 0;
			...
			for (short i = 0; i < cols; i++) {
				...
				if (cell != null) {
					...
					if ("U0401".equalsIgnoreCase(field)) {
						persontype = i;
						continue;
					}
			...
			if (fields.indexOf("U0401") == -1)
				throw new GeneralException("标题行不存在人员分类的U0401批注！请设置人员分类来导入数据！");

			sql.setLength(sql.length() - 1);
			sql.append(" where u0401=? and unitcode=? and id='" + id + "'");
			insert_sql.append("U0401,U0403,editflag,id,unitcode) values (");
			...
			for (int j = 1; j <= rows; j++) {
				...
				//U0401,U0403,editflag,id,unitcode
			...
			info.append("&nbsp;&nbsp;导入：" + list_insert.size() + " 条；");
