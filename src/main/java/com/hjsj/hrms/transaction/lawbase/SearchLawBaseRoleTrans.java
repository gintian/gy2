 ...
 *
 */
public class SearchLawBaseRoleTrans extends IBusiness {
	 public void execute() throws GeneralException 
	 {
	        HashMap hm=(HashMap)this.getFormHM().get("requestPamaHM");
	        String base_id=(String)hm.get("a_base_id");
	        base_id = PubFunc.decrypt(SafeCode.decode(base_id));
	        String closeFlag = (String)hm.get("close");
	        StringBuffer strsql=new StringBuffer();
		    strsql.append("select role_id,role_name,role_desc,role_property,valid,status from t_sys_role where valid=1 order by norder");
		    ...
		    this.getFormHM().put("closeFlag", closeFlag);
	 }
	 /***
	     * 分析登录用户是否拥有此角色
	     * @param role_id
