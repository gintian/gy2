	...
	public void execute() throws GeneralException {
		...
		if("1".equalsIgnoreCase(flag)){
			...
			if(salaryid.length()>0){
				...
				//根据原来的排序顺序重新排列已选列表 zhanghua 2017-8-28
				ArrayList itemlistTemp=new ArrayList();
				for(String str:sortitems){
					for(int i=0;i<sortitemlist.size();i++){
						String  tempstr=(String)sortitemlist.get(i);
						if(str.toLowerCase().indexOf(tempstr.split(":")[0].toLowerCase())!=-1){
							itemlistTemp.add(tempstr);
							break;
						}
					}
				}
				sortitemlist=itemlistTemp;
