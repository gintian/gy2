	...
	public void execute() throws GeneralException {
		...
		try{
			...
			for(int i=0;i<list.size();i++){
				...
				if("01".equals(sp)){
					exper.append("\n\n["+r2502+"]"+ResourceFactory.getProperty("train.b_plan.approval.submit.drafting")+"!");
					continue;
				}else if("03".equals(sp)){
					exper.append("\n\n["+r2502+"]"+ResourceFactory.getProperty("train.b_plan.approval.approved")+"!");
					continue;
				}else if("04".equals(sp)){
					exper.append("\n\n["+r2502+"]"+ResourceFactory.getProperty("train.b_plan.approval.published")+"!");
					continue;
				}else if("05".equals(sp)){
					exper.append("\n\n["+r2502+"]"+ResourceFactory.getProperty("train.b_plan.approval.perform")+"!");
					continue;
				}else if("06".equals(sp)){
					exper.append("\n\n["+r2502+"]"+ResourceFactory.getProperty("train.b_plan.approval.end")+"!");
					continue;
				}else if("09".equals(sp)){
					exper.append("\n\n["+r2502+"]"+ResourceFactory.getProperty("train.b_plan.approval.suspended")+"!");
					continue;
				}
			...
			if(namestr.trim().length()>0){
				...
				sqlstr.append("update r25 set r2509='03' where R2501 in(");
				...
				if(tbb.getBudget()!=null&&tbb.getBudget().length()>0){
					...
					for (int i = 0; i < pri.length; i++) {
						if(pri[i]!=null||pri[i].length()>0)
							tbb.updateTrainPlanBudget("0", pri[i], -999999, false);
					}
				}
			}
		}catch(Exception ex){
