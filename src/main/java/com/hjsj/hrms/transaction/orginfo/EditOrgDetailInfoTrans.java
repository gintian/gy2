 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class EditOrgDetailInfoTrans extends IBusiness {
							...
							if("update".equals(edittype))
							{
								if(isExistData)
								{
									...
									}else if("D".equals(fieldItem.getItemtype()))                 //日期型有待格式化处理
									{
										...
										}else if(rec.get(fieldItem.getItemid())!=null && rec.get(fieldItem.getItemid()).toString().length()>=18 && fieldItem.getItemlength()==18)
										{
