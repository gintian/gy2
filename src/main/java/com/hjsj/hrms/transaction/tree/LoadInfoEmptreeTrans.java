...
public class LoadInfoEmptreeTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
	    ...
	    String url="/common/orgemp/loadtree?target="+target+"&action=" + action + "&flag=1&dbtype=1&privtype="+privtype+"&kq_type="+kq_type;
	    String urls = "";
		if(!("UN".equals(a_code)))
		{
		...
		{
			urls=urls+"&params=codeitemid%3Dparentid&id=UN";
		}
		
		if ("kq".equalsIgnoreCase(privtype)) {
			urls = urls + "&privtype=" + privtype;
		}
	    ...
	    if(!"org".equals(treetype) && !"duty".equals(treetype))
	        treeItem.setAction(action + "?b_search=link&isroot=1&a_code="+codeid+""+ codevalue);
	    else
