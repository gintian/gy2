 ...
 * Title:保存自评分数交易类
 * 
 * create time:2005-6-21:13:52:54
 * 
 * @author luangaojiong
 */
public class SaveAppraiseselfTrans extends IBusiness {
	...
	public void execute() throws GeneralException 
	{
		 ...
		 */
		String nowPlanNum = "";
		if (this.getFormHM().get("nowPlanNum") != null) 
		{
			nowPlanNum = this.getFormHM().get("nowPlanNum").toString();
		 ...
		 */
		 ...
		 LoadXml loadxml=new LoadXml(this.getFrameconn(),nowPlanNum);
		 htxml=loadxml.getDegreeWhole();
		 /**最高标度加上限制*/
		 GradeCodeBo codebo=new GradeCodeBo(this.getFrameconn(),this.userView.getA0100(),Integer.parseInt(nowPlanNum));
		 codebo.initdata();
...
//						}
		...
		SaveSorce(listpointid,listquantity,tableName);
		/**执行了解程度及总体评价*/
		doWholeDegree(wholeEven ,knowDegree,nowPlanNum);
		/**保存打分提交状态*/		
		PerMainBody permainbody=new PerMainBody(this.getFrameconn());
		permainbody.updateEditStatus(userView.getA0100(),userView.getA0100(),nowPlanNum,"1");
	 ...
	 * @param planId
	 */
	void SaveSorce(ArrayList listpointid, ArrayList listquantity,String tableName)
	{
		saveDxScore(listpointid, tableName);
		saveDlScore(listquantity, tableName);		
	}
	 ...
	 * @param tableName
	 */
	private void saveDlScore(ArrayList listquantity, String tableName) {
		...
		strUpdate.append(" set score=?,degree_id=?,amount=? where point_id=? and object_id=? and mainbody_id=?");		

		strInsert.append("insert into ");
		strInsert.append(tableName);
		strInsert.append(" (id,object_id,mainbody_id,score,degree_id,amount,point_id) values (?,?,?,?,?,?,?)");
		...
		try
		{
			for(int i=0;i<listquantity.size();i++)
			{
				...
				if(this.frowset.next())
				{
					...
					temp.add(Double.toString(vo.getScore())/* amountToScore.getAmountToScore(vo.getPoint_id(),vo.getValue())*/);
	 ...
	 * @param tableName
	 */
	private void saveDxScore(ArrayList listpointid, String tableName) {
		StringBuffer strquery=new StringBuffer();
		strquery.append("select id from ");
		strquery.append(tableName);
		strquery.append(" where point_id=? and object_id=? and mainbody_id=?");
	 ...
	 * @return
	 */
	void doUpdateOper(ArrayList listpointid, ArrayList listquantity,String tableName,String planId)
	...
	ArrayList getFilter(ArrayList list,String planId)
	{
		...
		catch (Exception ex) 
		{
			...
			System.out.println("----->com.hjsj.hrms.transaction.performance.SaveAppraiseselfTrans--->read planid second error");
			ex.printStackTrace();
		}
	 ...
	 * 执行总体评价
	 */
	public void doWholeDegree(String wholeEven ,String knowDegree,String nowPlanNum)
	{
	...
	public void doInsertOper(ArrayList listpointid, ArrayList listquantity,
			String tableName,String planId) {
		...
		try {
			...
			if (listpointid.size() > 0) {
				...
				if(num>0)
				{
					 ...
					 * 设为提交状态
					 */
					String sql="update per_mainbody set status=2 where object_id='"+this.userView.getA0100()+"' and mainbody_id='"+this.userView.getA0100()+"' and plan_id="+planId;
					dao.update(sql);
				}
			}
			/**
