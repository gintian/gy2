...
public class ShowCardDetailTrans extends IBusiness{
	...
	public void execute() throws GeneralException {
		try{
			...
			if(!(fromjs==null || "".equals(fromjs))){
				object_id = PubFunc.decryption((String)hm.get("cardObject_id")); //从获得考核对象
				hm.remove("cardObject_id");
			}
			else
				object_id = PubFunc.decryption((String) this.getFormHM().get("cardObject_id")); //获得考核对象
			...
			if("1".equals(scoreExplainFlag)){//如果用户勾选了"显示评分说明"
				...
				cardHtml = bo.getTableHtmlDetail();
			}
			...
			this.getFormHM().put("cardObject_id", PubFunc.encryption(object_id));
