 ...
 * @date 2019年3月28日
 */
public class BatchImportObjectCardTrans extends IBusiness {
    @Override
    public void execute() throws GeneralException {
        ...
        try
        {
            ...
            String[] objectIdArr = object_ids.split(",");
            List<String> objectIdArrNew = new ArrayList<String>();
            ...
            for(String object_id_e : objectIdArr) {
                if(StringUtils.isEmpty(object_id_e))
                    continue;
                String object_id = PubFunc.decrypt(object_id_e);
                StringBuffer sql = new StringBuffer("select pp.plan_Id from per_plan pp,per_object po where pp.plan_id=po.plan_id  ");
                sql.append(" and  pp.method=2 and pp.cycle=? and pp.template_id=? ");
                valList.clear();
                valList.add(planVo.getInt("cycle"));
                valList.add(template_id);

                if (planVo.getInt("cycle") == 3) //月度
                {
                    sql.append(" and ( pp.theyear<?");
                    sql.append(" or ( pp.theyear=? and pp.themonth<? ) )");
                    valList.add(planVo.getString("theyear"));
                    valList.add(planVo.getString("theyear"));
                    valList.add(planVo.getString("themonth"));
                } else if (planVo.getInt("cycle") == 2 || planVo.getInt("cycle") == 1) //季度 || 半年
                {
                    sql.append(" and ( pp.theyear<?");
                    sql.append(" or ( pp.theyear=? and pp.thequarter<? ) )");
                    valList.add(planVo.getString("theyear"));
                    valList.add(planVo.getString("theyear"));
                    valList.add(planVo.getString("thequarter"));
                ...
                } else //不定期
                {
                    ...
                    sql.append(" and ( " + Sql_switcher.year("start_date") + "<?");
                    sql.append(" or ( " + Sql_switcher.year("start_date") + "=? and " + Sql_switcher.month("start_date") + "<? ) ");
                    sql.append(" or ( " + Sql_switcher.year("start_date") + "=? and " + Sql_switcher.month("start_date") + "=? and " + Sql_switcher.day("start_date") + "<? ) ) ");
                    valList.add(date.substring(0, 4));
                    valList.add(date.substring(0, 4));
                    valList.add(date.substring(date.indexOf("-") + 1, date.lastIndexOf("-")));
                    valList.add(date.substring(0, 4));
                    valList.add(date.substring(date.indexOf("-") + 1, date.lastIndexOf("-")));
                    valList.add(date.substring(date.lastIndexOf("-") + 1));
                }
                ...
                if ("false".equalsIgnoreCase(noApproveTargetCanScore)) {//上期目标卡所在计划启动时选择了“允许对未批准的目标卡进行评分”，就无需判断目标卡是否已批了 zhaoxg add 2016-8-16
                    sql.append(" and po.object_id=? and (sp_flag='03' or sp_flag='06') "); // 引入上期已经批准的目标卡
                    valList.add(object_id);
                }
                if (planVo.getInt("cycle") == 3) //月度
                ...
                rowSet = dao.search(sql.toString(),valList);
                while (rowSet.next()) {
                    ...
                    valList.clear();
                    String sql0 = "select count(p0400) from P04 where plan_id=?  and ";
                    valList.add(a_plan_id);
                    ...
                    valList.add(object_id);
                    rowSet2 = dao.search(sql0,valList);
                    if (rowSet2.next() && rowSet2.getInt(1) > 0) {
                        valList.clear();
                        sql0 = "select * from P04 where plan_id=? and a0100=? and ( chg_type<>3 or chg_type is null) order by seq";
                        if (object_type == 1 || object_type == 3 || object_type == 4) //团队
                            sql0 = "select * from P04 where plan_id=? and b0110=? and ( chg_type<>3 or chg_type is null) order by seq";
                        valList.add(a_plan_id);
                        valList.add(object_id);
                        rowSet2 = dao.search(sql0,valList);
                        ...
                        if (list.size() > 0) {
                            objectIdArrNew.add(object_id_e);
                            List strValues = new ArrayList();
                            List sql1Values = new ArrayList();
                            if (opt != null && "1".equalsIgnoreCase(opt)) {
                                ...
                                if (parameter.get("TaskSupportAttach") != null && "True".equalsIgnoreCase((String) parameter.get("TaskSupportAttach"))) {
                                    str = "delete from per_article where plan_id=? and a0100=? and article_type=3 ";
                                    strValues.add(plan_id);
                                    strValues.add(object_id);
                                }

                                String sql_1 = "delete from p04 where plan_id=? and a0100=? ";
                                sql1Values.add(plan_id);
                                sql1Values.add(object_id);
                                if (object_type == 1 || object_type == 3 || object_type == 4) //团队
                                {
                                    sql_1 = "delete from p04 where plan_id=? and b0110=? ";
                                    ...
                                    if (parameter.get("TaskSupportAttach") != null && "True".equalsIgnoreCase((String) parameter.get("TaskSupportAttach"))) {
                                        ...
                                        if (mainbody_id != null && mainbody_id.trim().length() > 0) {
                                            ...
                                            strValues.clear();
                                            strValues.add(plan_id);
                                            strValues.add(mainbody_id);
                                        }
                                    }
                                }
                                dao.delete(sql_1, sql1Values);
                                if (parameter.get("TaskSupportAttach") != null && "True".equalsIgnoreCase((String) parameter.get("TaskSupportAttach"))) {
                                    if (str != null && str.trim().length() > 0)
                                        dao.delete(str.toString(), strValues);
                                }
                                ...
                                if (signFlag && (parameter.get("TaskSupportAttach") != null && "True".equalsIgnoreCase((String) parameter.get("TaskSupportAttach"))) && (params.get("TaskSupportAttach") != null && "True".equalsIgnoreCase((String) params.get("TaskSupportAttach")))) {
                                    for (int i = 0; i < p04List.size(); i++) {
                                        ...
                                        valList.clear();
                                        if (object_type == 2) {
                                            strsql = ("select * from per_article where plan_id=? and a0100=? and article_type=3 and task_id=?");
                                            valList.add(a_plan_id);
                                            valList.add(object_id);
                                            valList.add(p0400);
                                            ...
                                            strsql = ("select * from per_article where plan_id=? and article_type=3 and task_id=?");
                                            valList.add(a_plan_id);
                                            valList.add(p0400);
                                        }
                                        rwSet = dao.search(strsql,valList);
                                        while (rwSet.next()) {
                                            int article_id = 0;
            ...
            object_ids = "";
            if(objectIdArrNew.size()>0){
                object_ids = StringUtils.join(objectIdArrNew.toArray(),",");
            }
            this.getFormHM().put("object_ids",object_ids);
    ...
    public LazyDynaBean getMainbodyBean(String plan_id,String object_id)
    {
        ...
        try
        {
            ...
            rowSet=dao.search("select * from per_mainbody where plan_id=? and object_id=? and body_id=-1",values);
            if(rowSet.next())
            {
