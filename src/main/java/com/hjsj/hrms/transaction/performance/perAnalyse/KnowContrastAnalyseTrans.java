...
/** 
 *<p>Title:KnowContrastAnalyseTrans.java</p> 
 *<p>Description:了解程度对比分析</p> 
 *<p>Company:HJHJ</p> 
...
public class KnowContrastAnalyseTrans extends IBusiness 
{
	...
	public void execute() throws GeneralException 
	{
		try
		{
			...
			ArrayList planList=bo.getPlanList_commonData("7",2,1,this.getUserView(),plan_id,busitype);
//			ArrayList planList=bo.getPlanList_commonData("7",0,0,this.getUserView(),plan_id,busitype);
			...
			if(hm.get("b_knowAnalyse0")!=null&& "query0".equals((String)hm.get("b_knowAnalyse0")))
			{
				if(planList.size()>0)
					planID=((CommonData)planList.get(0)).getDataValue();
				hm.remove("b_knowAnalyse0");
			...
			else if(hm.get("b_knowAnalyse0")!=null&& "query".equals((String)hm.get("b_knowAnalyse0")))
			{
			...
			{
				String object_id=(String)hm.get("codeitemid");
				if(!"root".equals(object_id)){
					object_id = PubFunc.decrypt(object_id);
				}
				planID=(String)this.getFormHM().get("planIds");
				pointToNameList=bo.getPlanPointList(planID);
			    ArrayList knowGradeList=bo.getKnowList();
			    ChartParameter chartParam=new ChartParameter();
			    ...
			    dataMap=bo.getKnowContrastData(planID, object_id,knowGradeList,pointToNameList,selectids,isShowPercentVal,chart_type);
				...
				this.getFormHM().put("chartParam", chartParam);
				scoreGradeStr=bo.getKnowScoreGradeStr(knowGradeList,selectids);
