...
public class EditStatItemTrans extends IBusiness{
	  ...
	  public void execute() throws GeneralException 
	  {
		  ...
		  if("edit".equals(opflag)&&(editid==null||editid.length()<=0))
  		  ...
  		  sql.append("select name,Infokind from hr_hisdata_sname where id='"+statid+"'");
  		  String stat_name="";
  		  try {
		...
		{
			  sql.setLength(0);
	  		  sql.append("select nOrder,legend,lexpr,factor from hr_hisdata_slegend where id='"+statid+"' and norder ='"+editid+"' order by nOrder");
	  		  int j=0;  		  
	  		  try {
				this.frowset=dao.search(sql.toString());
				Factor factor=null;			
