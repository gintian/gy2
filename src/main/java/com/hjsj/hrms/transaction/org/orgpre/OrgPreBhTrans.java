 ...
 * 
 */
public class OrgPreBhTrans extends IBusiness {
...
//		 TODO Auto-generated method stub
		...
		try{
			...
			if(list.size()<1)
				throw GeneralExceptionHandler.Handle(new Exception(ResourceFactory.getProperty("workdiary.message.select.app.subset")+"!"));
			...
			for(int i=0;i<list.size();i++){
				...
				if("01".equals(sp)){
					exper.append("\n\n["+namestr+"]"+ResourceFactory.getProperty("train.b_plan.dismissed.submit.drafting")+"!");
					continue;
				}else if("03".equals(sp)){
					exper.append("\n\n["+namestr+"]"+ResourceFactory.getProperty("train.b_plan.dismissed.approved")+"!");
					continue;
				}else if("07".equals(sp)){
					exper.append("\n\n["+namestr+"]"+ResourceFactory.getProperty("train.b_plan.dismissed.dismissed")+"!");
					continue; 
				}
			...
			String updatestr = "";
			if(fieldset.isMainset()){
				updatestr = "update "+name.toUpperCase()+" set "+sp_flag+"='07' ";
				if("B".equalsIgnoreCase(name.substring(0,1))){
				...
				updatestr = "update "+name+" set "+sp_flag+"='07' ";
				if("B".equalsIgnoreCase(name.substring(0,1))){
					updatestr+="where B0110=?";
					updatestr+=" and I9999=?";
				}else if("K".equalsIgnoreCase(name.substring(0,1))){
