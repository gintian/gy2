 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class SearchTarOrgTreeTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
		...
		else
		{
			if(userView.getStatus()==4 || userView.getStatus()==0)
				treeItem.setLoadChieldAction("/common/org/loadtree?params=root&nmodule="+nmodule+"&parentid=00&issuperuser=0&treetype=employee&action=javascript:void(0)&manageprive=" + userView.getManagePrivCode() + userView.getManagePrivCodeValue());
		    else
			    treeItem.setLoadChieldAction("/common/org/loadtree?params=root&nmodule="+nmodule+"&parentid=00&issuperuser=0&treetype=employee&action=javascript:void(0)&manageprive=" + userView.getManagePrivCode() + "no");
	    ...
	    this.getFormHM().put("tarTreeCode","Global.closeAction=\"savecode();window.close();\";" + treeItem.toJS());
	}
