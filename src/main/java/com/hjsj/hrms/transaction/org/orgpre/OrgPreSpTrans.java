 ...
 * 
 */
public class OrgPreSpTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
		...
		try{
			...
			for(int i=0;i<list.size();i++){
				...
				if("01".equals(sp)){
					exper.append("\n\n["+namestr+"]"+ResourceFactory.getProperty("train.b_plan.approval.submit.drafting")+"!");
					continue;
				}else if("03".equals(sp)){
					exper.append("\n\n["+namestr+"]"+ResourceFactory.getProperty("train.b_plan.approval.approved")+"!");
					continue;
				}else if("07".equals(sp)){
					exper.append("\n\n["+namestr+"]"+ResourceFactory.getProperty("train.b_plan.approval.dismissed")+"!");
					continue; 
				}
			...
			String updatestr = "";
			if(fieldset.isMainset()){
				updatestr = "update "+name.toUpperCase()+" set "+sp_flag+"='03' ";
				if("B".equalsIgnoreCase(name.substring(0,1))){
				...
				updatestr = "update "+name+" set "+sp_flag+"='03' ";
				if("B".equalsIgnoreCase(name.substring(0,1))){
					updatestr+="where B0110=?";
					updatestr+=" and I9999=?";
				}else if("K".equalsIgnoreCase(name.substring(0,1))){
