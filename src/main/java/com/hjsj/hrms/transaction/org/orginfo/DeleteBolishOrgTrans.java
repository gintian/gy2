 ...
 *
 * modify 2012-11-13 zxj 修复撤销机构并清空人员机构信息时未清空的问题。
 */
public class DeleteBolishOrgTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
		...
		HashMap paramMap = (HashMap)this.getFormHM().get("requestPamaHM");
		//String emptypersonorg = (String)paramMap.get("emptypersonorg");
		...
		if(userView.getVersion()>=50){
			...
			try{
				if (tarorglist != null && tarorglist.length() > 0) 
    		...
    		try{
    			...
    			for(int i=0;i<bolishlist.size();i++)
    			{
    				...
    				personwheresql.append(codeitemid);
    				...
    				orgwheresql.append(codeitemid);				
    				orgwheresql.append("%'");
    				
    				poswheresql.append(" or e01a1 like '");
    				poswheresql.append(codeitemid);				
    				poswheresql.append("%'");
		...
		try {
			for (int i = 0; i <bolishlist.size(); i++) {
				...
				sqlK01="update K01 set modusername=?,modtime=? where e01a1 like ?";
				sqlB01="update B01 set modusername=?,modtime=? where b0110 like ?";
		...
		//清空人员组织机构
		//if("1".equalsIgnoreCase(emptypersonorg))
