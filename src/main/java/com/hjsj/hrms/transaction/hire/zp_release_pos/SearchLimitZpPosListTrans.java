 ...
 * 
 */
public class SearchLimitZpPosListTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
		...
		if("su".equals(this.userView.getUserId())){
			strsql.append("select zp_pos_id,amount,plan_id,dept_id,pos_id,valid_date,domain,status from zp_position where pos_id in (select codeitemid from organization");
		}else if(this.userView.getManagePrivCode() != null && !"".equals(this.userView.getManagePrivCode())){
	       strsql.append("select zp_pos_id,amount,plan_id,dept_id,pos_id,valid_date,domain,status from zp_position where pos_id in (select codeitemid from organization where codeitemid like '"+this.userView.getManagePrivCodeValue()+"%')");
		}else{
			strsql.append("select zp_pos_id,amount,plan_id,dept_id,pos_id,valid_date,domain,status from zp_position where pos_id = ''");	
		}
