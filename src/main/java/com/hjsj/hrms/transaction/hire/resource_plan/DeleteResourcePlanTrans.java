...
/**
 * <p>Title:DeleteResourcePlanTrans</p>
 * <p>Description:删除人力规划，zp_hr_plan</p>
 * <p>Company:hjsj</p>
...
public class DeleteResourcePlanTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
        ...
        try
        {
        	for(int i=0;i<zpplanlist.size();i++)
        	{
        		...
        		sql="delete from zp_hr_plan where plan_id = '" + plan_id + "'";
        		this.frowset=dao.search("select status from zp_hr_plan where plan_id='" + plan_id + "'");
        		if(this.frowset.next())
        			status=this.frowset.getString("status");
        		ArrayList templst=new ArrayList();
        		dao.delete(sql,templst);
        		this.frowset=dao.search("select distinct gather_id from zp_hr_plan_details where plan_id='" + plan_id + "'");
        		...
        		sql_pos = "delete from zp_hr_plan_details where plan_id = '" + plan_id + "'";
