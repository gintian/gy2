	...
	public void execute() throws GeneralException {
	    ...
	    try{
	    	if(setStr != null && !"".equals(setStr)){
	    	   ...
	    	   subsetList = splitField(setStr);
	    	   setname = setStr.substring(0,3);
	    	   for(int i=0;i<subsetList.size();i++){
	    	   	  if(!infoSetList.isEmpty())
		    	  {
		    	     for(int k=0;k<infoSetList.size();k++)
		    	     {
		    	        ...
		    	        if(((String)subsetList.get(i)).toLowerCase().indexOf(fieldset.getFieldsetid().toLowerCase())!=-1)
		    	        {
	    	...
	    	if(fieldindex != -1){
	    		...
	    		subfieldstr = fieldStr.substring(fieldindex+4,fieldindex+subindex);
