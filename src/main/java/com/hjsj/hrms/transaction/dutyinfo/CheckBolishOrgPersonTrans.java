		...
		if(bolishlist==null && bolishlist.size()==0)
		 ...
		 ArrayList movedpersons = (ArrayList)this.getFormHM().get("movepersons");
		  ...
		  try{
			...
			if(ishaveperson)
			   this.getFormHM().put("ishavepersonmessage",ResourceFactory.getProperty("label.duty.haveperson"));
		    else
		       this.getFormHM().put("ishavepersonmessage",ResourceFactory.getProperty("label.duty.noperson"));
		}catch(Exception e)
		{
			e.printStackTrace();
