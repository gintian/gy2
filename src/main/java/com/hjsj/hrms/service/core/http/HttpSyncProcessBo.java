...
public class HttpSyncProcessBo {
								...
								for (int o = 0; o < subLists.size(); o++) {
									...
									for (int oo = 0; oo < subRecordChildList.size(); oo++) {
										...
										subMap.put(subRecordChildElement.getName(),
												subRecordChildElement.getValue() == null ? ""
														: subRecordChildElement.getValue());
									}
									...
									if (null != subChidMoveList && subChidMoveList.size() > 0) {
										for (int k = 0; k < subChidMoveList.size(); k++) {
											...
											if (isExecuteSql(subMoveConf, mainMap, subMap, sqlValueMap, conn, "1")) {
												...
												String subToStr = "";
												String a0100Str = "";
												if (!"".equals(subFrom.trim()) && !"".equals(subTo.trim())
														&& !"".equals(a0100.trim())) {
													// 有冒号的表示有引用XML中配置的值
													if (subFrom.toUpperCase().indexOf(":".toUpperCase()) != -1) {
														if (subFrom.toUpperCase().indexOf(".".toUpperCase()) != -1) {
															String[] subForms = subFrom.substring(1, subFrom.length())
																	.split(",");
															subFormStr = sqlValueMap.get(subForms[0]).toString();
														} else {
															subFormStr = mainMap
																	.get(subFrom.substring(1, subFrom.length()))
																	.toString();
														}
													} else {
														subFormStr = subFrom;
													...
													if (subTo.toUpperCase().indexOf(":".toUpperCase()) != -1) {
														if (subTo.toUpperCase().indexOf(".".toUpperCase()) != -1) {
															String[] mainTos = subTo.substring(1, subTo.length())
																	.split(",");
															subToStr = sqlValueMap.get(mainTos[0]).toString();
														} else {
															subToStr = mainMap.get(subTo.substring(1, subTo.length()))
																	.toString();
														}
													} else {
														subToStr = subTo;
													...
													if (a0100.toUpperCase().indexOf(":".toUpperCase()) != -1) {
														if (a0100.toUpperCase().indexOf(".".toUpperCase()) != -1) {
															String[] a0100s = a0100.substring(1, a0100.length())
																	.split("[.]");
															a0100Str = sqlValueMap.get(a0100s[0]).toString();
														} else {
															a0100Str = mainMap.get(a0100.substring(1, a0100.length()))
																	.toString();
														}
													} else {
														a0100Str = a0100;
													...
													// 移库操作
													DbNameBo dbNameBo = new DbNameBo(conn);
													String userName = dbNameBo.moveDataBetweenBase2(a0100Str,
															subFormStr, subToStr, "1");
													break;
												}
											} else {
												log.error("子集--->>第" + (k + 1) + "条移库操作条件不成立,不执行移库操作...");
											}
										}
									}
								}
	 ...
	 * @return A0100:nbase
	 */
	private Map getA0100AndNbaseMap(String fieldName, String fieldValue, Connection conn) {
		RowSet rs = null;
		Map str = new HashMap();
		try {
			ContentDAO dao = new ContentDAO(conn);
	 ...
	 * @return
	 */
	private int getI9999(String type, String nbase, String a0100, String setid, Connection conn) {
		int i9999 = 1;
		RowSet rs = null;
		try {
			ContentDAO dao = new ContentDAO(conn);
	 ...
	 * */
	public HashMap getTransFromXmlByParam(String path, PareXmlUtils utils, Map mainMap, Map subMap, Map sqlValueMap,
		...
		if ("1".equalsIgnoreCase(isMainOrSub.trim())) {
			for (int i = 0; i < mainList.size(); i++) {
				...
				if (mainNodeName != null && mainMap.get(mainNodeName) == null
						&& !"sql".equalsIgnoreCase(mainNodeType.trim())) {
					map.put(mainNodeId, "#&#");
				} else {
							...
							if (map == null || map.get(mainNodeDescId) == null
								...
								if (mainStr == null || mainStr.trim().length() <= 0)
									...
									map.put(mainNodeId, getCodeIdByCodeDesc(mainNodeSetId,
											map.get(mainNodeDescId).toString(), conn));
							}
						}
