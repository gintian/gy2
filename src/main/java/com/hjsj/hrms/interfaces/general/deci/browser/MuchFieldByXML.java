...
/**
 * <p>Title:MuchFieldByXML</p>
 * <p>Description:多个指标分析XML树图生成</p>
 * <p>Company:hjsj</p>
 * <p>create time:Aug 29, 2006:11:49:17 AM</p>
 * @author zhangfengjin
 * @version 1.0
 * 
 */
public class MuchFieldByXML {
	 ...
	 * @param target
	 */
	public MuchFieldByXML(String params , String actionName,String target) {
		this.params = params;
	...
	public String outPutSingleFieldXml() throws GeneralException{
		...
		try {
			...
			if("-1".equals(this.params)){
				strsql.append(" select * from ds_key_itemtype ");
			}else{
				strsql.append(" select * from ds_key_item where typeid = '");
			...
			while (rs.next()) {
				...
				if("-1".equals(this.params)){
					...
					child.setAttribute("text", itemname);
					child.setAttribute("title", itemname);
				/*	theaction = this.actionName + "?b_query=link&code="+ typeid;									
					...
					theaction = this.actionName + "?b_search=link&itemid="+ itemid;									
					child.setAttribute("href", theaction);
					child.setAttribute("target", this.target);
