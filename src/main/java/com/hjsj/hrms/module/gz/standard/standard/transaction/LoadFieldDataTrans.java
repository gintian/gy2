...
public class LoadFieldDataTrans extends IBusiness {
    ...
    private ArrayList loadFields(String key,int sourceSize){
        ...
        if("A".equalsIgnoreCase(key)){
            ...
            itemMap =  new HashMap();
            itemMap.put("id","A");
            itemMap.put("text",ResourceFactory.getProperty("gz_new.gz_personFieldTitle"));
        }
        if("B".equalsIgnoreCase(key)){
            domain = Constant.UNIT_FIELD_SET;
            itemMap =  new HashMap();
            itemMap.put("id","B");
            itemMap.put("text",ResourceFactory.getProperty("gz_new.gz_unitFieldTitle"));
        }
        if("K".equalsIgnoreCase(key)){
            domain = Constant.POS_FIELD_SET;
            itemMap =  new HashMap();
            itemMap.put("id","K");
            itemMap.put("text",ResourceFactory.getProperty("gz_new.gz_postFieldTitle"));
        }
    ...
    private ArrayList loadStrutsFieldList(String key){
        ...
        if(key.startsWith("A") || key.startsWith("B") || key.startsWith("K")){
            children = this.loadStrutsABKFieldList(key);
            return children;
        }else{
    ...
    private ArrayList loadStrutsABKFieldList(String key){
        ArrayList children = new ArrayList();
        ...
        String sql = "select itemid,fieldsetid,itemtype,itemdesc,codesetid from fielditem where fieldsetid = ?";
        ...
        try {
            ...
            while(this.frowset.next()){
                ...
                HashMap field = new HashMap();
                field.put("id",this.frowset.getString("itemid"));
                field.put("text",this.frowset.getString("itemdesc"));
                field.put("itemid",this.frowset.getString("itemid"));
                field.put("itemdesc",this.frowset.getString("itemdesc"));
                field.put("fieldsetid",this.frowset.getString("fieldsetid"));
                field.put("fieldsetdesc",fs.getCustomdesc());
