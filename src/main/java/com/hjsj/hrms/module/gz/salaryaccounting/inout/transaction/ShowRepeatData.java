 ...
 *@author lis
 */
public class ShowRepeatData extends IBusiness {
    ...
    public void execute() throws GeneralException {
		...
		try{
			...
			String tableName="t#import_gz_temptable_"+this.userView.getUserName();
			/**薪资类别*/
			ArrayList oriDataList=new ArrayList();
			// 由于现在vfs返回的流只能读一次 故这里再获取一遍
			input = VfsService.getFile(fileid);
			oriDataList=inOutBo.getRepeatDataList(input, relationTemp, originalHeadList, tableName);
			
			this.getFormHM().put("repeatDataList", oriDataList);
		}
		catch(Exception e)
		{
			e.printStackTrace();
