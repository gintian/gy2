...
/**
 * @Title DeleteSalaryHistoryDataTrans
 * @Description 删除历史数据
 * @Company hjsj
 * @Author wangbs
 * @Date 2020/1/13
 * @Version 1.0.0
 */
public class DeleteSalaryHistoryDataTrans extends IBusiness{
	...
	public void execute() throws GeneralException {
		...
		try {
			...
			if(b_units.length()==0 || StringUtils.equalsIgnoreCase(b_units,"UN")){
				...
				{
						throw new GeneralException("gz.historyData.msg.noDeletePriv");
				}
			}
			
			service.syncSalaryarchiveStrut();
			service.syncSalaryTaxArchiveStrut();
    		service.deleteSalaryHistoryData(type,startDate, endDate,salaryId);
