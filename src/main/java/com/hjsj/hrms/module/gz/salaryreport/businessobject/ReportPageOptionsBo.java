...
public class ReportPageOptionsBo {
	...
	public String createXML(ReportParseVo rpv, int type)
	{
		...
		try
		{
			...
			String sql = "select ctrlparam from reportdetail where rsid="+ this.rsid ;
			if(!"4".equals(this.rsid))
				sql+=" and rsdtlid=" + this.rsdtlid;
			rs = dao.search(sql);
			while(rs.next())
			{
	...
	public void saveXML(String xml){
		...
		try{
			{
				strsql.append("update reportdetail set ctrlparam=? where rsid='"+this.rsid+"' and rsdtlid='"+this.rsdtlid+"'");
