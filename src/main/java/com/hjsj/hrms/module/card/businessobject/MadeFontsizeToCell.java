...
public class MadeFontsizeToCell {
	...
	public int ReChangeRowNum(int rWidth,String strData,int fontsize,String fontname,String distinguish,String strType)
	{
		...
		if(strDatas!=null&&strDatas.length>1)
		{
			for(int i=0;i<strDatas.length;i++)
			{
				...
				int width=StringUtils.isNotEmpty(str)?(int)(g.getFontMetrics().stringWidth(str)*str.getBytes().length/(str.length())):0;
				int strwidth=width;//getStrWidth(fontsize,rWidth,str);
				int strwidth2=0;		
				if(strType!=null&& "D".equalsIgnoreCase(strType))
				  strwidth2=width;//getStrWidth(fontsize,rWidth,str);		
				else
				  strwidth2=width;//((fontsize/6*2)+fontsize)*str.length()+10;			
				if(strwidth2>=rWidth)
				{
		...
		{
			int width=(StringUtils.isNotEmpty(strData)?(int)(g.getFontMetrics().stringWidth(strData)*strData.getBytes().length/(strData.length()*1.4)):0);
			int strwidth=width;//getStrWidth(fontsize,rWidth,strData);
			int strwidth2=0;		
			if(strType!=null&& "D".equalsIgnoreCase(strType))
			  strwidth2=width;//getStrWidth(fontsize,rWidth,strData);		
			else
			  strwidth2=width;//((fontsize/6*2)+fontsize)*strData.length()+10;			
	...
	public int ReHeight(int fontsize,String fontname)
	{
        ...
        return  (int)Math.ceil(g.getFontMetrics().getHeight()*1.3); // 每一行字的高度
		/*if(gg!=null)
		{
			Graphics g = gg.createGraphics();   //获得画布
			Font font = new Font(fontname, Font.PLAIN, fontsize);
