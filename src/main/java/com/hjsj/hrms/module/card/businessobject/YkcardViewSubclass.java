...
public class YkcardViewSubclass {
	...
	//private String userpriv=""; fieldpurv 可替代 0 不校验权限 1 校验权限
				...
				if(field!=null&&field.length()>0)
				{
					 if(this.nFlag!=5)
					 {
						 ...
						 if("1".equals(fielditem.getUseflag())&&this.nFlag!=5)
						 {
							 ...
							 {
								 if((fieldpurv!=null&& "0".equals(fieldpurv))||!"0".equals(this.userview.analyseFieldPriv(field)))
								 {
									 this.fieldlist.add(field);
								 }
							 }else
			...
			if(field!=null&&field.length()>0)
			{
				 if(this.nFlag!=5)
				 {
					 ...
					 if("1".equals(fielditem.getUseflag())&&this.nFlag!=5)
					 {
						 if(this.userview!=null)
						 {
							 if((fieldpurv!=null&& "1".equals(fieldpurv))||!"0".equals(this.userview.analyseFieldPriv(field)))
							 {
								 this.fieldlist.add(field);
							 }
						 }else
...
//        	定位人员编号
    		...
    		int[] narrId=new int[1000];  // FIXME 记录超1000会有问题
    			try{
    				...
    				//查询出该人员的该子集的所有纪录
    				if(StringUtils.isNotEmpty(this.nId)) {	
	...
	/**************Pdf********************/
	 public PdfPTable viewSubClassPdf(String infokind,String userbase, Connection conn,UserView userview,RGridView rgrid,String disting_pt,byte nFlag,String platform)
	 {
		 ...
		 if(!"1".equals(isView))
		 {
			 if(nFlag!=5)
		     {
		    	 if(fieldpurv!=null&&"1".equalsIgnoreCase(fieldpurv))
			     {
			    	 ...
			    	 if(this.fenlei_type!=null&&this.fenlei_type.length()>0&&!userview.isSuper_admin()&&(nFlag==0))
		    	     {
		    	    	  ...
		    	    	  if(priv_flag==null||priv_flag.length()<=0|| "-1".equals(priv_flag))
		    	    		  return null;
		    	     }else if("0".equals(userview.analyseTablePriv(setname)))//招聘  子集导出pdf不需要权限   28708 changxy
				     {
				    		return null;
				      }
...
//				单位库
	    	...
	    	if(StringUtils.isNotEmpty(nId))
	    		subClassList=rowHeightNum(para_array,rgrid,disting_pt,sql);
	    	else
	    		subClassList=new ArrayList();
	    	html.append("<table id=sub_table_"+rgrid.getGridno()+" border=\"0\" cellspacing=\"0\" valign=\"top\" width=\""+(int)Float.parseFloat(rgrid.getRwidth())+"px\" height=\""+rgrid.getRheight()+"\"  cellpadding=\"0\">");
	    	MadeFontsizeToCell mc=new MadeFontsizeToCell();                              //创建的字体适应cell大小的对象
	       // mc.setAuto("1");
	    	newline=newline==0?1:newline;
	    	int rowHeight=0;
	    	rowHeight=(int)Float.parseFloat(rgrid.getRheight())/(subClassList.size()==0?1:subClassList.size());
    		fontsize=fontsize+1;
	    	for(int r=0;r<subClassList.size();r++) {//子集
	    		HashMap one_hash=(HashMap)subClassList.get(r);
	    		html.append("<tr id=\"sub_tr_"+rgrid.getGridno()+"_"+r+"\" >");
	    		for(int i=0;i<this.fieldlist.size();i++) {//子集指标列
	    			String field=this.fieldlist.get(i).toString();
	    			String class_str="";    		    		
					HashMap hash=(HashMap)one_hash.get(field);
					if(this.customcolhead!=null&& "true".equalsIgnoreCase(this.customcolhead))
		    		...
		    		if(colhead!=null&& "true".equals(colhead)&&r==0) {//设置标题显示下格线
		    			html.append("<td  id=\"sub_td_"+rgrid.getGridno()+"_"+r+"_"+i+"\" align=\""+align[0]+"\" style=\"display:table-cell;text-align:"+align[1]+";\" height=\""+(rowHeight)+"\" width=\""+widthn+"\" class=\""+class_str+"\" >");
		    		}else {
		    			html.append("<td  id=\"sub_td_"+rgrid.getGridno()+"_"+r+"_"+i+"\" align=\""+align[0]+"\" style=\"display:table-cell;text-align:"+align[1]+";\" height=\""+(rowHeight)+"\" width=\""+widthn+"\" class=\""+class_str+"\" >");
		    		}
	    			if(ykcard_auto) {
	    				html.append("<div id=\"sub_div_"+rgrid.getGridno()+"_"+r+"_"+i+"\" style=\"overflow:hidden;padding-top:1px;max-height:"+(rowHeight)+"px;"+setFontMarginPx(align[0])+"\" width=\""+widthn+"px\" \">");
	    			}
		    		if(colhead!=null&& "true".equals(colhead)&&r==0)
			    		html.append("<font id=\"sub_font_"+rgrid.getGridno()+"_"+r+"_"+i+"\"  style=\"font-weight:" + fontweight + ";font-family:"+rgrid.getFontName()+";font-size:" + (this.fontsize) + "pt\"  color=\"#000000\">");
			    	else
			    		html.append("<font id=\"sub_font_"+rgrid.getGridno()+"_"+r+"_"+i+"\" style=\"font-weight:" + fontweight + ";font-family:"+rgrid.getFontName()+"; font-size:" + (this.fontsize) + "pt\"  color=\"#15428b\">");
		    		...
		    		if(i==this.fieldlist.size()-1)//模板设置子集附件添加附件列 
		    		{
						 if (this.domain.getMultimedia()!=null&& "true".equals(this.domain.getMultimedia())) {
							boolean flag = false;
							int I9999 = 0; 
							//判断在集合里面是否有I9999这个键
							...
							if(strtype==null|| "".equals(strtype)||userbase==null|| "".equals(userbase)||setname==null|| "".equals(setname)){
								...
								if(I9999 > 0) {
									MultiMediaBo mmBo= new MultiMediaBo(conn,userview,dbflag,userbase,setname,nId,I9999);
									//查询是否有记录
				    		...
				    		/*if(colhead!=null&&colhead.equals("true")&&r==0) {
				    			html.append("<td  id=\"sub_td_"+rgrid.getGridno()+"_"+r+"_"+(i+1)+"\" align=\""+align[0]+"\" style=\"border-bottom:1px solid ;;display:table-cell;text-align:"+align[1]+";\" height=\""+rowHeight+"\" width=\""+widthn+"\" class=\""+class_str+"\" >");
				    		}else {
				    		}*/
				    		html.append("<td  id=\"sub_td_"+rgrid.getGridno()+"_"+r+"_"+(i+1)+"\" align=\""+align[0]+"\" style=\"display:table-cell;text-align:"+align[1]+";\" height=\""+rowHeight+"\" width=\""+widthn+"\" class=\""+class_str+"\" >");
			    			if(flag==true) {
			    			...
			    			if(ykcard_auto) {
			    				html.append("<div id=\"sub_div_"+rgrid.getGridno()+"_"+r+"_"+(i+1)+"\" style=\"overflow:hidden;padding-top:1px;;max-height:"+(rowHeight)+"px;"+setFontMarginPx(align[0])+"\" width=\""+widthn+"px\" \">");
			    			}

			    			if(colhead!=null&& "true".equals(colhead)&&r==0){
					    		html.append("<font  id=\"sub_font_"+rgrid.getGridno()+"_"+r+"_"+(i+1)+"\" style=\"font-weight:" + fontweight + ";font-family:"+rgrid.getFontName()+"; font-size:" + (this.fontsize) + "pt\"  color=\"#000000\">");
					    		html.append("附件");
				    		}else{
				    			html.append("<font id=\"sub_font_"+rgrid.getGridno()+"_"+r+"_"+(i+1)+"\" style=\"font-weight:" + fontweight + ";font-family:"+rgrid.getFontName()+";font-size:" + (this.fontsize) + "pt\"  color=\"#15428b\">");
				    			if(flag==true) {
				    				html.append("<a href=\"javascript:\" onClick=\"window.open('/general/inform/multimedia/multimedia_tree.do?b_query=link&setid="+setname+"&a0100="+nId+"&nbase="+userbase+"&i9999="+I9999+"&dbflag=A&canedit=false','','left:400px;top:200px;width:800px; height:500px; toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')\">");
						    		html.append(title);
						    		html.append("</a>");
				    			}else {
				    				html.append("<font  id=\"sub_font_"+rgrid.getGridno()+"_"+r+"_"+(i+1)+"\" style=\"font-weight:" + fontweight + ";"+setFontMarginPx(align[0])+"font-family:"+rgrid.getFontName()+"; font-size:" + (this.fontsize) + "pt\"  color=\"#15428b\">");
...
//				单位库
	 ...
	 public ArrayList outWordListMap(String infokind,String userbase, Connection conn,UserView userview,RGridView rgrid,String disting_pt,byte nFlag) throws Exception{
			...
			if(!"1".equals(isView))
			{
				if(fieldpurv!=null&&"1".equals(fieldpurv)) {
					if(this.fenlei_type!=null&&this.fenlei_type.length()>0&&!userview.isSuper_admin()&&(nFlag==0))
		    	     {
		    	    	  String priv_flag= userview.analyseSubTablePriv(this.fenlei_type,setname)+"";    	  
		    	    	  if(priv_flag==null||priv_flag.length()<=0|| "-1".equals(priv_flag))
		    	...
		    	{
		    		if(!"1".equals(fieldpurv)&&userview.analyseTablePriv(setname,0).equals("0"))
			    	{
		    			return new ArrayList();
			    	}
		    	}else
		    	{
		    		if(this.fenlei_type!=null&&this.fenlei_type.length()>0&&!userview.isSuper_admin()&&(nFlag==0))
		    	     {
		    	    	  String priv_flag= userview.analyseSubTablePriv(this.fenlei_type,setname)+"";    	  
		    	    	  if(priv_flag==null||priv_flag.length()<=0||priv_flag.equals("-1"))
		    	    		  return new ArrayList();
		    	     }else if(userview.analyseTablePriv(setname).equals("0"))
			    	{
			    		return new ArrayList();
			    	}
		    	}*/
...
//				单位库
    ...
    //查询人员库信息
    public ArrayList<HashMap<String, String>> getdblist(UserView userView,Connection conn,String userBase){
    	ContentDAO dao=new ContentDAO(conn);
    	RowSet rs=null;
    	...
    	try {
    		...
    		while(rs.next()) {
    			...
    			map.put("id", rs.getString("pre"));
    			map.put("dbname", rs.getString("dbname"));
    			list.add(map);
    			if(StringUtils.isNotEmpty(userBase)&& rs.getString("pre").equalsIgnoreCase(userBase)) {
    				break;
    			}
    		}
