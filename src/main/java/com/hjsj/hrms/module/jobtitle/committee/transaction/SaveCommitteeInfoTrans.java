 ...
 *
 */
public class SaveCommitteeInfoTrans extends IBusiness {
	...
	public void execute() throws GeneralException {
		try
		{
			...
			String needToSave = ",committee_id_e,w0101_e,start_date,end_date,flag,fixed_member,";//只需要保存
			sql.append("update zc_judgingpanel_experts set ");
			for(int i=0;i<datalist.size();i++){
				...
				java.sql.Date endDate = null;//haosl 20160826
				while (iter.hasNext()) {
					Map.Entry entry = (Map.Entry) iter.next();
					Object key = entry.getKey();
					Object val = entry.getValue();
					if(needToSave.indexOf(","+key.toString().toLowerCase()+",") != -1){//只需要保存
						if("committee_id_e".equalsIgnoreCase(key.toString())) {//评委会编号
							where1.append(" committee_id");
							where1.append("=?");
							...
							//haosl 20160826 保存前对开始和结束日期进行校验
							if("start_date".equalsIgnoreCase(key.toString())){
								...
								list.add(endDate);
							}else if("flag".equalsIgnoreCase(key.toString()) || "fixed_member".equalsIgnoreCase(key.toString())){
								//haosl 20160920 修改保存固定成员列什么都不选时报错
				...
				//如果更新w01表，在评委会中的字段是flag，前台查的时候给的字段不是w0109，而是flag，这里仅涉及保存,改一下影响不大
...
//					resultVo.setString("b0110", this.userView.getUnitIdByBusi("9"));
	...
	private void addOutExpert(ContentDAO dao, ArrayList<HashMap> outExpert) throws GeneralException {
		try {
			...
			String committee_id = "";//评审会议id

			for (int i = 0; i < outExpert.size(); i++) {
				HashMap addmap = outExpert.get(i);

				if (StringUtils.isBlank(committee_id))
					committee_id = (String) addmap.get("committee_id_e");
			...
			committeeBo.createCommitteePerson(PubFunc.decrypt(committee_id), personidList);
