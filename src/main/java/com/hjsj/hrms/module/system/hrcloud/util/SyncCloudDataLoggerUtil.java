 ...
 *
 */
public class SyncCloudDataLoggerUtil {
	...
	static{
		...
		try{
			...
			getPath:if(filePath!=null && !"".equals(filePath)){
				...
				//默认日志文件名为cloudsyncdata+date.log
				SimpleDateFormat sdf = new SimpleDateFormat("yyMMdd");
				String date = sdf.format(new Date());
				filePath = filePath.substring(0, filePath.lastIndexOf(sysFileSep)+1)+"cloudsyncdata"+date+".log";
			}
	 ...
	 * @param obj 预处理sql参数
	 */
	public static void start(String info,JSONArray array){
		...
		try {
			...
			if(array!=null)
				info+=";Array="+array.toString();
			sqlKeeper.error(info);
	...
	private static Logger getSqlLogger(String path) throws IOException{
		//动态创建 Logger
		if(sql_logger==null){
			sql_logger = RollLoggerUtil.createLogger("cloudkeeper");
		...
		try {
			...
			if (!file.isDirectory()) {
				...
				for (int i = 0; i < filelist.length; i++) {
					...
					if (readfile.isFile()) {
						if(filelist[i].indexOf("cloudsyncdata")>-1){
