...
public class ValidateOrgTrans extends IBusiness {
    ...
    public void execute() throws GeneralException {
		...
		table_name = PubFunc.decrypt(table_name);
		String tabid = (String)this.getFormHM().get("tabid");
		String d="",maxstartdate="",msg=""; 
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		TemplateUtilBo utilBo = new TemplateUtilBo(this.frameconn,this.userView);
		TemplateParam paramBo=new TemplateParam(this.frameconn,this.userView,Integer.valueOf(tabid));
		String operationType = paramBo.getOperationType() + "";
		try{
			...
			ArrayList<TemplateSet> templateSetList = utilBo.getAllCell(Integer.valueOf(tabid));
			boolean flag1=false;
			boolean flag2=false;
			boolean flag3=false;
			for(int j=0;j<templateSetList.size();j++){
				TemplateSet templateSet = templateSetList.get(j);
				if(templateSet!=null&& "start_date_2".equalsIgnoreCase(templateSet.getField_name().toString().trim()+"_"+templateSet.getChgstate()))
					flag1=true;
				if(templateSet!=null&& "codeitemdesc_2".equalsIgnoreCase(templateSet.getField_name().toString().trim()+"_"+templateSet.getChgstate()))
					flag2=true;
				if(templateSet!=null&& "parentid_2".equalsIgnoreCase(templateSet.getField_name().toString().trim()+"_"+templateSet.getChgstate()))
					flag3=true;
			}
		...
		//	throw GeneralExceptionHandler.Handle(e);
			...
			this.getFormHM().put("operationType", operationType);
