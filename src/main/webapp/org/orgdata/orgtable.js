function viewRecord(viewdata,infor,a_code){
   ...
   orgDataForm.action="/org/orgdata/orgdata.do?b_rmain=link&infor="+infor+"&a_code="+a_code;
   orgDataForm.submit();  	
}
...
function batchHand(flag,a_code,infor){
	var thecodeurl =""; 
	var return_vo;	
	...
	switch(flag){ 
         ...
         case 2 : 
         	  var strId = selectValueStr(setname,infor);
              thecodeurl="/general/inform/emp/batch/alertmoreind.do?b_query=link&viewsearch=0&setname="+setname+"&a_code="+a_code+"&infor="+infor+"&strid="+strId+"&path=4";
...
{  
    ...
    var iframe_url="/general/query/common/iframe_query.jsp?src="+theurl;
   	var return_vo= window.showModalDialog(iframe_url, 'orgedit_win', 
      				"dialogWidth:900px; dialogHeight:800px;resizable:no;center:yes;scroll:no;status:no");	
 	parent.ril_body2.location="/org/orgdata/orgdata.do?b_menu=link&itemid="+itemVal+"&infor="+infor;
  	if(!return_vo)
	    return false;	   
