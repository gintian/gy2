...
<%@ page import="java.util.*,
				...
				com.hjsj.hrms.actionform.org.OrgInformationForm"%>
...
<script language="javascript">  
   function openOrgTreeDialog()
   {
        ...
        var hiddenobj=orgInformationForm.tarorgid;
	...
	function showPersonList(outparamters)
	{
		...
		AjaxBind.bind(orgInformationForm.left_fields,personlist);
	}			
	/**查询人员*/
	function searchPersonList()
	{
	   //var orgid=$F('searchbolishorglist');
	   //alert(orgInformationForm.bolishorgname.value);
	   ...
	   var in_paramters="orgid="+orgInformationForm.bolishorgname.value + "&dbpre=" + orgInformationForm.dbpre.value+"&movedpersonsstr="+movedpersonsstr;
	...
	function showTarOrgList(outparamters)
	{
	   ...
	   AjaxBind.bind(orgInformationForm.right_fields,tarorglist);
	}	
	/**查询目标机构*/
	function changepos()
	{
	   var in_paramters="orgid="+orgInformationForm.tarorgid.value;
	...
	function movepersonsubmit()
	{
	  if(confirm("<bean:message key="label.org.moveperson"/>?"))
          {
             orgInformationForm.action="/org/orginfo/searchorglist.do?b_moveperson=link";
             orgInformationForm.submit();
          }
        }
        function refreshperson()
	{
	    orgInformationForm.action="/org/orginfo/searchorglist.do?br_bolish=link";
            orgInformationForm.submit();
        }
    ...
    function volidatestart(){
        ...
        function exebolishsubmit()
        {
	        ...
	        <%}%>
	        //ie非兼容模式浏览器兼容：   撤销机构下没有人是document.getElementById("left_fields").options 方法报错  xus 19/3/5
	        if(!document.getElementById("left_fields"))
	        	var personlist = "";
	        else
	        	var personlist = document.getElementById("left_fields").options;
     	   var psonmsgflag = "<bean:write name="orgInformationForm" property="ishavepersonmessage"/>";
           if("<bean:write name="orgInformationForm" property="ishavepersonmessage"/>"=="")
           {
              if(personlist.length>0){
               ...
               orgInformationForm.action="/org/orginfo/searchorglist.do?b_bolishs=link&emptypersonorg=1";
               orgInformationForm.submit();
               <%
           ...
           else
           {
             ...
             orgInformationForm.action="/org/orginfo/searchorglist.do?b_bolishs=link";
             orgInformationForm.submit();
             <%
				for(int y=0;y<bolishlist.size();y++){
					CommonData data = (CommonData)bolishlist.get(y);
...
<table width="90%" border="0" cellspacing="0"  align="center" cellpadding="0">
		...
		<bean:message key="label.org.bolishorg"/>&nbsp;&nbsp;
   	  ...
   	  <tr>
   	   <td align="center"  width="100%"  colspan="3" colspan="3" <logic:notEmpty name="orgInformationForm" property="ishavepersonmessage">class="RecordRowHr"</logic:notEmpty>>
   	      <bean:write name="orgInformationForm" property="ishavepersonmessage"/>                                
           </td>                    
   	  </tr>
   	  <tr>
   	   <td align="left"  width="100%"  class="RecordRowHr" nowrap colspan="3">
   	      <html:select name="orgInformationForm" property="dbpre" size="1" onchange="searchPersonList();">   
   	   ...
   	   <tr>
            <td width="100%" align="top" class="" nowrap colspan="3"  class="RecordRowHr">
                   ...
                   <table align="center" width="100%">
                    ...
                    <tr>
                      <td align="left"  nowrap>
                        <html:select name="orgInformationForm" property="bolishorgname" size="1" onchange="searchPersonList();">   
              ...
              %>
           &nbsp;&nbsp;<bean:message key="conlumn.codeitemid.end_date"/>&nbsp;<input type="text" name="end_date" value="<%=date %>" maxlength="50" class="textColorWrite" style="BACKGROUND-COLOR:#F8F8F8;width:100px" extra="editor" dropDown="dropDownDate" onchange="if(!validate(this,'有效日期止')) {this.focus(); this.value='<%=date %>'; }"/>                                            
           <%} %>                                       
                        </td>  
                    </tr>
                   <tr>
                       <td align="center">
                        <html:select name="orgInformationForm" property="left_fields" size="10"  style="height:207px;width:100%;font-size:9pt">
                        </html:select>
                       </td>
                    </tr>
                   </table>
                ...
                <td width="42%" align="center">
                 <table width="100%">
                  ...
                  <tr>
                   <td width="100%" align="left">
                      <html:hidden name="orgInformationForm" property="tarorgid"/> 
                     <html:text name="orgInformationForm" property="tarorgname" readonly="false"  styleClass="textColorWrite" onchange="changepos()"/> 
                     <img align="middle" src="/images/code.gif" style="cursor:pointer" onclick='javascript:openOrgTreeDialog();'/>&nbsp;
                   </td>
                  </tr>
                  <tr>
                  <td width="100%" align="left">
		 		     <html:select name="orgInformationForm" property="right_fields"  size="10"  style="height:209px;width:100%;font-size:9pt">
 ...
 %>
									...
									<logic:equal name="element" property="itemtype" value="D">
									    ...
									    <input type="text" name='<%="fieldlist[" + index
											...
											dataType="simpledate"
											onchange=" if(!validate(this,'${element.itemdesc}')) {this.focus(); this.value='<%=date3 %>'; }">
										<%
											if (isFillable1) {
										%> &nbsp;<font color='red'>*</font>&nbsp;<%
 	}
 %>
								...
								<logic:notEqual name="element" property="codesetid" value="0">
									<logic:equal name="element" property="itemid" value="b0110">
										...
										<img  align="middle"  src="/images/code.gif"
