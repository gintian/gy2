...
function fileQueueError(file, errorCode, message) {
	try {
		...
		tr.style.color="red";
		var bar = document.getElementById(file.id+"_bar_border_parent");
		var errInfo = "&nbsp;选择失败";
		/* comment by stephen
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setError();
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	try {
		...
		var tr = document.getElementById(file.id);
		//tr.style.color="blue";
		var wait = document.getElementById(file.id+"_wait_upload");
		wait.style.display = "none";
		var bar_border = document.getElementById(file.id+"_bar_border");
		bar_border.style.display="";
		var bar_pens = document.getElementById(file.id+"_bar_pens");
		bar_pens.innerText = percent + "%";
		/*var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setProgress(percent);
		progress.setStatus("完成"+percent + "%");*/
	} catch (ex) {
		this.debug(ex);
	}
	/*
...
function uploadSuccess(file, serverData) {
	...
	try {
		if(isSuccess){
			...
			var ids = logicFileName.split(":");
			document.getElementById(file.id + "_hidden_value").value = ids[1];
			document.getElementById(file.id + "_hidden_name").value = file.name;
			document.getElementById(file.id + "_checkbox").disabled = false;
