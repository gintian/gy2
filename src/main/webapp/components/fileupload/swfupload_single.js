...
SWFUpload.prototype.initSWFUpload = function (userSettings) {
	try {
		...
		this.callBackScop = userSettings.callBackScop;
...
SWFUpload.prototype.queueEvent = function (handlerName, argumentArray) {
	...
	argumentArray.push(this.callBackScop);
...
SWFUpload.prototype.cleanUp = function () {
	...
	try {                                                                      //IE11 锟角硷拷锟斤拷模式锟斤拷锟斤拷锟斤拷  changxy
		if (movieElement && typeof(movieElement.CallFunction) === "unknown"&& (userAgent.indexOf("compatible") > -1 && userAgent.indexOf("MSIE") > -1 && !isOpera)) { // We only want to do this in IE
			this.debug("Removing Flash functions hooks (this should only run in IE and should prevent memory leaks)");
			for (var key in movieElement) {
				try {
					if (typeof(movieElement[key]) === "function"&&movieElement[key]>='A'&&movieElement[key]<='Z') {
...
SWFUpload.prototype.returnUploadStart = function (file) {
	...
	if (typeof this.settings.upload_start_handler === "function") {
		file = this.unescapeFilePostParams(file);
		returnValue = this.settings.upload_start_handler.call(this, file,this.callBackScop);
...
SWFUpload.prototype.cancelQueue = function (obj) {
	...
	while (stats.files_queued > 0) {
		fileId=this.getFile().id;//取锟斤拷锟较达拷锟斤拷锟斤拷锟叫碉拷一锟斤拷File锟斤拷ID add by stephen
...
/**
	 ...
	 * @param {} forbidden
	 */
SWFUpload.prototype.forbiddenButton=function(flag,forbidden) {
		if (forbidden) {

			for (i = 0; i < forbidden.length; i++) {
				if (typeof(forbidden[i]) == "string") {
					var obj = document.getElementById(buttons[i]);
					if (obj) {
						obj.disabled = flag;
					}
				} else {
					forbidden[i].disabled = flag;
				}
			}
		}
	};
