Ext.define("SYSF.SWFUpLoad",{
	//别名，方便引用
	xtype:"swfupload",
	
	// 回调函数scope
	callBackScop:undefined,
	...
	constructor:function(settings){
		...
		this.callBackScop = settings.callBackScop;
		this.swfobject = settings.swfobject;
	},
	...
	initSWFUpload :  function (userSettings) {
		try {
			...
			this.settings = {};
			this.eventQueue = [];
			this.movieName = "SWFUpload_" + this.movieCount++;
			this.movieElement = null;
	
	
			// Setup global control tracking
			this.instances[this.movieName] = this;
			...
			delete this.instances[this.movieName];
			console.log(ex);
			throw ex;
		}
	},
	...
	initSettings :  function (userSettings) {
		...
		this.ensureDefault =  function (settingName, defaultValue) {
			...
			if (setting != undefined) {
				me.settings[settingName] = setting;
			} else {
				me.settings[settingName] = defaultValue;
			}
		};
		...
		this.ensureDefault("button_action", this.BUTTON_ACTION.SELECT_FILE);
		this.ensureDefault("button_disabled", false);
		this.ensureDefault("button_placeholder_id", "");
		this.ensureDefault("button_placeholder", null);
		this.ensureDefault("button_cursor", this.CURSOR.ARROW);
		this.ensureDefault("button_window_mode", this.WINDOW_MODE.TRANSPARENT);
		...
		if (!this.settings.preserve_relative_urls) {
			...
			this.settings.button_image_url = this.completeURL(this.settings.button_image_url);
		}
	...
	getFlashHTML :  function (flashVersion) {
		...
		return ['<object id="', this.movieName, '" type="application/x-shockwave-flash" data="', (this.support.imageResize ? this.settings.flash_url : this.settings.flash9_url), '" width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="mybutton" style="margin-bottom:-4px;">',
	...
	destroy :  function () {
		...
		try {
			...
			this.instances[this.movieName] = null;
			delete this.instances[this.movieName];
	...
	******************************* */
	...
	queueEvent : function (handlerName, argumentArray) {
		...
		if (typeof this.settings[handlerName] === "function") {
			...
			this.eventQueue.push(function () {
				self.settings[handlerName].apply(this, argumentArray);
			});
	...
	swfuploadPreload :  function () {
		...
		if (typeof this.settings.swfupload_preload_handler === "function") {
			if (this.callBackScop) {
				returnValue = Ext.callback(this.settings.swfupload_preload_handler, this.callBackScop,[this])
			} else {
				returnValue = Ext.callback(this.settings.swfupload_preload_handler,undefined,[this])
			}
	...
	debugMessage :  function (message) {
		...
		if (me.settings.debug) {
			...
			if (typeof message === "object" && typeof message.name === "string" && typeof message.message === "string") {
				...
				this.Console.writeLine(exceptionMessage);
			} else {
