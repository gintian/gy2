...
function change() {
	...
	var request = new Request({
				...
				onSuccess : showFieldList,
				functionId : '3020110073'
			});
}
...
function searchSetCond(a_code, tablename, type) {
	...
	for (var i = 0; i < this.logicArr.length; i++) {
		...
		var itemtype = this.typeArr[i];
		var setid = this.codeArr[i];
		condstr += this.logicArr[i] + (i + 1);
		var itemvalue = document.getElementById(i + "_" + itemid + ".value").value;
		condlogic += itemid + ':' + itemtype + ':' + setid + ':' + this.eqArr[i] + ':' + itemvalue + "`";
	}
		...
		if (left_vo.options[i].selected) {
			...
			if (itemid != null && itemid.length > 0) {
				...
				if (arr.length == 4) {
					...
					tabelstr += addTable(arr[0], desc, arr[1].toUpperCase(),
							arr[2]);
				}
			}
		}
...
function addTable(itemid, desc, type, codesetid) {
	...
	if (n > 0) {
		...
		tabelstr += n + ",this);\"><option value=\"*\">" + GENERAL_AND
				+ "</option>";
	...
	if(itemid == "q1104"){
		var classid = document.getElementById("div_class_id");
		tabelstr += "<select name=\"" + id + ".value\" style=\"width:100%;\">";
		tabelstr += classid.innerHTML;
		tabelstr += "</select>";
		...
		tabelstr += "<input type=\"text\" name=\"" + id + ".value\"";
		tabelstr += " onclick=\"showDateSelectBox(this);\" ";
		tabelstr += " onblur=\"closeSelectBox();\" ";
		tabelstr += " style=\"width:120px;\"";
		tabelstr += ">";
	} else if (type == 'A') {
		if (codesetid == "0") {
			tabelstr += "<input type=\"text\" name=\"" + id
					+ ".value\" style=\"width:120px\">";
		} else if("UM,@K".indexOf(codesetid) != -1){// 按照单位或部门进行查询时组织机构树的显示请按权限过滤
			tabelstr += "<input type=\"text\" onchange=\"hzvalueTovalue('" + id
					+ "');\" name=\"" + id
					+ ".hzvalue\" style=\"width:120px\">";
			...
			tabelstr += codesetid + "','" + id + ".hzvalue','',1);\">";
		} else {
			tabelstr += "<input type=\"text\" onchange=\"hzvalueTovalue('" + id
					+ "');\" name=\"" + id
					+ ".hzvalue\" style=\"width:120px\">";
...
/**********考勤模块增加变更,按单位或部门显示时,增加权限(orgparentcode)过滤**********/
function additemtr(sourcebox_id,orgparentcode) {
	var left_vo, vos, i;
	vos = document.getElementsByName(sourcebox_id);
	if (vos == null)
		return false;
...
function addTable(itemid, desc, type, codesetid,orgparentcode) {
	var n = idArr.length - 1;
	var id = n + "_" + itemid;
	var tabelstr = "<tr onclick=\"onSelects('" + id + "');\">";
	tabelstr += "<td class=\"RecordRow\" align=\"center\" height=\"30\">";
	...
	if(itemid == "q1104"){
		...
		tabelstr += "<input type=\"text\" name=\"" + id + ".value\"";
		tabelstr += " onkeypress=\"event.returnValue=IsDigit();\" class=\"text4\" style=\"width:120px\">";
	} else if (type == 'D') {
		tabelstr += "<input type=\"text\" class=\"text4\" name=\"" + id + ".value\"";
		tabelstr += " onclick=\"showDateSelectBox(this);\" ";
		tabelstr += " onblur=\"closeSelectBox();\" ";
		tabelstr += " style=\"width:120px;\"";
		tabelstr += ">";
		...
		if (codesetid == "0") {
			tabelstr += "<input type=\"text\" name=\"" + id
					+ ".value\" class=\"text4\" style=\"width:120px\">";
		} else if("UN,UM,@K".indexOf(codesetid) != -1){// 按照单位或部门进行查询时组织机构树的显示请按权限过滤
			tabelstr += "<input type=\"text\" class=\"text4\" onchange=\"hzvalueTovalue('" + id
					+ "');\" name=\"" + id
					+ ".hzvalue\" style=\"width:120px\">";
			tabelstr += "<input type=\"hidden\" name=\"" + id + ".value\">&nbsp;";
			tabelstr += "<img  src=\"/images/code.gif\" align=\"absmiddle\" onclick=\"javascript:openInputCodeDialogOrgInputPos('";
			tabelstr += codesetid + "','" + id + ".hzvalue','"+orgparentcode+"',1);\">";
		} else {
			tabelstr += "<input type=\"text\" class=\"text4\" onchange=\"hzvalueTovalue('" + id
					+ "');\" name=\"" + id
					+ ".hzvalue\" style=\"width:120px\">";
...
function hzvalueTovalue(id) {
	...
	//if (values.indexOf("*") != -1 || values.indexOf("?") != -1 || values.indexOf("%") != -1 || values.indexOf("_") != -1) {
...
function delTableStr() {
	...
	if (this.idArr.length > 0 && n == 0) {
		...
		tabelstr = tabelstr.substring(tabelstr.indexOf("</SELECT>") + 9,
				tabelstr.length);
...
function checkMaintItem() {
	...
	for (var i = 0; i < this.fieldSetArr.length; i++) {
		if (this.fieldSetArr[i] != null && this.fieldSetArr[i] != undefined
				&& this.fieldSetArr[i].length > 0) {
