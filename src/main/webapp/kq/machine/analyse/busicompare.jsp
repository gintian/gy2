...
<script language="javascript">
   function change()
   {
    ...
    //判断日期
      ...
      dataAnalyseForm.action="/kq/machine/analyse/busicompare.do?b_search=link&select_flag=1";
   ...
   {	     
    ...
    function addQ()
    {
       ...
       if(isC)
       {
          var analyse_flag=$F('busi_analyse_flag');
          if(analyse_flag=="")
          {
             alert("请选择处理方式！");
             return false;
          ...
          { 
            ...
            if(analyse_flag=="1")
            {
               mess="您确定要处理选定的数据吗？\r\n\r\n说明：对于短时申请（申请时长小于8小时），该处理将自动按照申请的起始时间补充刷卡；";
               mess=mess+"\r\n      对于长期申请（申请时长大于等于8小时），将自动清除申请时段内的刷卡记录；"
               mess=mess+"\r\n      特别地，对于加班申请,无论申请时长为多少，处理方法均与短申请相同。"
            }else if(analyse_flag=="2")
            {
                mess="您确定要处理选定的数据吗？\r\n\r\n说明:该处理将用刷卡时间来自动修改申请的起始时间。";
                mess=mess+"\r\n     无刷卡或刷卡不完整的数据无法处理,请手工修改申请记录的刷卡数据。";
            }
            if(mess!="")
            {
               if(confirm(mess))
               {
                 ...
                 dataAnalyseForm.action="/kq/machine/analyse/busicompare.do?b_analyse=link&analyse_flag="+analyse_flag;
                 dataAnalyseForm.submit();
               }
            }            
          } 
 ...
 <% int i=0;
    String name="";
 %>
<html:form action="/kq/machine/analyse/busicompare">
<table width="100%" border="0" cellspacing="0"  align="left" cellpadding="0" style="margin-top:5px;">
       ...
       <tr>        
          ...
          <td align="left" width="170" nowrap> 
           <table border="0"  cellspacing="0"  align="left" valign="bottom" cellpadding="0">
             <tr>
		   <td>		   
		   <input type="text" name="start_date" extra="editor" dataType="simpledate" value="${dataAnalyseForm.start_date}" class="inputtext" style="width:100px;font-size:10pt;text-align:left" id="editor1" onclick='saveCurrDateValue(this);getKqCalendarVar();popUpCalendar(this,this,weeks,feasts,turn_dates,week_dates,false);' onchange="rep_dateValue(this);restoreDateValue(this,kq_duration)">
		   </td>
      ...
      <table width="100%" border="0" cellspacing="0"  align="left" cellpadding="0">
       <tr>   
          ...
          <td nowrap width="100" align="left">    
           <html:select name="dataAnalyseForm" property="app_type" size="1" onchange="change();">
                <html:optionsCollection property="applist" value="dataValue" label="dataName"/>	        
           </html:select>     
          </td>          
          <td align="left" nowrap> 
            <table border="0"  cellspacing="0"  align="left" valign="middle" cellpadding="0">
             <tr>
		       <td >		 
		         <html:radio name="dataAnalyseForm" property="busi_filtrate" value="0" onclick="change()"/> 
		       全部   &nbsp; 
		       </td>
		       <td >		 
		         <html:radio name="dataAnalyseForm" property="busi_filtrate" value="1" onclick="change()"/>   
		       申请时长大于实际时长  &nbsp; 
		       </td>
		       <td >		 
		         <html:radio name="dataAnalyseForm" property="busi_filtrate" value="2" onclick="change()"/>
		        申请时长小于实际时长 
		       </td>
		       <td width="100">		 
		         &nbsp;&nbsp;
           ...
           %>
             <logic:equal name="info" property="visible" value="true">
                 ...
                 <logic:equal name="info" property="itemtype" value="D">
                       <td align="left" class="RecordRow" nowrap>
                           <bean:write name="element" property="${info.itemid}" format="yyyy.MM.dd HH:mm" filter="false"/>&nbsp;  
                       </td>
                    </logic:equal>  
                 <logic:equal name="info" property="itemtype" value="N">
                   <td align="right" class="RecordRow" nowrap> 
                   <%                  
                     String value=(String)abean.get(name);                     
                   ...
                   %>
                   <bean:write format="#.##" name="element" property="${info.itemid}" />&nbsp;
                      <%
                      }
                      %>
                    </td> 
 ...
 <tr>
   <td width="360"  valign="bottom"  class="tdFontcolor" nowrap> 
             <table  width="100%" border="0" cellspacing="0"  align="left" cellpadding="0">   
              <tr>
                <td>
                    处理方式
                ...
                <td>
                   <html:radio name="dataAnalyseForm" property="busi_analyse_flag" value="1"/>
                </td>
                <td>
                 以申请为准
                </td>
                <td>
                   <html:radio name="dataAnalyseForm" property="busi_analyse_flag" value="2"/> 
                </td>
                 <td>
                 以刷卡为准
                </td>
                <td>
                    <input type="button" name="br_return" value='确定' class="mybutton" onclick="addQ();"> 
