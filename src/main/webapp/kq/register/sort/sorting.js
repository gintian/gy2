...
function outTable(sortitem){
	var tabelstr = "<table width=\"100%\" border=\"0\" class=\"ListTable1\" style=\"border-right:0px;\">";
	tabelstr += "<tr class=\"fixedHeaderTr1\">";
	tabelstr += "<td class=\"TableRow\" style=\"border-left:0px;\" width=\"10%\" align=\"left\">&nbsp;</td>";
	tabelstr += "<td class=\"TableRow\" width=\"65%\" align=\"center\">指标名称</td>";
	tabelstr += "<td class=\"TableRow\" style=\"border-right:0px;\" width=\"25%\" align=\"center\">升降<td>";
	tabelstr += "</tr>";
	var arr = sortitem.split("`");
	if(arr.length>0){
		var n=1;
		for(var i=0;i<arr.length;i++){
			...
			if(arr_item.length==3){
				...
				tabelstr+="<td align=\"center\" style=\"border-right:0px;\" class=\"RecordRow\" onclick=\"tr_bgcolor('";
				tabelstr+=arr_item[0]+"')\" ondblclick=\"deletefield();\" nowrap>";
...
function changeItem(outparamters){
	...
	AjaxBind.bind(dailyRegisterForm.itemid,itemlist);
...
function defOrder(){
		...
		var hashVo = new ParameterSet();
		hashVo.setValue("sortitem",sortitem);
		var request=new Request({method:'post',asynchronous:false,onSuccess:defOrderSuccess,functionId:'15301110112'},hashVo);
