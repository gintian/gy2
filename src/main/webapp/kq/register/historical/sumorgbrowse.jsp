...
<html:form action="/kq/register/historical/sumorgbrowsedata">
<script language="javascript">
   //验证是否报批成功 
   ...
   function couser()
   {
      browseHistoricalForm.action="/kq/register/historical/sumorgbrowsedata.do?b_couser=link&code=${browseHistoricalForm.code}&kind=${browseHistoricalForm.kind}";
   ...
   {
       //browseHistoricalForm.action="/general/muster/hmuster/select_muster_name.do?b_query=link&nFlag=81&kqtable=Q09_arc&relatTableid=${browseHistoricalForm.relatTableid}";
      // browseHistoricalForm.submit();
      document.mysearchform.submit();
      var returnURL = getEncodeStr("/kq/register/historical/sumorgbrowsedata.do?b_search=link");
      var urlstr = "/general/muster/hmuster/searchHroster.do?b_search=link&nFlag=81&kqtable=Q09_arc&relatTableid=${browseHistoricalForm.relatTableid}&closeWindow=1";
...
{
	if(dd==2){
 		browseHistoricalForm.action="/kq/register/historical/sumorgbrowsedata.do?b_search=link&selectys=2&code=${browseHistoricalForm.code}&kind=${browseHistoricalForm.kind}";
    	browseHistoricalForm.submit();
 	}else if(dd==1){
 		browseHistoricalForm.action="/kq/register/historical/sumorgbrowsedata.do?b_search=link&selectys=1";
    	browseHistoricalForm.submit();
 	}
}
    function sum_historyt(obj)
   {
   	if(obj.value=="0"){
        ...
        browseHistoricalForm.action="/kq/register/history/sumorgbrowse.do?b_search=link&action=sumorgbrowsedata.do&target=mil_body&a_inforkind=1&privtype=kq";
        browseHistoricalForm.target="il_body";
        browseHistoricalForm.submit();
     }
   }
   ...
   <table width="60%" border="0" cellspacing="0"  align="left" cellpadding="0">
    <tr> 
	  ...
	  <bean:message key="kq.register.daily.menu"/><html:select name="browseHistoricalForm" property="year" size="0" onchange="javascript:couser();">
        ...
        <html:hidden name="browseHistoricalForm" property="condition" styleClass="text"/>        </td>
       <td>
                    ...
                    <logic:equal name="info" property="itemtype" value="A">
                       <logic:notEqual name="info" property="codesetid" value="0">
                            <td align="left" class="RecordRow" style="border-left:none;" nowrap> 
                               ...
                               {
                               %>
                               <a href="/kq/register/historical/orgyearsingle.do?b_browse=link&&year=${browseHistoricalForm.year}&duration=${browseHistoricalForm.duration}&b0110=<bean:write name="element" property="b0110" filter="true"/>">
...
<script language="javascript">   
   showValidate('<bean:write name="browseHistoricalForm"  property="orgsumvali"/>');
</script>
<script language="javascript">
