...
.divStyle{
	...
	border:1px solid #C4D8EE;margin-top:3px;margin-top:3px;overflow: auto;left:5;
...
%>
<html:form action="/kq/register/history/dailyorgbrowsedata">
<script language="javascript">
   //验证是否报批成功 
   ...
   {
        browseHistoryForm.action="/kq/register/history/dailyorgbrowse.do?b_search=link&action=dailyorgbrowsedata.do&target=mil_body&a_inforkind=1&viewPost=kq";
        browseHistoryForm.target="il_body";
        browseHistoryForm.submit();
   }
          function daily_historicalt(obj)
   {
     if(obj.value=="1"){
        ...
        browseHistoryForm.action="/kq/register/historical/dailyorgbrowse.do?b_search=link&action=dailyorgbrowsedata.do&target=mil_body&a_inforkind=1&viewPost=kq";
        browseHistoryForm.target="il_body";
        browseHistoryForm.submit();
     }
   }
   ...
   {
        browseHistoryForm.action="/kq/register/history/sumorgbrowse.do?b_search=link&action=sumorgbrowsedata.do&target=mil_body&a_inforkind=1&viewPost=kq";
        browseHistoryForm.target="il_body";
        browseHistoryForm.submit();
   }
   function change()
   {
      browseHistoryForm.action="/kq/register/history/dailyorgbrowsedata.do?b_search=link&code=${browseHistoryForm.code}&kind=${browseHistoryForm.kind}";
      browseHistoryForm.submit();
   } 
   function couser()
   {
      browseHistoryForm.action="/kq/register/history/dailyorgbrowsedata.do?b_couser=link&code=${browseHistoryForm.code}&kind=${browseHistoryForm.kind}";
      browseHistoryForm.submit();
   } 
...
{
	if(dd==2){
 		browseHistoryForm.action="/kq/register/history/dailyorgbrowsedata.do?b_search=link&selectys=2&code=${browseHistoryForm.code}&kind=${browseHistoryForm.kind}";
    	browseHistoryForm.submit();
 	}else if(dd==1){
 		browseHistoryForm.action="/kq/register/history/dailyorgbrowsedata.do?b_search=link&selectys=1";
    	browseHistoryForm.submit();
 	}
}
...
<table>
 <tr>
  <td>
   <table width="50%" border="0" cellspacing="0"  align="left" cellpadding="0">
    <tr>
     <td align="left">
       ...
       --> 
        <td nowrap>&nbsp;
	  <bean:message key="kq.register.daily.menu"/><html:select name="browseHistoryForm" property="year" size="0" onchange="javascript:couser();">
   ...
   --> 
    <td align= "left" nowrap>        
        <html:hidden name="browseHistoryForm" property="codesetid" styleClass="text"/> 
        <html:hidden name="browseHistoryForm" property="code" styleClass="text"/> 
        <html:hidden name="browseHistoryForm" property="kq_duration" styleClass="text"/>
                    ...
                    <logic:equal name="info" property="itemtype" value="A">
                       <logic:notEqual name="info" property="codesetid" value="0">
                            <td align="left" class="RecordRow" style="border-left:none;" nowrap>
                               ...
                               {
                               %>
                               <a href="/kq/register/history/orgdailysingle.do?b_browse=link&&sessiondate=${browseHistoryForm.sessiondate}&b0110=<bean:write name="element" property="b0110" filter="true"/>">
...
<script language="javascript">   
   showValidate('<bean:write name="browseHistoryForm"  property="orgvali"/>');
</script>
<script language="javascript">
