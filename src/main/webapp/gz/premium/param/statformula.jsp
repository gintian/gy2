  	  ...
  	  if(document.premiumParamForm.fielditem.value!='')
  	   {
  	        for(var i=0;i<document.premiumParamForm.fielditem.options.length;i++)
  	        {
				...
				if(arr.length==2){
  	        	...
  	        	//	symbol('formula',document.premiumParamForm.fielditem.options[i].text);
...
function statConditions(){
    ...
    var thecodeurl ="/org/autostatic/mainp/setstatecond.do?b_query=link&expresion="+expresion; 
    var return_vo= window.showModalDialog(thecodeurl,"", 
...
function changebox(checkvalue,smode,itemname,hzname){
	var useflag = '0';
	...
	document.getElementById("fieldid").value= checkvalue;
	itemname=itemname!=null?itemname:'';
	document.getElementById("itemname").value=itemname;
	document.getElementById("hzname").value=hzname;
...
function getformulavalue(outparamters){
	...
	document.getElementById("wizard").disabled="";
	}
	document.getElementById("stat_methods").value=smode;
	document.getElementById("expresion").value =outparamters.getValue("expresion");
	if(runflag==1){
		linkIframe(standid);
	}else if(runflag==2){
		linkScale(standid);
...
function defCheck(itemname){
	...
	var hzname ="${premiumParamForm.hzname}";
	//toggles("saveformula_brilliant");	
	//hides("saveformula_dark");
    ...
    changebox(item,smode,itemname,hzname);
}
...
function addFormula(){
	...
	if(return_vo!=null){
		var fmode ="${premiumParamForm.fmode}";
		var thecodeurl ="/gz/premium/param/formula.do?b_stat=add&fmode="+fmode+"&itemid="+return_vo; 
...
function reflesh(){
	var fmode ="${premiumParamForm.fmode}";
    var thecodeurl ="/gz/premium/param/formula.do?b_stat=link&fmode="+fmode; 
    window.location.href=thecodeurl;
} 
function sorting(){
	var setid=document.getElementById("setid").value;
...
function savemula(){
	...
	var hzname = trim(document.getElementById("hzname").value);
	if(hzname==null||hzname==""){
	alert("请选择指标项");
	return;
	}
	//alert(document.getElementById("stat_methods").value);
	if(formula==""&&document.getElementById("stat_methods").value!="0"){
	alert("表达式不能为空!");
	return;
	}
...
function resultCheckExpr(outparamters){
	...
	if(info=="ok"){
		...
		var itemid = document.getElementById("item").value;
		hashvo.setValue("item",itemid);
		hashvo.setValue("cond",expression);
		hashvo.setValue("formula",getEncodeStr(formula));
		hashvo.setValue("fmode","${premiumParamForm.fmode}");
		hashvo.setValue("smode",stat_methods);
...
<html:hidden name="premiumParamForm" property="hzname"/>
<html:hidden name="premiumParamForm" property="expresion"/>
<table width="100%" height="250" border="0" align="center">
  <tr> 
    <td width="40%" height="200" align="center" valign="middle">
    <fieldset align="center" style="width:100%;height: 362px;">
    <legend><bean:message key="gz.formula.list.table"/></legend> 
      <table width="100%" height="200" border="0">
        <tr > 
          <td height="200" align="center" valign="top"> 
          <div id="scroll_box" style="height: 240px;margin-bottom: 5px;">
            <table width="100%" border="0" cellspacing="0"><!-- modify by xiaoyun 2014-10-15 月奖金，导入公式和统计公式 -->
			   ...
			   <tr> 
                <td class="RecordRow" style="border-left: none;border-top: none;" align="center" nowrap>
                	<logic:equal name="element" property="useflag" value="1">
                		<input type="checkbox"  name="${itemid}" value="1" onclick="changebox(${itemid},'${smode}','${itemname}','${hzname}');alertUseFlag(this);" checked/>
                	</logic:equal>
                	<logic:notEqual name="element" property="useflag" value="1">
                		<input type="checkbox"  name="${itemid}" value="0" onclick="changebox(${itemid},'${smode}','${itemname}','${hzname}');alertUseFlag(this);"/>
                	</logic:notEqual>
                </td>
                <td class="RecordRow" style="border-left: none;border-top: none;border-right: none;" onclick="changebox(${itemid},'${smode}','${itemname}','${hzname}');" nowrap>${hzname}
    ...
    <tr><td>
    <fieldset align="center" style="width:92%;">
	<legend><bean:message key="kq.wizard.expre"/></legend> 
		<table width="100%" border="0">
		<tr> 
          <td height="27" colspan="2" align="left" style="padding-left: 4px;">&nbsp;<bean:message key="kq.formula.fashion"/>
          	<html:select name="premiumParamForm" property="stat_methods" onchange="statmethods();" style="width:120">
        	...
        	<tr> 
          		<td colspan="2" align="center"> 
            		<html:textarea name="premiumParamForm" property="formula" onkeydown="discover();" cols="75" rows="7" styleId="shry"></html:textarea> 
            	</td>
        	</tr>
        	 <tr> 
         	<td height="21" colspan="2" align="right">
         		...
         		<input type="button" value="<bean:message key='makeupanalyse.stat'/>" onclick="statConditions();" Class="mybutton"> 
        	...
        	<tr> 
          		<td width="52%" align="center"> 
          		 <fieldset  align="center" style="width:92%;height: 132px;">
