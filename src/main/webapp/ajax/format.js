...
function parseDate(val) {
 ...
 var checkList = new Array('generalFormats',
      preferEuro ? 'dateFirst ':'monthFirst',preferEuro ?
...
function formatFloat(value, des) {
	...
	if(desCheck.test(des)) {
		...
		var marker=RegExp.$3; //千分符，例如233,334.98$
		var multi=RegExp.$4;
		...
		if(marker!=null && marker!=""){
			nF.setSeparators(true, marker, decimal);
		} else {
			nF.setSeparators(false, marker, decimal);
		}
