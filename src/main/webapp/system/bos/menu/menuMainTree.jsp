...
function openAdd()
  {
  	    ...
  	    var theurl="/system/bos/menu/menuMain.do?b_newMenu=new`menuid="+base_id;
    	var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
    	var dw=400,dh=360,dl=(screen.width-dw)/2;dt=(screen.height-dh)/2;
  		...
  		// var menu_base_bean = new Object();
    	   if(retvo!=null){
	    	   var menu_name = retvo.menu_name;
	    	   var menu_id = retvo.menu_id;
	    	   var parentid=retvo.parentid;
	    	  // var hashvo = new ParameterSet();
    	   	  // hashvo.setValue("menu_base_bean",menu_base_bean);
    	   	  ...
    	   	  // var request=new Request({asynchronous:false,onSuccess:add_user_ok,menuId:'1010800104'},hashvo); 
    	      //var type ="2&menu_name="+menu_name+"&menu_id="+menu_id+"&parentid="+parentid;
    	      //dealwith(type);
    	      add_user_ok(menu_id,menu_name);
    	      
    	   }
  }

  function add_user_ok(menu_id,menu_name){
  var currnode=Global.selectedItem;
	     var groupid=menu_id;
	     var groupname=menu_name;
	     
	 	 if(currnode.load)
	     {
			var imgurl;
			imgurl="/images/open.png";
			var tmp = new xtreeItem(groupid,groupname,"/system/bos/menu/menuMain.do?b_search=query&parentid="+groupid,"mil_body",groupname,imgurl,"/system/bos/menu/menu_main_tree.jsp?opt=1&codeid=0&menu_id="+groupid);
		 	currnode.add(tmp);
	     }
	     else
	     	currnode.expand();
  ...
  function rename(){
   var currnode,base_id;
    	   ...
    	   {
    	 var theurl="/system/bos/menu/menuMain.do?b_editMenu=new`menuid="+base_id;
    	var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
    	var dw=400,dh=360,dl=(screen.width-dw)/2;dt=(screen.height-dh)/2;
   		var retvo= window.showModalDialog(iframe_url, 'template_win', 
      				"dialogLeft:"+dl+"px;dialogTop:"+dt+"px;dialogWidth:"+dw+"px; dialogHeight:"+dh+"px;resizable:no;center:yes;scroll:yes;status:no");
...
function delete_base1()
    	   ...
    	   //  var theArr=new Array(currnode);
    	    ...
    	    // var hashvo=new ParameterSet();
     		...
     		// hashvo.setValue("a_base_id",base_id);
              ...
              //                onSuccess:delete_ok,menuId:'1010800108'});
			//var request=new Request({asynchronous:false,onSuccess:delete_ok,menuId:'1010800108'},hashvo);
			 //target_url="/selfservice/lawbase/add_law_base.do?b_delete=link&a_base_id="+base_id;
    	     //parent.location.href=target_url;
    	     var type ="4&menu_id="+base_id;
    	     dealwith(type);
    	     delete_ok();
    	     }
    	     else
		...
		function copy(){
		window.location.href="/servlet/DownLoadMenu";
		}
		function fileInput() {
    	currnode=Global.selectedItem;
    		if(currnode==null)
    	   		return;
    		base_id=currnode.uid;
    	parent.mil_body.location = "/system/bos/menu/menu_file_input.jsp?b_query=link&base_id="+ base_id;
	}
	
 function dealwith(type,typeid){
  var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");  
	var url = "/system/bos/menu/menu_dw_tree.jsp?type="+type;
...
function dragendMenu(){
	var currnode=Global.selectedItem; 
	
	if(currnode.dragbool){
	if(currnode.dragFrom.uid=='root')
	return;
		 var type ="6&frommenu_id="+currnode.dragFrom.uid+"&tomenu_id="+currnode.uid;
    	     dealwith(type);
		if(currnode.uid=='root'){
			currnode.dragFrom.remove();
			currnode.clearChildren();
			currnode.loadChildren();
			parent.frames["mil_body"].location="/system/bos/menu/menuMain.do?b_search=query&parentid=-1";
		}else{
			...
			currnode.clearChildren();
	  		currnode.loadChildren();
	  parent.frames["mil_body"].location="/system/bos/menu/menuMain.do?b_search=query&parentid="+currnode.uid;
		}
	}
}
function payrollSort(){
    	   ...
    	   if(currnode==null)
    	    	return;
    	   base_id=currnode.uid;
    	   var theurl="/system/bos/menu/menuMain.do?b_payMenu=new`menuid="+base_id;
    	   var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
...
<table width="100%" height="100%" align="left" border="0" cellpadding="0" cellspacing="0" class="mainbackground">
	...
	<tr>  
		<td valign="top" >
			<div id="treemenu" ondragend="dragendMenu();"  style="height:100%;width:100%;overflow-x:auto;overflow-y:auto;"></div>
		</td>
	...
	var m_sXMLFile	= "/system/bos/menu/menu_main_tree.jsp?opt=0&codeid=0&menu_id=0";
	var href ="/system/bos/menu/menuMain.do?b_search=query&parentid=-1";		
	var newwindow;
	var root=new xtreeItem("root",MENUREGISTER,href,"mil_body",MENUREGISTER,"/images/add_all.gif",m_sXMLFile);
