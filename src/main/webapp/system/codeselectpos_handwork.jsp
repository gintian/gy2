...
<%
	...
	String rootdesc = "组织机构";
	if(session.getAttribute("rootdesc")!=null && !session.getAttribute("rootdesc").equals("")){
		rootdesc = session.getAttribute("rootdesc").toString();
	}
...
<script language="javascript">
   	...
   	function savecode()
   	{
   	   ...
   	   var currnode,codeitemid;
    	   ...
    	   else
    	   {
    	     if(flag=="query")
    	     {
    	     	if(codeitemid=='root')//选择了根节点[组织机构]
    	   		{
    	   			 targetobj.value="";
    	   			 targethidden.value="";
    	   		}
   	...
   	function showDateSelectBox(srcobj)
   {
      ...
      hashVo.setValue("needPrefix","1");/* 定义需要获取前缀*/
   ...
   function okSelect()
	{
	 ...
	 targetobj.value=tit;
	/*输入相关代码类和选择的相关代码一致*/
	if(input_code_id!=cont.substring(0,2)&&!flag)
		return ; 
	 /* 通过与savecode()方法相同的条件获取value的值*/
	if(!flag)
		targethidden.value=cont.substring(2);
	else{
   	     if(flag=="query")
   	     {
			targethidden.value=cont.substring(2);
...
<table width="100%" border="0" cellspacing="0" align="center"
			...
			<tr align="left">
				<td valign="top" nowrap>
					<bean:message key="column.name" /><Input class=text4 type='text' name='a_name' style="width: expression(document.body.clientWidth-40);margin-left:5px;" id="selectname" onkeyup="showDateSelectBox('selectname')" title='' />
				</td>
			</tr>
			<tr>
				<td align="left">
   ...
   <SCRIPT LANGUAGE=javascript>
             ...
             var itemdesc="代码项目";
             var codevalue="01";
             if(codesetid.toUpperCase( )=='UN'||codesetid.toUpperCase( )=='UM'||codesetid.toUpperCase( )=='@K')
             {
             	<% if(request.getParameter("codevalueName")!=null&&request.getParameter("codevalueName").length()>0){  %>
             	itemdesc="<%=request.getParameter("codevalueName")%>";
             	codevalue="<%=request.getParameter("codevalue")%>";
             	/**
             	*cmq changed at 20121001 
             	*/
             	m_sXMLFile=m_sXMLFile+"&privflag=1";
             	//end.
             	<% } else { %>
             	itemdesc="<%= rootdesc %>";
             	codevalue="root";
             ...
             var root=new xtreeItem(codevalue,itemdesc,"","",itemdesc,"/images/unit.gif",m_sXMLFile);
