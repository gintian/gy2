 ...
 * 
 * 二、支持对树的级联多选 
 *    当选择结点时，自动选择该结点下的所有子结点，以及该结点的所有父结点（根节点除外），特别是支持异步，当子结点还没显示时，会从后台取得子结点，然后将其选中/取消选中
...
Ext.extend(Ext.tree.TreeCheckNodeUI, Ext.tree.TreeNodeUI, {
    ...
    check : function(checked){
		...
		} else if(!this.onlyLeafCheckable && this.checkModel == 'childCascade'){
			/*var parentNode = n.parentNode;
			if(parentNode !== null) {
				this.parentCheck(parentNode,checked);
			}*/
		...
		} else if(!this.onlyLeafCheckable && this.checkModel == 'parentCascade'){
			var parentNode = n.parentNode;
			if(parentNode !== null) {
				this.parentCheck(parentNode,checked);
			}
