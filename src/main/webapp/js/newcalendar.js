...
var cal_width = document.getElementById('cal_width');
if(cal_width){
	cal_Width=parseInt(cal_width.value);
}

document.write("<html><body><div id='meizzCalendarLayer' style='position: absolute; z-index: 9999; width: " + (cal_Width+4).toString() + "px; height: 195px; display: none'>");
...
function cal_WebCalendar(){ //初始化日历的设置
    this.regInfo    = "关闭的快捷键：[Esc]";//WEB Calendar ver 3.0&#13;
    ...
    this.todayColor = "#95B7F3";    //今天在日历上的标示背景色
...
//    for (var i=yearMin; i<=yearMax; i++) strIframe += "<option value='"+i+"'>"+i+"年</option>";
    ...
    "    <table id=tableDay height='120px' width='"+ cal_Width +"px' border=0 cellspacing=1 cellpadding=0><tbody>";
         ...
         "<input style=' background-color: " + WebCalendar.btnBgColor +";cursor: hand; padding-top: 0px; width: 44%; height: 100%;' onfocus='this.blur()'"+
         " type=button value='清空' onclick='parent.WebCalendar.objExport.value=\"\";parent.cal_hiddenCalendar()'>" + 
         "&nbsp;" + 
         "<input style=' background-color: " + WebCalendar.btnBgColor +";cursor: hand; padding-top: 0px; width: 43%; height: 100%;' onfocus='this.blur()'"+
         " type=button value='关闭' onclick='parent.cal_hiddenCalendar()'>" + 
         "</td></tr></tbody></table>"+
    "</td></tr><tr><td height='20px' width='"+ cal_Width +"px' bgcolor='"+ WebCalendar.lightColor +"' style='vertical-align:top;' norow>"+
...
function cal_writeCalendar(){ //对日历显示的数据的处理程序
    ...
    for(var i=0; i<WebCalendar.dayShow; i++)    //这个循环是根据源数组写到日历里显示
    {
        ...
        {WebCalendar.dayObj[i].bgColor = WebCalendar.todayColor; WebCalendar.dayObj[i].style.color = WebCalendar.wordColor;}
    }
}
