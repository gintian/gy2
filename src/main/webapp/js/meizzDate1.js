...
document.writeln('<iframe id=meizzDateLayer Author=wayx frameborder=0 style="position: absolute; width: 144; height: 251; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
strFrame+='  <tr id="time" Author="wayx" style="display:none"><td width=142 height=18 Author="wayx">';
strFrame+='    <table border=1 cellspacing=2  height=20 cellpadding=0 BORDERCOLORLIGHT=#FF9900 BORDERCOLORDARK=#FFFFFF bgcolor=#fff8ec width=140 height=120 Author="wayx">';
strFrame+='<tr Author="wayx" align=center valign=bottom><td align="left" style="font-size:12px;" Author=meizz>';
strFrame+='<select id="hour">';
strFrame+='  </select>时';
strFrame+='<select id="minute">';
strFrame+='  </select>分';
...
*控制是否显示时间
*********************************/
function inittime(bdisplay)
{	
	btime=bdisplay;
	var  timeobj=window.frames.meizzDateLayer.document.getElementById("time");
	if(bdisplay)
	{
	  timeobj.style.display = '';
	  var temp;
	  var hour=window.frames.meizzDateLayer.document.getElementById("hour");
	  if(hour.value=="")
	  {
	  	for(var i=0;i<=23;i++)
	     {
    		...
    		no.value=temp;
    		no.text=temp;
    		hour.options[hour.options.length]=no;	    
	     }
	  }
	  
	  var minute=window.frames.meizzDateLayer.document.getElementById("minute");
	  if(minute.value=="")
	  {
	    for(var i=0;i<=59;i++)
	    {
    		...
    		no.value=temp;
    		no.text=temp;
    		minute.options[minute.options.length]=no;	    
	    }
	  }	  	  
	}
	else
 ...
 *设置小时和分钟对应的选项
 *******************************************/
function setSelectHourMinute(hour,minute)
{
	var hourobj=window.frames.meizzDateLayer.document.getElementById("hour");
	hourobj.selectedIndex=parseInt(hour,10);
	var minuteobj=window.frames.meizzDateLayer.document.getElementById("minute"); 
	minuteobj.selectedIndex=parseInt(minute,10);
}

function setday(tt,obj) //主调函数
{
	tt.fireEvent("onchange");   
...
function meizzToday()  //Today Button
  {
    ...
    if(outObject){
		outObject.value=meizzTheYear + "." + meizzTheMonth + "." + today;
		 if(outObject.value.length>dateLength)
			outObject.value=outObject.value.substring(0,dateLength);
    }
    closeLayer();
  }
...
function meizzDayClick(n,ex)  //点击显示框选取日期，主输入函数*************
{
  ...
  if (outObject)
  {
    ...
    if(btime)
    {
      var minuteobj=window.frames.meizzDateLayer.document.getElementById("minute");
      var hourobj=window.frames.meizzDateLayer.document.getElementById("hour");
      var hour=hourobj.options[hourobj.selectedIndex].value;
      var minute=minuteobj.options[minuteobj.selectedIndex].value;
      outObject.value= yy + "." + mm + "." + n +" "+hour+":"+minute; //注：在这里你可以输出改成你想要的格式    
      if(outObject.value.length>dateLength)
			outObject.value=outObject.value.substring(0,dateLength);	
    }
    else{
 ...
 function setDateLength(len){
	try{
		dateLength=parseInt(len);
	}catch(e){}
}