...
//	email :	fuushikaden@yahoo.com
	...
	if (vLangue==0) //FRENCH
		...
		weekString = "Sem"
		scrollLeftMessage = "Cliquer pour le mois pr?c?dent. Tenir enfonc?pour d?roulement automatique."
		//alert("scrollLeftMessage---"+scrollLeftMessage);
		scrollRightMessage = "Cliquer pour le mois suivant. Tenir enfonc?pour d?roulement automatique."
		//alert("scrollRightMessage---"+scrollRightMessage);
		selectMonthMessage = "Cliquer pour choisir un mois."
		selectYearMessage = "Clicquer pour choisir une ann?e."
    ...
    */
	...
	if (dom&&!document.getElementById("calendar"))
	{
		...
		document.write ("<div onclick='bShow=true' id='calendar'	style='z-index:+999;position:absolute;visibility:hidden;'><table	width="+((showWeekNumber==1)?250:220)+" style='font-family:arial;font-size:11px;border-width:1;border-style:solid;border-color:#a0a0a0;font-family:arial; font-size:11px}' bgcolor='#ffffff'><tr bgcolor='#0000aa'><td><table width='"+((showWeekNumber==1)?248:218)+"'><tr><td style='padding:2px;font-family:arial; font-size:11px;'><font color='#ffffff'><B><span id='caption'></span></B></font></td><td align=right><div onclick='hideCalendar()'><IMG SRC='"+imgDir+"close.gif' name=close WIDTH='15' HEIGHT='13' BORDER='0' ALT='" + altCloseCalendar + "'></div></td></tr></table></td></tr><tr><td style='padding:5px' bgcolor=#ffffff><span id='_content'></span></td></tr>")
	...
	function init()	{
		if (!ns4)
		{
			...
			{
				if (vLangue)
					document.getElementById("lblToday").innerHTML =	todayString + " <a style='"+styleAnchor+"' href='javascript:monthSelected=monthNow;yearSelected=yearNow;constructCalendar();'>"+dayName[firstdayofweek(today.getDay())]+", " + yearNow + " " + monthName[monthNow].substring(0,3)	+ "	" +	dateNow	+ "</a>"
				else
	...
	function setDate(datePointer_value)
	{
		dateSelected=datePointer_value;
		closeCalendar();
   ...
   function WeekNbr(n) {
	...
	function constructCalendar () {
		...
		for	(i=0; i<7; i++)	{
			...
			{
				sHTML += "<td width='33' align='center' valign=middle style='height:15;font-size:9pt;color:#000000 Author=meizz;cursor:default;background-color:#FF9999;'><B>"+ dayName[i]+"</B></td>"
			 }else
		        {
		        	sHTML += "<td width='33' align='center' valign=middle style='height:15;font-size:9pt;color:#000000 Author=meizz;cursor:default;background-color:#99CCCC;'><B>"+ dayName[i]+"</B></td>"
			}
		}
		sHTML +="</tr><tr>"
		...
		for	( datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
		{
			...
			{ 
			    ...
			    {
			    	sHTML += "<b><div  title=\"" + sfeast_Hit + "\" style='"+sStyle+"' onclick='setDate("+datePointer+")'><font color=#ff0000>&nbsp;" + datePointer + "</font>&nbsp;</div></b>"
			    }else
			    {
			        sHTML += "<b><div title=\"" + sHint + "\" style='"+sStyle+"' onclick='setDate("+datePointer+")'><font color=#ff0000>&nbsp;" + datePointer + "</font>&nbsp;</div></b>"
			    }
			}
			else if	(weekString.indexOf(now_week)!=-1)//??????
			{
				if(!getWeek_date(yearSelected,monthSelected,datePointer))
				{
					if(getfeastStr(yearSelected,monthSelected,datePointer))
			                 {
			                     sHTML += "<div title=\"" + sfeast_Hit + "\" style='"+sStyle+"' onclick='setDate("+datePointer+")'>&nbsp;<font color=#3333FF>" + datePointer + "</font>&nbsp;</div>" 
			                 }else
			                 {
			                   sHTML += "<div title=\"" + sweek_Hit + "\" style='"+sStyle+"' onclick='setDate("+datePointer+")'>&nbsp;<font color=#909090>" + datePointer + "</font>&nbsp;</div>" 	
			                 }
					
			        }else
			        {
			        	sHTML += "<div title=\"" + swork_Hit + "\" style='"+sStyle+"' onclick='setDate("+datePointer+")'>&nbsp;" + datePointer + "&nbsp;</div>" 
			        }
			...
			{ 
			    ...
			    }else
			    {
			        ...
			        {
			            sHTML += "<div  title=\"" + sturn_Hit + "\" style='"+sStyle+"' onclick='setDate("+datePointer+")'>&nbsp;<font color=#909090>" + datePointer + "</font>&nbsp;</div>" 
			        }else
			        {
			            sHTML += "<div  title=\"" + swork_Hit + "\" style='"+sStyle+"' onclick='setDate("+datePointer+")'>&nbsp;" + datePointer + "&nbsp;</div>" 
			        }
	...
	function popUpCalendar(ctl,ctl2,weeks,feastS,turn_date,week_date,showTime,dateFormat2) {		
		
		weekString=weeks+",";		
		feastString=feastS;
		var format="yyyy.mm.dd";		
		if(dateFormat2!=null&&dateFormat2!="")
		  format=dateFormat2;		
		...
		if (bPageLoaded)
		{
			...
			{
				hideCalendar()
				if (ctlNow!=ctl) {popUpCalendar(ctl, ctl2, format,feastS,turn_date,week_date)}
			}
			ctlNow = ctl
		}
	}
