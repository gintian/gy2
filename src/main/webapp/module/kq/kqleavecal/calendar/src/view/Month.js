...
Ext.define('Ext.calendar.view.Month', {
    ...
    enableSortEventRecordsForDay: true,
    /**
    ...
    afterRender: function() {
        if (!this.tpl) {
            this.tpl = new Ext.calendar.template.Month({
                ...
                enableSortEventRecordsForDay: this.enableSortEventRecordsForDay, 
            });
        }
        this.tpl.compile();
    ...
    onResize: function() {
        ...
        me.maxEventsPerDay = me.getMaxEventsPerDay()+2;
        if (me.monitorResize) {
            me.refresh();
        }
    },
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
                '<div id="{_elId}" class="{_selectorCls} {_colorCls} {spanCls} ext-cal-evt ext-cal-evr" style="background:{_itemColor};">',
                    body,
                '</div>'
            )
            : new Ext.XTemplate(
                '<tpl if="_renderAsAllDay">',
                    '<div id="{_elId}" class="{_selectorCls} {spanCls} {_colorCls} {_operaLT11} ext-cal-evo" style="background:#000;">',
                        '<div class="ext-cal-evm">',
                            '<div class="ext-cal-evi">',
                '</tpl>',
                '<tpl if="!_renderAsAllDay">',
                    '<div id="{_elId}" class="{_selectorCls} {_colorCls} {_operaLT11} ext-cal-evt ext-cal-evr" style="background:#000;">',
                '</tpl>',
                    body,
                '<tpl if="_renderAsAllDay">',
                            '</div>',
    ...
    getTemplateEventData: function(evt) {
        ...
        return Ext.applyIf({
            ...
            _itemColor: evt[M.CalendarId.name] || "#ffffff" != evt[M.CalendarId.name] ? evt[M.CalendarId.name] : '#306da6',  //鏀逛负鑰冨嫟瑙勫垯棰滆壊
