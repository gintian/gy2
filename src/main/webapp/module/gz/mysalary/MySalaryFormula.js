...
Ext.define('mysalary.MySalaryFormula', {
    requires: ['EHR.extWidget.proxy.TransactionProxy'],
    constructor: function (config) {
        ...
        thisScope.callbackfunc = config.callbackfunc;
        thisScope.hzname = "";
        thisScope.range = null;//计算公式选中的range对象
        // 计算公式参考项目数据
        thisScope.itemdata = config.itemdata;
        // 选择的指标类型
        thisScope.itemType = config.itemType;
        // 公式内容
        thisScope.calcFormat = config.calcFormat;
        // 选择的指标所在子集
        thisScope.fieldSetId = config.fieldSetId;
        // 选择的指标id
        thisScope.itemId = config.itemId;
        thisScope.init();
    ...
    saveFormula: function () {
        ...
        var json = {
            type: 'checkFormula',
            c_expr: getEncodeStr(c_expr),
            itemType:thisScope.itemType,
            fieldSetId:thisScope.fieldSetId
        };
        map.put("jsonStr", json);
        Rpc({functionId:'GZ00000804',async:false,success: function(form,action){
                var result = Ext.decode(form.responseText);
                var info=result.return_data.info;
                if(info=="ok"){
                    Ext.callback(thisScope.callbackfunc,null,[c_expr,thisScope.itemId]);
