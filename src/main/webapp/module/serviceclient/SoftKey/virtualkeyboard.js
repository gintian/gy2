...
var VirtualKeyboard = new function () {
            ...
            if (e) {
                if (Y.requires) {
                    ...
                    ScriptQueue.queue(iI, function () {
                        if (Y.toString() == il) {
                            g.apply(I, arguments);
                            if(il=='US US'){//此处处理第一次加载某个输入法的时候进行加载
                                $("#kbDesk div#kb_bimestatus a").css("background-position", "-822px -156px");
                            }else{
                                $("#kbDesk div#kb_bimestatus a").css("background-position", "-737px -156px");
                            }
                        }
                    });
                } else {
    ...
    var d = function (i) {
        ...
        switch (il) {
            case "caps":
                ...
                break;
            case "input_method":
                if (document.getElementById("kb_langselector").value == 'US US'&&'passWord-inputEl'!=U.attachedInput.id&&'vaildata-inputEl'!=U.attachedInput.id) {
    ...
    I.open = I.show = function (i, e) {
        /**
         * 自助终端实现方式,应当是给div添加键盘显示,input应当是聚焦的可输入框
        **/
    ...
    I.close = I.hide = function () {
        ...
        I.detachInput();
        I.IME.hide();
        if(U.keyboard.parentNode){
	        U.keyboard.parentNode.removeChild(U.keyboard);
        }
        return true
    };
    I.toggle = function (i, e, iI) {
        //open,以后还得判断当前附加的输入框是否等于I附加的框,open并且等于的话,就隐藏
        //传入i,是input输入框,传入e是键盘需要显示在的位置 无论如何不隐藏键盘?
    ...
    (function () {
        ...
        U.keyboard.innerHTML = (
            "<div id=\"kbDesk\"><!-- --></div>" + "<div class=\"progressbar\"><!-- --></div>" + 
            "<select id=\"kb_langselector\"></select>" + "<select id=\"kb_mappingselector\"></select>" + 
            '<div id="copyrights" nofocus="true"><a href="#" target="_self" title="点击进行中英文切换">中英文切换 </a></div>').replace(/(<\w+)/g, 
            "$1 unselectable='on' ");
        U.desk = U.keyboard.firstChild;//div id="kbDesk"
        U.progressbar = U.keyboard.childNodes.item(1);//div id="progressbar",下标估计是从0开始
        var i = U.keyboard.childNodes.item(2);//select id="kb_langselector"
        EM.addEventListener(i, 'change', function (e) {
            I.switchLayout(this.value)//当这个发生变化时,更新键盘布局,但是现在这个好像没有用了,被隐藏了
        });
        U.langbox = i;
        var i = i.nextSibling,
                iI = "";
...
VirtualKeyboard.IME = new function () {
    ...
    i.updatePosition = function () {
        ...
        var nowleft = x.x;//键盘本来应该在的位置
        var maxWidth = nowleft+640;//是否超出整个网页可视区域
        var moveWidth=0;
        if(maxWidth>width){//超出则需要挪动
        	moveWidth = (maxWidth- width)+10;
        }
        l.style.left = (nowleft-moveWidth) + 'px';
    ...
    var e = function () {
        var x = ['<table>'];
        for (var X = 0, z = Math.ceil(_.length /10) ; X < z; X++) {//123456789
            if (Q || X == O) {
                x.push('<tr>');
                for (var Z = 0, w = X * 10; Z < 10 && !isUndefined(_[w + Z]) ; Z++) {
                    x.push("<td><a href='' style='font-size:17px'>");
                    if (X == O) {
                        x.push("<b>&nbsp;" + ((Z + 1) % 10) + ": </b>");
                    } else {
                        x.push("<b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b>");
        ...
        l.firstChild.rows[1].cells[0].firstChild.firstChild.innerHTML = (O + 1) + "/" + (0 + Q || Math.ceil(_.length / 10));//1/1
