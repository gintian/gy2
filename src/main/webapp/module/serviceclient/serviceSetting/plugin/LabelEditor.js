Ext.define("ServiceClient.serviceSetting.plugin.LabelEditor",{
	...
	initMenu:function(){
		//菜单只创建一个，如果有了不再创建
		if(Ext.getCmp("serviceClient_label_menu"))
			return;
		//初始化菜单选项
		Ext.widget('container',{
				layout:'vbox',
				id:'serviceClient_label_menu',
				shadow:false,
				floating:true,
				items:[{
					xtype:'container',
					...
					defaults:{
					   listeners:{
						   render:function(){
							   var qq = this.up('#serviceClient_label_menu');
	...
	beEdit:function(){
	    var me = this;
		var editor = Ext.getCmp("serviceClient_label_menu");
		if(editor){
