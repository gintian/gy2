...
<!--
	function backDate(){
		...
		var backdate=showModalDialog('/train/resource/course/pos.do?br_backdate=link','_blank','dialogLeft:'+dl+'px;dialogTop:'+dt+'px;dialogHeight:320px;dialogWidth:350px;center:yes;help:no;resizable:no;status:no;');
		if(backdate) {
			<logic:equal value="1" name="courseForm" property="validateflag">
				courseForm.action="/train/resource/course/pos.do?b_tree=link&backdate="+backdate;
			</logic:equal>
			<logic:notEqual value="1" name="courseForm" property="validateflag">
				courseForm.action="/train/resource/course/pos.do?b_tree=link&checked="+backdate;
			</logic:notEqual>
			courseForm.target="il_body"
			courseForm.submit();
		}else
			return false;
	}
 ...
 function init(){
	var validateflag = '${courseForm.validateflag}';
	if(validateflag != 1){
		this.parent.document.getElementById("center_iframe").style.height ="0";
    }
 }
//-->
</script>
    ...
    <table width="1000" border="0" cellspacing="0"  align="center" cellpadding="0" >
		...
		<hrms:priv func_id="">
			<logic:equal value="1" name="courseForm" property="validateflag">
				...
				<script> toolFlag = true;</script>              
			</logic:equal>
			<!-- 
			<logic:notEqual value="1" name="courseForm" property="validateflag">
				<a href="###" onclick="return backDate();"><img src="/images/quick_query.gif" alt="查询设置" border="0" align="middle"></a>               
			</logic:notEqual>
			 -->
		</hrms:priv>
...
<script language="javascript">
  ...
  if(!toolFlag){
	  window.parent.document.getElementById("tooltd").style.height="0px";
  }
</script>
</body>