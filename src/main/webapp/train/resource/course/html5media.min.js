...
(function(window, document, undefined) {    
    ...
    function scanElementsByTagName(tagName) {
        ...
        for (n = 0; n < elementsList.length; n++) {
            ...
            if (!window.applicationCache || requiresFallback || html5media.forceFallback(tagName, element)) {
    ...
    html5media.flowplayerSwf = scriptRoot + "flowplayer-3.2.18.swf";
    html5media.flowplayerAudioSwf = scriptRoot + "flowplayer.audio.swf";
    html5media.flowplayerControlsSwf = scriptRoot + "flowplayer.controls-3.2.16.swf";
    ...
    html5media.createFallback = function(tagName, element) {
        ...
        if (tagName == "audio") {
            ...
            playlist[playlist.length - 1].autoBuffering = true;
        }
        // Load the Flowplayer.
        var config = {
            play: null,
            playlist: playlist,
            clip: {
                scaling: "fit",			  //瑙嗛鍦ㄥ睆骞曚笂鐨勭缉鏀�  fit锛氳嚜閫傚簲 ;half: 涓�鍗婏紙淇濇寔瀹介珮姣斾緥锛夛紱   ORIG锛氫娇鐢ㄧ紪鐮佸湪鏂囦欢涓殑灏哄;濡傛灉瑙嗛鏄お澶х殑鍙敤绌洪棿涓紝瑙嗛缂╂斁淇濇寔闀垮姣旓紱orig锛氱缉鏀捐棰戯紝浠ュ～琛ユ墍鏈夊彲鐢ㄧ┖闂�;蹇界暐鍦ㄥ厓鏁版嵁鐨勫昂瀵�
                fadeInSpeed: 0,			  //浠ユ绉掍负鍗曚綅锛屼粠闆跺紑濮嬬殑瑙嗛鎾斁鏃舵笎鍙樹负瀹屽叏涓嶉�忔槑銆傦紙鏈�澶т负1000锛�
                fadeOutSpeed: 0,		  //浠ユ绉掍负鍗曚綅娣″嚭浠庡叏瑙嗛鎾斁缁撴潫鍚庨浂閫忔槑搴�
                autoBuffering: true,      //鏄惁鑷姩缂撳啿瑙嗛锛岄粯璁rue   
                autoPlay: true			  //鏄惁鑷姩鎾斁瑙嗛锛岄粯璁rue   
            },
            canvas: {
                backgroundGradient: "none",
        ...
        myflowplayer = flowplayer(replacement, {
            src: fixPath(html5media.flowplayerSwf),
            expressInstall: fixPath(html5media.expressInstallSwf),
            wmode: "opaque"
        }, config);
        
        myflowplayer.onStart (
     			function(){
     				doThis();
					countDown();
	     			// 闃叉postion瓒呰繃瑙嗛鎬婚暱搴�
	     			if (learnedTime < myflowplayer.getClip().duration){
	     				myflowplayer.seek(learnedTime);
     			} 
      		});
        myflowplayer.onPause(
        		function(){
    				//saveThis(); //鏆傚仠鐨勬椂鍊欏紑濮嬭鏃�
        		}	
        	);
        myflowplayer.onResume(
				function(){
					if(msg && learnTime > 0) {
						doThis();
						c = 20+ (learnTime * 60);
     		...
     		// 鏈�鍚庝竴绉掔殑鏃跺�欏皢鏃堕棿淇濆瓨
        myflowplayer.onLastSecond(
     			function() {
     				saveTime(2);
     				isStop = '0';
     			}
     		);
     		//缁撴潫鏃朵繚瀛樻椂闂� 鍚屼簨瑙﹀彂浜嬩欢
        myflowplayer.onFinish(
     			function(){
     				saveTime(2);
     				isStop = 0;
     			}
