...
function IsDigit(obj){ 
    ...
    if ((event.keyCode >= 46) && (event.keyCode <= 57) && event.keyCode != 47) {
		...
		if ((event.keyCode == 46) && (values.indexOf(".") != -1)) {//鏈変袱涓?
			return false;
		}
		if ((event.keyCode == 46) && (values.length == 0)) {//棣栦綅鏄?
			return false;
		}
		return true;
	}
...
function addTable(itemid,desc,type,codesetid,eq,values){
	...
	tabelstr+="<td class=\"RecordRow noleft\" align=\"center\" style=\"border-top: none;\" height=\"30\">";
	tabelstr+=(n+1);
	tabelstr+="</td><td class=\"RecordRow noleft\" style=\"border-top: none;\" height=\"30\">";
	tabelstr+=desc;
	tabelstr+="</td><td class=\"RecordRow noleft\" align=\"center\" style=\"border-top: none;\" height=\"30\">";
	...
	tabelstr+="</td><td class=\"RecordRow noleft noright\" style=\"border-top: none;\">";
	if(type=='N'){
		tabelstr+="<input type=\"text\" name=\""+id+".value\" value=\""+values+"\" class=\"text4\"";
		tabelstr+=" onkeypress=\"event.returnValue=IsDigit(this);\" maxlength='8' style=\"width:120px\">";
	}else if(type=='D'){
		...
		if(codesetid=="0"){
			tabelstr+="<input type=\"text\" name=\""+id+".value\" value=\""+values+"\" class=\"text4\" style=\"width:120px\">";
		}else{
			...
			if(codesetid=='UN'||codesetid=='UM'||codesetid=='@K'){
			 	tabelstr+="&nbsp;<img align=\"absMiddle\" src=\"/images/code.gif\" onclick=\"javascript:openInputCodeDialogOrgInputPos('";
			 	tabelstr+=codesetid+"','"+id+".hzvalue','"+manageCode+"',1);\">";
			}else if("55"==codesetid){
				tabelstr+="&nbsp;<img align=\"absMiddle\" src=\"/images/code.gif\" onclick=\"javascript:openCondCodeDialogsx('";
				tabelstr+=codesetid+"','"+id+".hzvalue');\">";
			}else{
				tabelstr+="&nbsp;<img align=\"absMiddle\" src=\"/images/code.gif\" onclick=\"javascript:openCondCodeDialog('";
				tabelstr+=codesetid+"','"+id+".hzvalue');\">";
			}
		}
	}
...
function delTableStr(){
	...
	*/
	for(var i=n;i<this.idArr.length;i++){
		...
		var str_new  = "height=30 align=middle>"+(i+1)+"</TD>";
		var str_old  = "height=30 align=middle>"+(i+2)+"</TD>";
