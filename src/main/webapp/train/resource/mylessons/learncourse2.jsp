...
<%
	...
	//试听页面内容显示是否有权限控制
	String piv = request.getParameter("piv");
	// 课程id
	...
	String lessonName = bo.getLessonName(lessonId,b0110);
	// 课程分类
	...
	ArrayList courseList = bo.getNoCheckCourseList(lessonId);
	...
	try {
		...
		if (courseId == null || courseId.length() <= 0) {
			...
			if( "".equals(courseType.trim()) ||courseType == null ){ //查询当前课程下的第一个课件 如果是未选择的课程
				String sql = "select * from r50 r,r51 t where r.r5000 = t.r5000 and t.r5000 = '"+lessonId+"'";
				rs = dao.search(sql);
				if(rs.next()){
					courseId = rs.getString("r5100");
					courseType = rs.getString("r5105");
				...
				if(courseList == null || courseList.size() == 0){
					courseList = bo.getNoCheckCourseList(lessonId); //课件列表信息 当前课程没有被选择的时候 既没被选课
				}
			}
		} else {
			// 查询课件
			buff.delete(0, buff.length());
			buff.append("select r5100,r5105,r5113,r5115,r5111,r5117");
...
<body >
	...
	<%if("1".equalsIgnoreCase(piv)){
		out.println("<hrms:priv func_id=\"090903\">");
	}
	...
	if(lessonName != null && !"".equals(lessonName)){ %>
		...
		<div class="headdiv">
		<div class="leftdiv">
		<div class="tableRow" style="border-bottom: none;padding-top: 7px;">【<%=lessonName %>】</div>
			<%if ("3".equals(courseType)) {%>
	...
	<%}%>						
			...
			if(null != coursePath){	
			 String ppath = coursePath.replaceAll(Matcher.quoteReplacement(System.getProperty("file.separator")),"/");
			%>
				...
				<tr>
					<td height="60" class="RecordRow" >
						...
						<table border="0" width="250" style="margin-top:5px;margin-left:30px;margin-bottom:10px;">
							<tr>
								<td align="left" width="230" colspan="2">
									<%if (courseId.equals(map.get("r5100").toString())){ %>
										<font color="red">*</font>
									<%} %>
								    <a href="/train/resource/mylessons/learncourse2.jsp?opt=sss&classes=<%=SafeCode.encode(PubFunc.encrypt(classes)) %>&lesson=<%=SafeCode.encode(PubFunc.encrypt(lessonId)) %>&course=<%=SafeCode.encode(PubFunc.encrypt(map.get("r5100").toString())) %>" onclick="javascirpt:if (document.getElementById('WMP')) {pos=document.getElementById('WMP').controls.currentPosition;}"><%=map.get("r5103") %></a>
								 </td>
								<td nowrap="nowrap">
								...
								<%if (!"3".equals(map.get("r5105")) && courseId.equals(map.get("r5100").toString()) &&(!"2".equals(map.get("state")))) {%>
								<%} %>
								</td>
							</tr>
							<tr>
							...
							<%} else { %>
								<tr>
									<td colspan="3">学习状态：<%if ("0".equals(map.get("state"))) {%>
										<span id="span_<%=map.get("r5100") %>">未学</span>
									<%} else if ("1".equals(map.get("state"))) {%>
										<span id="span_<%=map.get("r5100") %>">正学</span>
									<%} else if ("2".equals(map.get("state"))) {%>
										<span id="span_<%=map.get("r5100") %>">已学</span>
									<%} %>
									</td>
								</tr>
							<%} %>
	...
	<%}else{ %>
		<table width="30%"  border="0" cellspacing="0" align="center" cellpadding="0" class="ListTable" style="margin-top:20px;">
			...
			<tr><td style="border: solid 1px;height: 100px" class="common_border_color" align="center" valign="middle">您暂时还不能试听或查看该课程！</td></tr>
		</table>
	<%} 
...
<script type="text/javascript">
	...
	function PlayFlv(divid,filePath){	
		...
		if (fileNa.toLowerCase().indexOf('.mp3') != -1 || fileNa.toLowerCase().indexOf('.mp4') != -1 || fileNa.toLowerCase().indexOf('.flv') != -1 || fileNa.toLowerCase().indexOf('.f4v') != -1) {
			...
			var htmlStr ="";
			htmlStr += "<a href='http://get.adobe.com/cn/flashplayer/download/' style='display:block;width:"+619+"px;height:"+330+"px'";  
...
function delNote() {
	if (document.getElementById("noteid").value && document.getElementById("noteid").value.length > 0) {
		...
		var map = new HashMap();
		map.put("R5100","<%=SafeCode.encode(PubFunc.encrypt(courseId))%>");
		map.put("id",document.getElementById("noteid").value);
		Rpc({functionId:'2020030173',success:succDelNote},map);
	}
}
...
function succDelNote(response) {
	...
	if(map.succeed){
		alert("删除成功！");
		document.getElementById("noteText").value = map.comment;
		document.getElementById("noteid").value = map.id == null ? "" : map.id;
		document.getElementById("noteupid").value = map.upId == null ? "" : map.upId;
		document.getElementById("notenextid").value = map.nextId == null ? "" : map.nextId;
		upNote();
	}
}
...
function upNote() {
	if (document.getElementById("noteupid").value && document.getElementById("noteupid").value.length > 0) {
		var map = new HashMap();
		map.put("R5100","<%=SafeCode.encode(PubFunc.encrypt(courseId))%>");
		map.put("id",document.getElementById("noteupid").value);
		Rpc({functionId:'2020030171',success:succUpNote},map);
	}
}

function succUpNote(response) {
	var value=response.responseText;
	var map=Ext.decode(value);
	if(map.succeed){
		...
		document.getElementById("noteupid").value = map.upId == null ? "" : map.upId;
		document.getElementById("notenextid").value = map.nextId == null ? "" : map.nextId;
		if (map.upId &&map.upId.length > 0) {
			document.getElementById("up").style.display = "";
		} else {
			//document.getElementById("up").style.display = "none";
		}
		
		if (map.nextId && map.nextId.length > 0) {
...
function nextNote() {
	if (document.getElementById("notenextid").value && document.getElementById("notenextid").value.length > 0) {
		var map = new HashMap();
		map.put("R5100","<%=SafeCode.encode(PubFunc.encrypt(courseId))%>");
		map.put("id",document.getElementById("notenextid").value);
		Rpc({functionId:'2020030171',success:succUpNote},map);
	}
}
...
function saveNote() {
	...
	map.put("state","1");
	map.put("comment",comment);  
	Rpc({functionId:'2020030170',success:succNote},map);
}

// 保存笔记成功
function succNote(response) {
	var value=response.responseText;
	var map=Ext.decode(value);
	if(map.succeed){
		alert("保存成功！");
		...
		if (document.getElementById("noteid").value && document.getElementById("noteid").value.length > 0) {
			...
			document.getElementById("noteText").value = "";
			document.getElementById("noteid").value = "";
			document.getElementById("noteupid").value = map.id == null ? "" : map.id;
			document.getElementById("up").style.display = "";
			document.getElementById("notesave").disabled = true;
		}
...
function initNote() {
	...
	Rpc({functionId:'2020030171',success:succinitNote},map);
}
function succinitNote(response) {
	var value=response.responseText;
	var map=Ext.decode(value);
	if(map.succeed){
	if(document.getElementById("noteupid") != null &&
			document.getElementById("noteupid") != "undefined"){	
		document.getElementById("noteupid").value = map.upId == null ? "" : map.upId;
		}
	}
}
...
function saveTime() {
	...
	if (document.getElementById("WMP")) {
		...
		map.put("learned",pos+"");
		map.put("R5100","<%=SafeCode.encode(PubFunc.encrypt(courseId))%>");
		map.put("total",document.getElementById("WMP").currentMedia.duration+"");
		Rpc({functionId:'2020030172',success:succsaveTime},map);
	}
}
...
function saveTime2() {
	...
	var map = new HashMap();
	if (document.getElementById("WMP")) {
		map.put("learned",document.getElementById("WMP").currentMedia.duration);
		map.put("R5100","<%=SafeCode.encode(PubFunc.encrypt(courseId))%>");
		map.put("total",document.getElementById("WMP").currentMedia.duration);
		Rpc({functionId:'2020030172',success:succsaveTime},map);
	}
