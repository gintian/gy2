...
<script language="javascript">
 ...
 function wsubmit(){
 	...
 	var	IllegalString = "?*:\"<>\\/|";
	var ft="";
	for(var i=0;i<ftitle[0].value.length;i++){
		ft=ftitle[0].value.substring(i,i+1);
	...
	if(filesort.length==0 || filesort[0].value.length==0){
		...
		var fileobj=document.getElementsByName("mediafile");
	 	...
	 	if(!AxManager.setup(null, 'FileView', 0, 0)){
	 		multMediaForm.action = "/workbench/media/upmediainfo.do?b_appsave=link&returnvalue=${multMediaForm.returnvalue}";
...
<table width="500" border="0" cellpadding="0" cellspacing="0" align="center" class="ftable">
    ...
    <tr>
          ...
          <tr>
             ...
             <td align="left"  nowrap ><html:text name="multMediaForm" property="filetitle" styleClass="inputtext" value="" onkeyup="ValidateValue(this)"/></td>
          </tr>
         <tr>
             <td align="right"  nowrap ><bean:message key="conlumn.mediainfo.filename"/></td>
             <td align="left"  nowrap ><html:file name="multMediaForm" property="mediafile" onchange="getFileName();" styleClass=""/></td> 
    </tr>
    <tr>
       <td  nowrap colspan="2" style="height:35px;" align="center">
        
            <input type="button" class="mybutton" id="butonid" 
            	onclick="document.multMediaForm.target='_self';validate('R','filetitle','标题','R','mediafile','上传文件');if(document.returnValue&&validatefilepath())wsubmit();"
            	value='<bean:message key="button.save"/>'/>
                 
	        <hrms:submit styleClass="mybutton"  property="b_appclear">
                    <bean:message key="button.clear"/>
	       </hrms:submit>    
	        <hrms:submit styleClass="mybutton"  property="b_appreturn">
