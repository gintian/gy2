...
<script>
	function doSubmit(value)
	{
		...
		else
		{
			if(value==3)
			{
				// statisticForm.action="/performance/showkhresult/showTotalEvaluate.do?b_query=link&planid="+statisticForm.planNum.value+"&objectid=<%=(userView.getA0100())%>&type=0";
                //  statisticForm.target="il_body";
               
                // statisticForm.submit();
                 
                 window.open("/performance/showkhresult/showTotalEvaluate.do?b_query=link&planid="+statisticForm.planNum.value+"&objectid=<%=(userView.getA0100())%>&type=5","tbi_body");
			
			}
			else if(value==4)
			{
				 window.open("/performance/showkhresult/showDirectionAnalyse.do?b_init=link&objectid=<%=(userView.getA0100())%>&operate=0","il_body");
			
			
			}
			else if(value!=2)
			{
				//location.href="/selfservice/performance/statistic.do?b_query=link&picFlag="+value+"&planFlag=2";
			      statisticForm.action="/selfservice/performance/statistic.do?b_query=link&picFlag="+value+"&planFlag=2";
                             statisticForm.target="il_body";
                             statisticForm.submit();
			}	
			else
			{
			    //location.href="/selfservice/performance/statistic.do?b_query2=link&picFlag="+statisticForm.planNum.value+"&planFlag=2";
			     statisticForm.action="/selfservice/performance/statistic.do?b_query2=link&picFlag="+statisticForm.planNum.value+"&planFlag=2";
                             statisticForm.target="il_body";
                             statisticForm.submit();
			}
...
//活动计划
		...
		else
		{
			...
			else if(statisticForm.drawingFlag.value!=2)
			{
				//location.href="/selfservice/performance/statistic.do?b_query=link&planNum="+value+"&planFlag=2";
			      statisticForm.action="/selfservice/performance/statistic.do?b_query=link&planNum="+value+"&planFlag=2";
                  statisticForm.target="il_body";
                  statisticForm.submit();
			}
			else
			{
				//location.href="/selfservice/performance/statistic.do?b_query2=link&picFlag="+value+"&planFlag=2";	
			     statisticForm.action="/selfservice/performance/statistic.do?b_query2=link&picFlag="+value+"&planFlag=2";	
                 statisticForm.target="il_body";
                 statisticForm.submit();
			}
		}
	...
	function set(){
		...
		if(chart != null){
			//location.href="/selfservice/performance/statistic.do?b_query=link&picFlag=1&planFlag=2&chartParameters="+chart;
		         statisticForm.action="/selfservice/performance/statistic.do?b_query=link&picFlag=1&planFlag=2&chartParameters="+chart;	
                         statisticForm.target="il_body";
                         statisticForm.submit();
		}else{
			return;
...
<table border="0" width="70%" cellpmoding="0" cellspacing="0"  class="DetailTable"  cellpadding="0">
 	<tr class="list3" >
