...
<script language="JavaScript">
   function change()
   {
      ...
      staticFieldForm.action="/general/static/select_field.do?b_next=link";
    ...
    if(right_vo.options.length==0)
     {
       ...
       if(right_vo.options[0].text==left_vo.options[i].text)
       {
           var no = new Option();
           no.value=left_vo.options[i].value;
           no.text=left_vo.options[i].text;
		...
		<tr>
			<td valign="top">
				<table width="100%" border="0" cellspacing="0" align="center"
					...
					<tr>
						<td align="left" width="48%"><bean:message key="static.target" /></td>
					...
					<tr>
						<td height="250px">
							<table width="100%">
								...
								<tr><td height="225px" align="left">
									<select name="left_fields" multiple="multiple" ondblclick="additems('left_fields','right_fields');" style="height:100%; width: 100%; font-size: 9pt;">
									</select>
								</td></tr>
							</table>
						</td>
						...
						<hrms:optioncollection name="staticFieldForm" property="rightFieldList" collection="selectedlist"/> 
							<html:select name="staticFieldForm" property="right_fields" multiple="multiple" size="10" ondblclick="removeitem('right_fields');" style="height:100%;width:100%;font-size:9pt;">
							<html:options collection="selectedlist"   property="dataValue" labelProperty="dataName"/>
							</html:select>
						</td>
						<script>/*add by xiegh bug36218  ie浏览器刷新界面时，里面的参数没有消失  添加代码刷新时执行清除  */
								var selectedlist = '${staticFieldForm.selectedlist}';
								if(selectedlist.length == 0){
									var optionObj = document.getElementsByName("right_fields")[0];
									optionObj.innerHTML = "";
								}
						</script>
					</tr>
					<tr>
						<td align="center" class="RecordRow" nowrap colspan="3"
							style="height: 35;border-bottom: none;border-left: none;border-right: none;border-right-style: solid;border-right-width: 1px;">
