...
function delete_stat()
{
   ...
   if(currnode.uid=="root")
    ...
    var request=new Request({asynchronous:false,onSuccess:delete_ok,functionId:'11080204093'},hashvo);  	
}
    ...
    if(opflag=="true")
    {
      ...
      if(preitem.childNodes.length>0){
	      ...
	      if(action=="javascript:void(0)")
	         action="/general/static/commonstatic/history/statshow.do?b_chart=link";
	      historyStatForm.action=action;
	      historyStatForm.target="mmil_body";
      	...
      	if(preitem.uid==preitem.text){
		      ...
		      if(action=="javascript:void(0)")
		         action="/general/static/commonstatic/history/statshow.do?b_data=link";
		      historyStatForm.action=action;
		      historyStatForm.target="mmil_body";
		      historyStatForm.submit();
      	}else{
      		 var action="/general/static/commonstatic/history/statshow.do?b_data=link";
		      historyStatForm.action=action;
		      historyStatForm.target="mmil_body";
		      historyStatForm.submit();
      	}
...
function add_stat()//新增只能新增统计项
{
   ...
   {	
       var theurl="/general/static/commonstatic/editstatic/history.do?b_edit=link`count=1`statid="+statid+"`editid=`infor_Flag=${historyStatForm.infokind}`opflag=new`chart_type=${historyStatForm.chart_type}";
       ...
       if(getBrowseVersion()){
        	...
        	//window.open(iframe_url);        
        	 ...
        	 if(return_vo.flag=="true")
        	 {
            	 ...
            	 parent.frames['mil_menu'].add(uid,text,action,"mmil_body",text,imgurl,"");
            	 historyStatForm.action=action;
           		 historyStatForm.target="mmil_body";
           	  	 historyStatForm.submit();
         	}
   ...
   {
        var theurl="/general/static/commonstatic/editstatic/history.do?b_edit=link`count=2`statid="+statid+"`editid=`infor_Flag=${historyStatForm.infokind}`opflag=new`chart_type=${historyStatForm.chart_type}";
       var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
       var dw=620,dh=520,dl=(screen.width-dw)/2;dt=(screen.height-dh)/2;
       if(isIE6()){
          dw += 10;
     	  ...
     	  if(return_vo)
         	if(return_vo.flag=="true")
         	{
            	...
            	var text=return_vo.legend;
             	var uid=return_vo.uid;
             	var imgurl="/images/prop_ps.gif";
             	parent.frames['mil_menu'].addparent(uid,text,action,"mmil_body",text,imgurl,"");
             	historyStatForm.action=action;
             	historyStatForm.target="mmil_body";
             	historyStatForm.submit();
         	}
   ...
   }else
   {
      ...
      var theurl="/general/static/commonstatic/editstatic/history.do?b_add=link&infor_Flag=${historyStatForm.infokind}&opflag=new&categories="+getEncodeStr(categories)+"&type=1&count=3";
      theurl = theurl.replace(/&/g,"`");
      var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
      var return_vo;
      var dw=352,dh=560,dl=(screen.width-dw)/2;dt=(screen.height-dh)/2;
      if(getBrowseVersion()){
      		 return_vo= window.showModalDialog(iframe_url,'_blank','dialogLeft:'+dl+'px;dialogTop:'+dt+'px;dialogHeight:640px;dialogWidth:502px;center:yes;help:no;resizable:no;scroll:no;status:no;');
      		//window.open(theurl);
      		if(return_vo)
         	if(return_vo.flag=="true")
         	{
         		...
         		if(categories.length>0){
	             	...
	             	//var currnode=parent.frames['mil_menu'].Global.selectedItem;
	             	parent.frames['mil_menu'].addNameCategories(uid,text,action,"mmil_body",categories,imgurl,xml,categories);
            	}else{
	             	var action=return_vo.action;
	             	var text=return_vo.text;
	             	var uid=return_vo.uid;
	             	...
	             	var imgurl="/images/groups.gif";
	             	//var currnode=parent.frames['mil_menu'].Global.selectedItem;
	             	parent.frames['mil_menu'].addName(uid,text,action,"mmil_body",text,imgurl,xml);
             	}
            }
      }else{//非IE浏览器 调用弹窗 wangb 20180127
      		var dialog=[];dialog.dw=dw+150;dialog.dh=dh;dialog.iframe_url=iframe_url;
       		openWin(dialog);
      }
   }
}
		...
		if(return_vo&&return_vo=="ok")
     	 {
    		...
    		if(currnode!=null) {
         	...
         	if(centerIFrame==null)
          	...
          	centerIFrame.src="/general/static/commonstatic/history/statitemtree.jsp";          
      	}
...
function update_stat()
{
   ...
   if(opid!="root"&&action.indexOf("norder")!=-1)
   {
       ...
       if(norder.indexOf("&")!=-1)
         norder=norder.substring(0,norder.indexOf("&"));       
       var theurl="/general/static/commonstatic/editstatic/history.do?b_edit=link`statid="+statid+"`editid="+norder+"`infor_Flag=${historyStatForm.infokind}`opflag=edit`count=4";
       var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
   ...
   {
   	var currn=parent.frames['mil_menu'].Global.selectedItem;
    var theurl="/general/static/commonstatic/editstatic/history.do?b_add=link&infor_Flag=${historyStatForm.infokind}&opflag=edit&statid="+statid+"&count=5";
   	var dw=352,dh=560;
   	if(currn){
   		if(currn.uid==currn.text){
   			theurl="/general/static/commonstatic/editstatic/history.do?b_add=link&infor_Flag=${statForm.infokind}&opflag=edit&categories="+getEncodeStr(currn.text)+"&type_categories=categories&count=5";
   		 	dw=352;
   		 	dh=200;
   		}else{
   			if(getBrowseVersion()){
...
function order_stat()
{
    ...
    if(currnode!=null) {
      ...
      var theurl="/general/static/commonstatic/editstatic/history.do?b_order=link`infor_Flag=${historyStatForm.infokind}`count=order";  
	  if (""!=statid)
...
function infosetup(){
   ...
   // alert(currnode.uid+currnode.text);
    if(currnode.uid=='root'){   
    	//alert("dddd"); 
		var theurl="/general/static/commonstatic/statshowsetup.do?b_query=link&inforkind=${historyStatForm.infokind}";
		theurl = theurl.replace(/&/g,"`");
		var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
	    var return_vo;
...
function exeshow(outparamters){
	...
	if(msg!="error"){
		var theurl="/general/static/commonstatic/statshowsetup.do?b_setup=link&id="+id+"&name="+getEncodeStr(name)+"&infokind=${historyStatForm.infokind}&type="+msg+"&count=exeshow";
   		theurl = theurl.replace(/&/g,"`");
		var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
		var return_vo;
		var dw=600,dh=500;
		...
		var dl=(screen.width-dw)/2,dt=(screen.height-dh)/2;
   		if(getBrowseVersion()){
   			return_vo= window.showModalDialog(iframe_url,name,'dialogLeft:'+dl+'px;dialogTop:'+dt+'px;dialogHeight:'+dh+'px;dialogWidth:'+dw+'px;center:yes;scroll:no;help:no;resizable:no;status:no;');
			if(return_vo&&return_vo.length>0){
				var currnode=parent.frames['mil_menu'].Global.selectedItem;
				var pnode=currnode.parent;
				while(pnode.uid!='root'&&pnode.uid!=pnode.text){
   		   ...
   		   titlename = name;
		   //谷歌浏览器修改窗口样式 wangbs 20190314
   			var dialog=[];dialog.dw=dw+15;dialog.dh=dh-20;dialog.iframe_url=iframe_url;
       		openWin(dialog);
   		}
	}
}
   ...
   // alert(currnode.uid+currnode.text);
    if(currnode.uid!='root'&&currnode.uid!=currnode.text){
		...
		if(msg=="1"||msg=="2"){//在这里只有一维和二维才能设置统计方式
			var theurl="/general/static/commonstatic/statshowsetup.do?b_sformula=link&id="+id+"&name="+getEncodeStr(name)+"&count=sformula&infokind=${historyStatForm.infokind}";
			var iframe_url="/general/query/common/iframe_query.jsp?src="+$URL.encode(theurl);
   			iframe_url = iframe_url.replace(/&/g,"`");
			var return_vo;
			var dw=600,dh=625,dl=(screen.width-dw)/2;dt=(screen.height-dh)/2;
			if(getBrowseVersion()){
...
<hrms:themes />
<body border="0" cellspacing="0" cellpadding="0" onload="isShow('${historyStatForm.infokind}');">
<html:form action="/general/static/commonstatic/history/statshow" > 
<!-- 非兼容模式浏览器兼容修改：按钮看不全被遮住一半，去掉margin-left guodd 2018-03-01 -->
   <table width="1000" style="margin-top:0px;" border="0" cellspacing="0"  align="center" cellpadding="0" >     
   <tr align="left" class="toolbar" style="height:40px;line-height:40px;">
		<td valign="middle" align="left" style="padding-left:4px;">
		<logic:equal name="historyStatForm" property="infokind" value="2">
		   ...
		   <hrms:priv func_id="231706">
		    <img src="/images/sys_config.gif" border=0 title='<bean:message key="button.info.setup"/>' style="cursor:pointer;display:none;" onclick="infosetup();"/>		
		   </hrms:priv>
		   <hrms:priv func_id="231707">
		    <img src="/images/img_q.gif" border=0 title='<bean:message key="button.info.sformula"/>' style="cursor:pointer;display:none;" onclick="sformula();"/>		
		   </hrms:priv>
		</logic:equal>
		...
		<logic:equal name="historyStatForm" property="infokind" value="3">
...
if(!getBrowseVersion()){ //非IE浏览器兼容性   wangb 20180127
	var form = document.getElementsByName('historyStatForm')[0];
