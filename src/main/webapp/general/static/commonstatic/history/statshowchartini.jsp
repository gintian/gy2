...
-->
     ...
     <SCRIPT LANGUAGE=javascript src="/js/validate.js"></SCRIPT>
       ...
       function returnhome()
       {
            ...
            historyStatForm.action="/workbench/browse/history/showinfo.do?b_search=link&action=showinfodata.do&target=nil_body";
       ...
       function testchart(e)
       {
    	  var key,name, backdates;
   		  ...
   		  if("${historyStatForm.chart_type}" == '29'){
   			...
   			name = e.name;//传入的是统计项的名称不是 统计图名称  wangb 20180713 bug 50155
   		  }
      	  if(name!="")
      	  {
       ...
       function changechar_type(val){
       		historyStatForm.action="/general/static/commonstatic/history/statshowchart.do?b_chart=link&chart_type="+val;
       		historyStatForm.submit();
       }
...
{ 
   ...
   for(var i=0;i<unArrs.length;i++)
   {
    ...
    document.getElementById('dict').style.top=aTag.offsetTop+42;
} 
function onV(j,flag){
...
function hiddendict(val){
	<logic:equal  name="historyStatForm"  property="chart_type"  value="11" >
		...
		if(oldbackdates!=backdates){
			historyStatForm.action="/general/static/commonstatic/history/statshowchart.do?b_chart=link";
	       	historyStatForm.submit();
		}
	</logic:equal>
...
function getbackdates(){
	...
	var config = {id:'getbackdatesini_showModalDialogs',width:dw,height:dh};
    ...
    var return_vo= window.showModalDialog(thecodeurl, "_blank", 
              "dialogLeft:"+dl+"px;dialogTop:"+dt+"px;dialogWidth:320px; dialogHeight:"+dh+"px;center:yes;help:no;resizable:no;status:no;scroll:no;");
...
function closeExtWin(){
	if(Ext.getCmp('getbackdatesini_showModalDialogs'))
		Ext.getCmp('getbackdatesini_showModalDialogs').close();
}
   </SCRIPT>
   <hrms:themes />
<br>
   ...
   <logic:equal name="historyStatForm" property="infokind" value="1">
    ...
    <td align="center">
	    <%-- <input type="button" name="b_save" value="<bean:message key="button.return"/>" class="mybutton" onclick="returnhome()"> --%>
	    	&nbsp;&nbsp;<a onclick="returnhome()"  href="###" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;返回</a><!-- style="color:#00008b" -->
    </td>
  </tr>
  </table>
  <br><br>
	...
	<tr>
		<td>
         ...
         <logic:equal  name="historyStatForm"  property="chart_type"  value="5" >
         	<hrms:chart name="historyStatForm" title="${historyStatForm.snamedisplay}" scope="session" legends="list" data="" width="1200" height="350" chart_type="${historyStatForm.chart_type}" pointClick="testchart">
	 		</hrms:chart>
         </logic:equal>  
         <logic:equal  name="historyStatForm"  property="chart_type"  value="11" >
         	<hrms:chart name="historyStatForm" title="${historyStatForm.snamedisplay}" scope="session" legends="jfreemap" data="" width="1200" height="350" chart_type="${historyStatForm.chart_type}" pointClick="testchart">
	 		</hrms:chart>
         </logic:equal>     
        <logic:notEqual name="historyStatForm"  property="chart_type"  value="5" >   
         <logic:notEqual name="historyStatForm"  property="chart_type"  value="11" >         
	 	   <hrms:chart name="historyStatForm" title="${historyStatForm.snamedisplay}" scope="session" legends="list" data="" width="1200" height="530" chart_type="${historyStatForm.chart_type}" pointClick="testchart">
	 	    </hrms:chart>
	 	 </logic:notEqual>
        </logic:notEqual>
            </td>
