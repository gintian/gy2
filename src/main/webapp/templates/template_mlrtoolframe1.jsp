...
<frameset cols="250,*"  bordercolor='#DCEFFE' framespacing="2" >
  ...
  <frame src="<hrms:insert parameter="HtmlBody" />" frameborder="1"  name="mmil_body" scrolling="auto" >
</frameset>
 <%}%>
  <%if(ver>40){%>
 <script type="text/javascript">
	Ext.onReady(function() {
		...
		header:false,
		 ...
		 region:'west' 
		,width:180 
		,html:'<iframe src="<hrms:insert parameter="HtmlTool" />" <%if("hcm".equals(bosflag)){%>height="40"<%}else{%>height="23"<%}%> width="100%" name="nil_tool" id="top_iframe" scrolling="no" frameborder="0"></iframe><iframe src="<hrms:insert parameter="HtmlMenu" />" name="mil_menu" id="center_iframe" scrolling="auto" width="100%" height="100%" frameborder="0"></iframe> '
		,collapsible:false
		,split:true 
		,collapseMode:'mini' 
			,border:<%=bborder%>
		...
		header:false, 
		 region:'center' 
	    ,margins:'-1 -1 -1 0'			 
		,html:'<iframe src="<hrms:insert parameter="HtmlBody" />" name="mmil_body" id="center_iframe" scrolling="auto" width="100%" height="100%" frameborder="0"></iframe> ' 
			,border:<%=bborder%>
		}] 
		}); 		
	});  
